...
var table = $('#statisticsDayList').dataTable({
    ...
    "aoColumns": [
                ...
                { "data": "stName",
                "render":function(data, type, full, meta){
                	var returnInfo = "<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"group_day('"+full.stName+"->街道办件量统计','/statistics/statistics/itemStreetQuery.do','"+data+"')\" href=\"javascript:;\" title=\"查看\">"+data+"</a>";
                     return returnInfo;
      ...
      "bLengthChange": false,//改变每页显示数据数量
       "ajax": {
		    url:webRoot+'/statistics/selmStatistics/addresslistType.do',
		    type:"POST",
		    data: function(d){
		    /* var stAreaId = areaId;
		      d.stAreaId = stAreaId; 
		      ...
		      d.stPermission = stPermission;  */
		       var stItemName = '<%=selmStatistics.getStName()%>';
...
function group_day(title,url,name){
    var index = layer.open({
        type: 2,
        title: title,
        content: webRoot+url+'?stName='+name+'&itemName='+'<%=selmStatistics.getStName()%>'+'&stDate='+$("#startDate").val()+'&edDate='+$("#endDate").val()
    });
    layer.full(index);
} 
