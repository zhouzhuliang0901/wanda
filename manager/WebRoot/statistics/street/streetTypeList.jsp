		...
		<div class="mt-20">
			<table width="100%"
				...
				<tbody>
					...
					<tr>
						<td style="pointer-events: none;">总计</td>
						<td style="pointer-events: none" ng-repeat="name in Name">{{name.count}}</td>
...
$scope.Item=[];
  ...
  if(stStreetName=="undefined"){
  	...
  	$.ajax({
  		...
  		//dataType: 'jsonp',
  		jsonp: 'jsonpCallback',
  		url: webRoot+"/statistics/selmStatistics/streetlistTypeDevice.do",
  		data: {
  			
  			stAddressName:stAreaName || '',
  			stStreetName: stStreetName || '',
  			stPermission:permission || '',
  			stAreaId:areaId || '',
  			fCode:'statistics',
  			sCode:'selmStatistics',
  			tCode:'streetlistTypeDevice.do'
  		},
			...
			if(tdName!=""){
				...
				tdName = encodeURI(encodeURI(tdName));
				 var index = layer.open({
        				type: 2,
        				title: '设备分类',
        				content: webRoot+'/statistics/statistics/singlestreetInfo.do?areaName='+trName+'&typeName='+tdName,
    				});
    					layer.full(index);
				} 
