...
<body>
	...
	<div class="page-container">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;所属街道: <input type="text" name="" id="searchStreet" placeholder="所属街道" style="width:250px"
			...
			<h5>注意：如需查看设备一级的信息，请点击街道名。如需查看操作统计，请点击办件量。感谢使用！！！</h5>
		</div>
...
var table = $('#statisticsDayList').dataTable({
    ...
    "aoColumns": [
      ...
      "bLengthChange": false,//改变每页显示数据数量
       "ajax": {
		    url:webRoot+'/statistics/selmBStreetQueryDay/list.do',
		    type:"POST",
		    data: function(d){
		    var stAreaId = areaId;
		      d.stAreaId = stAreaId; 
...
function group_day(title,url,name){
    var index = layer.open({
        type: 2,
        title: title,
        content: webRoot+url+'?ST_NAME='+name+'&ST_EXT1='+$("#startDate").val()+'&ST_EXT2='+$("#endDate").val()
    });
    layer.full(index);
} 
...
function item(title,url,name){
    ...
    var index = layer.open({
        type: 2,
        title: title,
        content: webRoot+url+'?street='+name+'&startDate='+$("#startDate").val()+'&endDate='+$("#endDate").val()+'&area='+'<%=selmStatistics.getStName()%>'
    });
    layer.full(index);
} 
