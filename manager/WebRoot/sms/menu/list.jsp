...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>
        系统管理 <span class="c-gray en">&gt;</span> 菜单设置
...
function beforeClick(treeId, treeNode){
     ...
     demoIframe.attr("src",webRoot+"/sms/menu/edit.do?ST_MENU_ID="+
           treeNode.stMenuId+"&ST_PARENT_CATEGORY_ID="+treeNode.stParentId);
     return true;
}
...
function onRename (event, treeId, treeNode, isCancel) {
     //询问框
     layer.confirm('确认修改目录名为:'+treeNode.stMenuName, { btn: ['确认','取消'] }
      ,function(){
          $.ajax({
              type : "POST",
              url : webRoot+'/sms/menu/save.do',
              dataType:"json",
              data:{"ST_MENU_ID":treeNode.stMenuId,
                    "ST_MENU_NAME":treeNode.stMenuName},
...
function beforeRemove(treeId, treeNode) {
    // 根目录的场合、禁止删除
    if (treeNode.stMenuId == "sms_menu_root") {
    ...
    paramsArray.push(treeNode.stMenuId);
    // 父节点的场合
    if(treeNode.isParent){
         ...
         for(var i = 0; i < childNodes.length; i++){
            // 根目录的场合、禁止删除
		    if (childNodes[i].stMenuId == "sms_menu_root") {
		        layer.msg('当前目录禁止删除',{icon:2,time:1000});
		        return false;
		    }
             paramsArray.push(childNodes[i].stMenuId);
         }
     }
     //询问框
     layer.confirm('确认删除该级目录和目录下的样表？', { btn: ['确认','取消'] }
      ,function(){
	       $.ajax({
	          type : "POST",
	          url : webRoot+'/sms/menu/remove.do',
	          dataType:"json",
	          data:{
	                "stMenuId":paramsArray},
	          success : function(data) {
        ...
        if (treeNode.isParent) {
			layer.confirm('请选择所要添加的节点', {
			   btn: ['同级节点','子节点','取消'] 
			     ...
			     // 添加子节点
                addCategory (treeNode, treeNode.stMenuId);
			 }, function(){
			    layer.msg('取消', {icon: 1});
			 });
        } else {
   ...
   var zTree = $.fn.zTree.getZTreeObj("treeDemo");
    $.ajax({
          ...
          data:{ "ST_PARENT_ID":parentId,
                 "ST_MENU_NAME":"请输入目录名称"},
          success : function(data) {
              // 添加目录
              zTree.addNodes(treeNode, {stMenuId: data.SmsMenu.stMenuId,
               stParentId: parentId, stMenuName:"请输入目录名称"});
