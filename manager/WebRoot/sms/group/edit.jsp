...
<body>
	<div class="page-container">
		<form class="form form-horizontal" id="form-group-add">
			<input type="hidden" name="<%=SmsGroup.ST_GROUP_ID %>" value="<%=smsGroup.stGroupId2Html() %>" /> 
			...
			<div class="row cl">
				...
				<div class="formControls  col-xs-4 col-sm-4">
					<input type="text" class="input-text"
						value="<%=smsGroup.stGroupCode2Html() %>" 
						name="<%=SmsGroup.ST_GROUP_CODE %>" required>
				</div>
			</div>
            ...
            <div class="row cl">
                ...
                <div class="formControls  col-xs-4 col-sm-4">
                    <input type="text" class="input-text"
                        value="<%=smsGroup.stGroupName2Html() %>"
                        name="<%=SmsGroup.ST_GROUP_NAME %>" required >
                </div>
            </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls  col-xs-4 col-sm-4">
                    <input type="text" class="input-text"
                        value="<%=smsGroup.nmOrder2Html(0) %>" digits="true"
                        name="<%=SmsGroup.NM_ORDER %>" >
                </div>
            </div>
            
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2">组描述：</label>
                <div class="formControls col-xs-4 col-sm-4">
                    <textarea name="<%=SmsGroup.ST_DESC %>"  cols="" rows="" class="textarea"
                        placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true"
                        nullmsg="备注不能为空！" onKeyUp="textarealength(this,100)"><%=smsGroup.stDesc2Html()%></textarea>
...
function article_save_submit() {
	$.ajax({
		type : "POST",
		url : webRoot + "/sms/group/save.do",
		data : $('#form-group-add').serialize(),// 你的formid
		error : function(request) {
                  layer.msg("用户组信息添加失败", {icon : 2, time : 1000});
		},
		success : function(data) {
			layer.msg("用户组信息添加成功", {icon : 1, time : 1000});
		}
	});
