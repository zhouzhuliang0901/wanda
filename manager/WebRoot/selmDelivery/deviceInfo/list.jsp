...
<body>
	<nav class="breadcrumb">
		...
		证照管理 <span class="c-gray en">&gt;</span>设备信息 <a
	...
	<div class="page-container">
		...
		<div class="text-x" style="text-align: left;">
			设备区域：
			<select name="searchDistrictId" id="deviceDistrictSelect" class="input-text" style="width:250px" onchange="streetType()">
				<script id="districtList" type="text/html">
					 {{each data}}
                        <option value="{{$value.stDistrict}}" {{if $value.stAddressId == ""}} selected='selected' {{/if}}>
										{{$value.stDistrict}}</option>
                     {{/each}} 
				</script>
			 ...
			 <!-- &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;设备街道：
			<select name="searchStreet" id="searchStreetSelect" class="input-text" style="width:250px">
				<script id="streetList" type="text/html">
						 {{each data}}
                        <option value="{{$value.stStreet}}" {{if $value.stAddressId == ""}} selected='selected' {{/if}}>
										{{$value.stStreet}}</option>
                     {{/each}} 
				</script>
			</select> -->
			 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;设备街道：<input name="searchStreet" id="searchStreetSelect" type="text" class="input-text" style="width:250px" list="streetList" autocomplete="off">
		...
		<%-- <%if(((Object)request.getSession().getAttribute("权限")).equals("project_admin")){ %> --%>
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
					<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
					     <i class="Hui-iconfont">&#xe6e2;</i>
							批量删除</a>
					<a class="btn btn-primary radius" onclick="device_add('添加设备','../../infopub/deviceinfo/add.jsp')" href="javascript:;">
...
var table = $('#deviceList').dataTable({
    ...
    "aoColumns": [
                  ...
                  }},
                {
                 "data": "stDeviceId",
                 "render": function (data, type, full, meta) {
                        var returnInfo = "<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"device_info('"+full.stDeviceName+"->查看','/infopub/delivery/deliveryDeviceInfo.do','"+data+"')\" href=\"javascript:;\" title=\"查看设备\"><i class=\"icon iconfont icon-chakan\"></i></a>";
...
//                 }
      ...
      "bLengthChange": true ,//改变每页显示数据数量
       "ajax": {
		    ...
		    type:"POST",
		    data: function(d){
		   	 /* var companyCodeId = companyId;
		      d.companyCodeId = companyCodeId;  */
		      var username = user;
		      d.userName = username;
		      var stAreaId = areaId;
		      d.stAreaId = stAreaId; 
              ...
              d.endDate= endDate;
              var deviceDistrict = $("#deviceDistrictSelect").val();
              d.deviceDistrict = deviceDistrict;
              var deviceStreet = $("#searchStreetSelect").val();
...
function districtType(){
	$.ajax({
		url : webRoot+'/infopub/deviceDistric/init.do',
		type : "POST",
		dataType : "json",
		success : function(result){
			var deviceDistrict = result.deviceDistrict;
			$("#deviceDistrictSelect").html(template('districtList',deviceDistrict));
		},
		error : function(){
		}	
	});
...
//街道查询
function streetType(){
	$("#searchStreetSelect").val("");
	var deviceDistrict = $("#deviceDistrictSelect").val();
	if(deviceDistrict == null){
		deviceDistrict = "";
	}
	$.ajax({
		url : webRoot+'/infopub/deviceStreet/init.do?distirct='+deviceDistrict,
		type : "POST",
		dataType : "json",
		success : function(result){
			var deviceStreet = result.deviceStreet.data;
			if (deviceStreet.length > 0) {
                for(var i = 0; i < deviceStreet.length; i++){
                	if(deviceStreet[i].stStreet != ""){
                		$("#streetList").append("<option value='"+deviceStreet[i].stStreet+"'>"+deviceStreet[i].stStreet+"</option>");
                	}else{
                		$("#streetList").append("<option value='"+deviceStreet[i].stStreet+"' selected='selected'>"+deviceStreet[i].stStreet+"</option>");
                	} 
                }
            }
		},
