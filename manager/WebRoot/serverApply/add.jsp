...
<%
	 ...
	 String serverApplyId = (String)request.getSession().getAttribute("serverApplyId");
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<textarea id="sqqk" rows="10" style="width:260%;height:100px;padding-top:1px;font-size:14px;" type="textarea" class="input-text"
						...
						name="<%=SelmServerApply.ST_SERVER_CONTENT%>" placeholder="申请情况说明" required></textarea>
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">
				备注：</label>
				<div class="formControls col-xs-4 col-sm-4">
					<textarea id="bz" rows="10" style="width:260%;height:100px;padding-top:1px;font-size:14px;" type="text" class="input-text"
						value="<%=selmServerApply.stExt12Html() %>"
						name="<%=SelmServerApply.ST_EXT1%>" placeholder="备注"></textarea>
				</div>
			</div>
...
var table = $('#typeList').DataTable({
    ...
    "aoColumns": [
                ...
                
                {
                 "data": "stMachineId",
                 "render": function (data, type, full, meta) {
                  		 	var returnInfo = "<a style=\"text-decoration:none;color:blue;\" class=\"ml-5\" onClick=\"operationDevice('"+full.stAuditUserId+"网点审批情况','/serverApply/selmServerApply/checkItemStatus.do','"+data+"','"+full.stAuditUserName+"')\" href=\"javascript:;\" title=\"查看关联事项\"><u>查看</u>&nbsp;</a>";
                  		 	returnInfo += "<a style=\"text-decoration:none;color:blue;\" class=\"ml-5\" onClick=\"addItem('添加事项','/serverApply/selmServerApply/addItem.do','"+data+"')\" href=\"javascript:;\" title=\"添加事项\"><u>添加事项</u>&nbsp;</a>";
...
function info(){
	...
	var sqqk = $("#sqqk").val();
	var bz = $("#bz").val();
	sessionStorage.setItem("sqdw2",sqdw);
	sessionStorage.setItem("lxr2",lxr);
	sessionStorage.setItem("sj2",sj);
	sessionStorage.setItem("gddh2",gddh);
	sessionStorage.setItem("dzyx2",dzyx);
	sessionStorage.setItem("startDate2",startDate);
	sessionStorage.setItem("sqqk2",sqqk);
	sessionStorage.setItem("bz2",bz);
}

$(function(){
	var sqdw = sessionStorage.getItem("sqdw2");
	var lxr = sessionStorage.getItem("lxr2");
	var sj = sessionStorage.getItem("sj2");
	var gddh = sessionStorage.getItem("gddh2");
	var dzyx = sessionStorage.getItem("dzyx2");
	var startDate = sessionStorage.getItem("startDate2");
	var sqqk = sessionStorage.getItem("sqqk2");
	var bz = sessionStorage.getItem("bz2");
	console.log(sqdw);
	$("#sqdw").val(sqdw);
	$("#lxr").val(lxr);
	$("#sj").val(sj);
...
function exitRemove(){
	$.ajax({
		type:"get",
		url:webRoot+"/serverApply/selmServerApply/remove.do?stApplyId="+stApplyId,
		error:function(){
		},
		success:function(result){
		}
