...
function addDevice(title){
	...
	var index = layer.open({
		type:2,
		title:title,
		content:webRoot+'/serverApply/deviceChange/choiceChangeDevice.do'+'?stDeviceApplyId='+stapplyId
	});
	layer.full(index);
}
...
function info(){
	...
	var sqqk = $("#sqqk").val();
	sessionStorage.setItem("sqdw3",sqdw);
	sessionStorage.setItem("bzbm3",bzbm);
	sessionStorage.setItem("lxr3",lxr);
	sessionStorage.setItem("sj3",sj);
	sessionStorage.setItem("gddh3",gddh);
	sessionStorage.setItem("dzyx3",dzyx);
	sessionStorage.setItem("startDate3",startDate);
	sessionStorage.setItem("sqqk3",sqqk);
}

$(function(){
	var sqdw = sessionStorage.getItem("sqdw3");
	var bzbm = sessionStorage.getItem("bzbm3");
	var lxr = sessionStorage.getItem("lxr3");
	var sj = sessionStorage.getItem("sj3");
	var gddh = sessionStorage.getItem("gddh3");
	var dzyx = sessionStorage.getItem("dzyx3");
	var startDate = sessionStorage.getItem("startDate3");
	var sqqk = sessionStorage.getItem("sqqk3");
	$("#sqdw").val(sqdw);
	$("#bzbm").val(bzbm);
	$("#lxr").val(lxr);
	$("#sj").val(sj);
...
var table = $('#typeList').DataTable({
    ...
    "aoColumns": [
                ...
                { "data": "stAddressId",
                   "render":function(data, type, full, meta){
                       return '上海市'+full.stAreaId+full.stAddressId+full.stDeviceAddress;
                }},
                { "data": "stDeviceMac",
                   "render":function(data, type, full, meta){
                       return setNullData(data);
                ...
                
                {
                 "data": "stMachineId",
                 "render": function (data, type, full, meta) {
                  		var returnInfo = "<a style=\"text-decoration:none;color:blue;\" class=\"ml-5\" onClick=\"deviceEdit('设备编辑','/serverApply/selmDeviceAlink/changeEdit.do','"+data+"')\" href=\"javascript:;\" title=\"编辑\"><u>编辑</u>&nbsp;</a>";
                        returnInfo += "<a style=\"text-decoration:none;color:red;\" class=\"ml-5\" onClick=\"removeDevice('删除设备','/serverApply/selmDeviceAlink/deviceChange/changeRemove.do','"+data+"')\" href=\"javascript:;\" title=\"删除\"><u>删除</u></a>";
                        return returnInfo;
                    },
                  "bSortable": false
                }
      ...
      "bLengthChange": true ,//改变每页显示数据数量
       "ajax": {
		    url:webRoot+'/serverApply/selmDeviceAlink/stDeviceList.do',
		    type:"GET",
		    data: function(d){
...
var formData = new FormData($("#form-device-add")[0]);
	$.ajax({
		type : "POST",
		url : webRoot + "/serverApply/selmDeviceApply/save.do?stDeviceApplyId="+stapplyId+"&type=change",
...
function removeSession(){
	$.ajax({
		type:"get",
		url:webRoot+"/serverApply/selmDeviceApply/addDeviceChangeApply.do?stDeviceApplyIdSession=ff",
		error:function(){
		},
		success:function(){
		}
...
function deviceInfo(title,url,id){
    var index = layer.open({
        type: 2,
        title: title,
        content: webRoot+url+'?stMachineId='+id
    });
    layer.full(index);
}
