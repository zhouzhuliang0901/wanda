...
<body>
<div style="padding-top: 10px;font-size: 18px;padding-left: 95px;font-weight: 600;margin-bottom: -14px;">请填写不通过的原因</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<textarea rows="10" style="width:250%;height:180px;padding-top:1px;font-size:14px;" type="textarea" class="input-text"
						value=""
						name="stResult" placeholder="小于1000字" required></textarea>
				</div>
...
var stMachineId = "<%=selmDeviceAlink.getStMachineId()%>";
var stApplyId = "<%=selmDeviceAlink.getStDeviceApplyId()%>";
$(function(){
   // 数据校验
   $("#form-device-add").validate();
	...
	var formData = new FormData($("#form-device-add")[0]);
		$.ajax({
			type : "POST",
			url : webRoot + '/serverApply/selmDeviceApply/saveNoPassReason.do?stMachineId='+stMachineId+'&stDeviceApplyId='+stApplyId,
...
function nopass() {
		$.ajax({
			type : "POST",
			url : webRoot + '/serverApply/selmDeviceAlink/deviceNoPass.do?stMachineId='+stMachineId+'&stDeviceApplyId='+stApplyId,
			error : function(request) {
				layer.msg("审核失败", {
					icon : 1,
					time : 1000
