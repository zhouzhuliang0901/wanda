					...
					.dataTable(
							{
								...
								"aoColumns" : [
										...
										}, */
										{
						                 "data": "stDeviceId",
						                 "render": function (data, type, full, meta) {
						                        var returnInfo = "<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"device_add('"+full.stDeviceName+"->授权','/app/selmDeviceItem/save.do','"+data+"')\" href=\"javascript:;\" title=\"授权\">授权</a>";
						                        return returnInfo;
						                    },
						                  "bSortable": false
						                }
								...
								"createdRow" : function(row, data, dataIndex) {
									...
									$(row).children('td').eq(4).attr('style',
											'text-align: center;');
									$(row).children('td').eq(5).attr('style',
											'text-align: center;');
								},
								"bPaginate" : true, //翻页功能
								"bLengthChange" : true,//改变每页显示数据数量
								"ajax" : {
									url : webRoot + '/app/selmItem/noOCDevicelist.do',
									type : "POST",
									data : function(d) {
										 var stAreaId = areaId;
									      d.stAreaId = stAreaId; 
										...
										var stItemId = '<%=selmItem.stItemId2Html()%>';
										d.stItemId = stItemId;
									},
									/* success : function() {
									}, */
...
function deviceAdd () {
    ...
    device_add('','/app/selmDeviceItem/save.do',idArray);
}
			// 添加
			function device_add(title,url,id){
			    $.ajax({
						...
						dataType : "json",
						data : {
							stItemId : '<%=selmItem.stItemId2Html()%>',
							stDeviceId : id
						},
...
var table1 = $('#itemList1').dataTable({
    ...
    "aoColumns": [
				...
				}},
                {
                 "data": "stDeviceId",
                 "render": function (data, type, full, meta) {
                        var returnInfo1 = "<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"device_remove('"+full.stDeviceName+"->移除','/app/selmDeviceItem/remove.do','"+data+"')\" href=\"javascript:;\" title=\"移除\">移除</a>";
                        return returnInfo1;
                    },
                  "bSortable": false
                }
      ...
      "bLengthChange": true ,//改变每页显示数据数量
       "ajax": {
		    url:webRoot+'/app/selmItem/OCDevicelist.do',
		    type:"POST",
		    data: function(d){
              ...
              var stItemId1 = '<%=selmItem.stItemId2Html()%>';
				d.stItemId1 = stItemId1;
		    },
		    error:function(){
		      window.parent.location.reload();
		    }
...
function deviceRemove () {
    ...
    for (var i = 0; i< selectList.length; i++){
        idArray.push(selectList[i].value);
   		...
   		device_remove('','/app/selmDeviceItem/remove.do',idArray);
			}
// 移除
