			...
			<table width="100%"
				...
				<thead>
					<tr class="text-c">
						...
						<!-- <th width="40">名称</th> -->
						<th width="60">当前成功次数</th>
						<th width="60">当前失败次数</th>
						<th width="40">更新时间</th>
						<th width="20">信息操作</th>										
					</tr>
				</thead> 
    ...
    "aoColumns": [
               ...
               /*  { "data": "stExt1",
                ...
                }},
                {
                 "data": "stOutDeviceResultId",
                 "render": function (data, type, full, meta) {
                        var returnInfo = "<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"odevicewarn_info('"+full.stOutDeviceCode+"->查看','/infopub/odeviceStatus/info.do','"+data+"')\" href=\"javascript:;\" title=\"查看\"><i class=\"icon iconfont icon-chakan\"></i></a>";
      ...
      "bLengthChange": true ,//改变每页显示数据数量
       "ajax": {
		    url:webRoot+'/infopub/odevicewarn/list.do',
		    type:"POST",
		    data: function(d){
		      var stTypeCode = $("#searchName").val();
		      d.stTypeCode = stTypeCode;
		      var stTypeId = '<%=infopubDeviceInfo.stTypeId2Html()%>';
		      d.stTypeId = stTypeId;
		      var stDeviceMac = '<%=infopubDeviceInfo.stDeviceMac2Html()%>';
...
// 查看
function odevicewarn_info(title,url,id){
    var index = layer.open({
        type: 2,
        title: title,
...
console.log(data)
    /* if(data == '0'){
        ...
        //return '<i class=\"iconfont icon-yichang\" style="color:green" ></i>';
    } else  */
    if(data == '1'){
    	return '<span class=\"label label-danger radius\">异常</span>';
        //return '<i class=\"iconfont icon-yichang\" style="color:red"></i>';
    }else{
    	return '<span class=\"label label-warning radius\">正常</span>';
    }
}
//是否通知
