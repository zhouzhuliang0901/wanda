...
<%
	...
	SmsOrgan smsOrgan = (SmsOrgan) request
			.getAttribute(SmsOrgan.SMS_ORGAN);
	...
	<div class="page-container">
		<form class="form form-horizontal" id="form-device-add">
			...
			<input type="hidden" value="<%=infopubDeviceInfo.stAddressId2Html()%>"
				id="addressInfo" />
			<input type="hidden" value="<%=infopubDeviceInfo.stOrganId2Html()%>"
				id="organId" />
			<div class="row cl">
 ...
 --%>
		...
		<label class="form-label col-xs-4 col-sm-2">证书唯一标识：</label>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<span class="select-box"> <select
							name="<%=InfopubDeviceInfo.ST_ORGAN_ID %>" class="select"
							id="id_organList">

							<script id="organIdList" type="text/html">
										<option value=""></option>
										{{each data}}
		...
		function initData() {
			$.ajax({
				...
				success : function(result) {
					...
					organList.organId = $("#organId").val();
					$("#id_organList").html(
							template('organIdList', organList));
