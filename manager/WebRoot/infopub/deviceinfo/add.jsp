...
<%
    ...
    String areaId = (String) request.getSession().getAttribute("smsUserStAreaId");
    String permission = (String) request.getSession().getAttribute("权限");
%>
<!DOCTYPE HTML>
<html>
<head>
    ...
    <form class="form form-horizontal" id="form-device-add">
        ...
        <input type="hidden" name="<%=InfopubOnoff.ST_DEVICE_ID%>"
            ...
            value="<%=infopubOnoff.stOnoffId2Html()%>"/> <input type="hidden"
                                                                value="<%=infopubOnoff.stPeriod2Html()%>" id="weekMsg"/>
        <input
        ...
        <input type="hidden" value="<%=infopubDeviceInfo.stTypeId2Html()%>"
               id="deviceType"/>
        <input type="hidden" value="<%=infopubDeviceInfo.stAreaId2Html()%>"
               id="area"/>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-4 col-sm-4">
                <input type="text" class="input-text"
                       ...
                       name="<%=InfopubDeviceInfo.ST_DEVICE_IP %>">
            </div>
            <%-- <label class="form-label col-xs-4 col-sm-2"><span
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-4 col-sm-4">
                <input type="text" class="input-text MacCheck"
                       ...
                       name="<%=InfopubDeviceInfo.ST_DEVICE_MAC%>" required>
            </div>
            <%-- <label class="form-label col-xs-4 col-sm-2"><span
            ...
            <div class="formControls col-xs-4 col-sm-4">
                ...
                <script id="addressList" type="text/html">
                    <option selected class='get-class' disabled>请选择地址</option>
                    {{each data}}
                    <option value="{{$value.stAddressId}}" {{if addressId== $value.stAddressId}} selected='selected'
                            {{/if}}>
...
--%>
        <div class="row cl">
            <%-- <%
                if(permission.equals("project_admin")){
            %> --%>
            <%-- <label class="form-label col-xs-4 col-sm-2"><span
                class="c-red">*</span> 所属项目：</label>
            <div class="formControls col-xs-4 col-sm-4">
                <span class="select-box"> <select
                    name="<%=InfopubDeviceInfo.ST_AREA_ID %>" class="select"
                    id="areaSelect">
                        <script id="areaList" type="text/html">
                                    {{each data}}
                                        <option value="{{$value.stAreaId}}" {{if areaId == $value.stAreaId}} selected='selected' {{/if}}>
            ...
            <%-- <%
             ...
             %>
                     <input type="hidden" name="<%=InfopubDeviceInfo.ST_AREA_ID%>"
                    id="stSampleId" value="<%=areaId%>" />
                    <label class="form-label col-xs-4 col-sm-2"><span
                        class="c-red">*</span>所属项目：</label>
                    <div class="formControls col-xs-4 col-sm-4">
                        <input type="text" class="input-text"
                            value="<%=areaId%>"
                            name="<%=InfopubDeviceInfo.ST_AREA_ID%>" disabled="disabled">
                    </div>
            ...
            <div class="formControls col-xs-4 col-sm-4">
					<span class="select-box"> <select
							...
							<script id="typeList" type="text/html">
                                        {{each data}}
                                        <option value="{{$value.stTypeId}}" {{if typeId== $value.stTypeId}}
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-4 col-sm-4">
					<span class="select-box"> <select
                            name="<%=SmsOrgan.ST_ORGAN_ID %>" class="select"
                            id="id_organList">
							...
							<script id="organList" type="text/html">
                                        ...
                                        <option value="{{$value.stOrganId}}" {{if organId== $value.stOrganId}}
                                                selected='selected' {{/if}}>
    ...
    function initData() {
        $.ajax({
            ...
            success: function (result) {
                ...
                //devicetype.addressId = $("#deviceType").val();
                $("#id_select").html(
                    template('addressList', address));

                var organList = result.organList;
                $("#id_organList").html(
                    template('organList', organList));

                /* var area = result.area;
                area.areaId = $("#area").val();
                $("#areaSelect").html(
                        template('areaList', area)); */
