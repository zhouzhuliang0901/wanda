	...
	<div class="page-container">
		<form class="form form-horizontal" id="form-device-add">
			...
			<div class="row cl">
				...
				<label class="form-label col-xs-4 col-sm-2">设备子类型：</label>
				<div class="formControls col-xs-4 col-sm-4">
					<input type="text" class="input-text"
					<%if(infopubDeviceInfo.getNmSdtype()!=null){
						if(infopubDeviceInfo.getNmSdtype().intValue()==0){ %>
						value="中心政务终端"<%}else{ %>
						value="延伸政务终端"<%} }else{ %>
						value=""<%} %>
						name="<%=InfopubDeviceInfo.NM_SDTYPE %>"
						disabled="disabled">
				</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<input type="text" class="input-text"
					<%if(infopubDeviceInfo.getNmOnline().intValue()==0){ %>
						value="离线"<%}else{ %>
						value="在线"<%}%>disabled="disabled">
				</div>
			</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-4 col-sm-4">
					<input type="text" class="input-text"
						value="<%=infopubDeviceInfo.stOrganId2Html() %>"
						disabled="disabled">
				</div>
			</div>
			<!-- <div class="row cl">
			...
			<div class="mt-40">
			<span style="font-size: 20px;font-weight: bold;padding-left: 38%;">设备对应事项办件量排行TOP5</span>
			<a class="btn btn-primary radius" onclick="selm_query_list('办件查询','/business/selmQueryHis/selmQueryDeviceinfo.do','<%=infopubDeviceInfo.getStDeviceMac()%>')" href="javascript:;" style="margin-left: 88%; margin-bottom: 1%;">
			 查看详细办件信息</a> 
      ...
      "bLengthChange": false ,//改变每页显示数据数量
       "ajax": {
		    ...
		    var stAreaId = areaId;
		      ...
		      var stMac = '<%=infopubDeviceInfo.getStDeviceMac()%>'
		      d.stMac = stMac;
		    },
		    error:function(){
...
//添加
function selm_query_list(title,url,id){
