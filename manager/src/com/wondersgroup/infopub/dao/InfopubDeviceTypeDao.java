...
public class InfopubDeviceTypeDao {
	...
	public List<InfopubDeviceType> streetDeviceType(Conditions conds,
			String  suffix) {
		RecordSet rs;
        String content = " COUNT(IDI.ST_DEVICE_ID) ZHENGWU";
        ...
        while (rs.next()) {
        	 InfopubDeviceType info = new InfopubDeviceType();
        	info.setNmDtype(rs.getBigDecimal("ZHENGWU"));
            al.add(info);
        }
        return al;
	}

	public List<String> getByCompanyId(String stCompanyId) {
		String sql = "SELECT ST_TYPE_NAME from INFOPUB_DEVICE_TYPE where ST_COMPANY_ID = ?";
		Object[] obj = { stCompanyId};
		RecordSet rs;
		if (con == null) {
			rs = SQL.execute(sql, obj);
		} else {
        ...
        while (rs.next()) {
            al.add(rs.getOriginalString("ST_TYPE_NAME"));
        }
        return al;
	}

	public List<String> getTypeIdList(String string) {
		String sql = "SELECT ST_TYPE_ID from INFOPUB_DEVICE_TYPE where ST_TYPE_NAME LIKE ?";
		Object[] obj = { string};
		RecordSet rs;
		if (con == null) {
			rs = SQL.execute(sql, obj);
        ...
        ArrayList<String> al = new ArrayList<String>();
        while (rs.next()) {
            al.add(rs.getOriginalString("ST_TYPE_ID"));
        }
        return al;
