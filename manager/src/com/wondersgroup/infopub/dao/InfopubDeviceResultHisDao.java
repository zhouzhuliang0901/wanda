 ...
 */
@Repository
public class InfopubDeviceResultHisDao {

    private Connection con = null;

    public InfopubDeviceResultHisDao() {
    }

    public InfopubDeviceResultHisDao(Connection con) {
        this.con = con;
    }

    public void add(InfopubDeviceResult info) {
        String sql = "insert into INFOPUB_DEVICE_RESULT_HIS(ST_DEVICE_RESULT_ID, ST_DEVICE_ID, NM_MEM_USED, NM_CPU_USED,CL_HD_USED,CL_NET_USED,CL_SERVICE_USED,DT_CREATE, ST_EXT1, ST_EXT2) values (?, ?, ?,?, ?, ?, ?,?,?, ?)";
        Object[] obj = {
    ...
    public void update(InfopubDeviceResult info) {
        String sql = "update INFOPUB_DEVICE_RESULT_HIS set ST_DEVICE_ID = ?, NM_MEM_USED = ?, NM_CPU_USED = ?,CL_HD_USED=?,CL_NET_USED=?,CL_SERVICE_USED=?, DT_CREATE = ?, ST_EXT1 = ?, ST_EXT2 = ? where ST_DEVICE_RESULT_ID = ?";
        Object[] obj = {
            info.getStDeviceId(),
            info.getNmMemUsed(),
            info.getNmCpuUsed(),
    ...
    public int update(Map<String, Object> map, Conditions conds) {
        String sql = "update INFOPUB_DEVICE_RESULT_HIS set ";
    ...
    public int delete(Conditions conds) {
        String sql = "delete from INFOPUB_DEVICE_RESULT_HIS";
        String subsql = conds != null ? conds.toString() : "";
        if ("".equals(subsql)) {
            if (con == null) {
                return SQL.execute(sql).TOTAL_RECORD_COUNT;
    ...
    public PaginationArrayList<InfopubDeviceResult> query(Conditions conds, String suffix, int pageSize, int currentPage) {
        RecordSet rs;
        if (con == null) {
            rs = SQL.query("INFOPUB_DEVICE_RESULT_HIS", "*", conds, suffix, pageSize, currentPage);
        } else {
            rs = SQL.query(con, "INFOPUB_DEVICE_RESULT_HIS", "*", conds, suffix, pageSize, currentPage);
        }
    ...
    public List<InfopubDeviceResult> query(Conditions conds, String suffix) {
        RecordSet rs;
        if (con == null) {
            rs = SQL.query("INFOPUB_DEVICE_RESULT_HIS", "*", conds, suffix);
        } else {
            rs = SQL.query(con, "INFOPUB_DEVICE_RESULT_HIS", "*", conds, suffix);
        }
	...
	public InfopubDeviceResult get(String stDeviceResultId) {
        Conditions conds = Conditions.newAndConditions();
        conds.add(new Condition("ST_DEVICE_RESULT_ID", Condition.OT_EQUAL, stDeviceResultId));
