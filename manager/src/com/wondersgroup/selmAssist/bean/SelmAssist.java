...
public class SelmAssist implements Serializable {
     ...
     */
    @XmlJavaTypeAdapter(TimestampXmlAdapter.class)
    public Timestamp getDtUpadte() {
        return this.dtUpadte;
    }
    ...
    public String dtUpadte2Html(String pattern) {
        if (this.dtUpadte == null) {
            return "";
        } else {
            if (pattern == null) {
                pattern = "yyyy年MM月dd日 HH时mm分";
            }
            return new SimpleDateFormat(pattern).format(this.dtUpadte);
        }
    }
     ...
     * 修改时间
     */
    public void setDtUpadte(Timestamp dtUpadte) {
        this.dtUpadte = dtUpadte;
    }
