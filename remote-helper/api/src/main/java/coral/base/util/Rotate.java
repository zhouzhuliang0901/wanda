...
public class Rotate extends ImageFilter {
	...
	public Rotate(int angle) {
		...
		this.angle = angle;
		this.degree = Math.toRadians(angle);
		cos = Math.cos(this.degree);
		sin = Math.sin(this.degree);
	...
	/**
	 * Tell the consumer the new dimensions based on our rotation of coordinate
	 * space.
	 * 
	...
	/**
	 * Tell the consumer that we use the defaultRGBModel color model NOTE: This
	 * overrides whatever color model is used underneath us.
	 * 
	 * @param model
	 *            contains the color model of the image or filter beneath us
	 *            (preceding us)
	...
	/**
	 * Set the pixels in our image array from the passed array of bytes. Xlate
	 * the pixels into our default color model (RGB).
	 * 
	 ...
	 * Xlate the pixels into our default color model (RGB).
	 * 
	 * @see ImageConsumer#setPixels
	 */
	@Override
	...
	/**
	 * Notification that the image is complete and there will be no further
	 * setPixel calls.
	 * 
