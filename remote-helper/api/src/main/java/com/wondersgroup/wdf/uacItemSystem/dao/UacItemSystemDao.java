 ...
 */
@Repository
public class UacItemSystemDao {

    private Connection con = null;

    public UacItemSystemDao() {
    }

    public UacItemSystemDao(Connection con) {
        this.con = con;
    }

    public void add(UacItemSystem info) {
    ...
    public void update(UacItemSystem info) {
    ...
    public PaginationArrayList<UacItemSystem> query(Conditions conds, String suffix, int pageSize, int currentPage) {
        RecordSet rs;
        if (con == null) {
        ...
        PaginationArrayList<UacItemSystem> pal = new PaginationArrayList<UacItemSystem>(rs.TOTAL_RECORD_COUNT, rs.COMMON_PAGE_SIZE, rs.CURRENT_PAGE);
        while (rs.next()) {
    ...
    public List<UacItemSystem> query(Conditions conds, String suffix) {
        RecordSet rs;
        if (con == null) {
        ...
        ArrayList<UacItemSystem> al = new ArrayList<UacItemSystem>();
        while (rs.next()) {
            UacItemSystem info = new UacItemSystem();
            setProperties(info, rs);
            al.add(info);
    ...
    public static void setProperties(UacItemSystem info, RecordSet rs){
	...
	public UacItemSystem get(String stItemId, String stWebSystemId) {
        ...
        conds.add(new Condition("ST_ITEM_ID", Condition.OT_EQUAL, stItemId));
        conds.add(new Condition("ST_WEB_SYSTEM_ID", Condition.OT_EQUAL, stWebSystemId));
        List<UacItemSystem> list = query(conds, null);
