 ...
 */
@Repository
public class UacWebSystemDao {

    private Connection con = null;

    public UacWebSystemDao() {
    }

    public UacWebSystemDao(Connection con) {
        this.con = con;
    }

    public void add(UacWebSystem info) {
    ...
    public void update(UacWebSystem info) {
    ...
    public PaginationArrayList<UacWebSystem> query(Conditions conds, String suffix, int pageSize, int currentPage) {
        RecordSet rs;
        if (con == null) {
        ...
        PaginationArrayList<UacWebSystem> pal = new PaginationArrayList<UacWebSystem>(rs.TOTAL_RECORD_COUNT, rs.COMMON_PAGE_SIZE, rs.CURRENT_PAGE);
        while (rs.next()) {
            UacWebSystem info = new UacWebSystem();
    ...
    public List<UacWebSystem> query(Conditions conds, String suffix) {
        RecordSet rs;
        if (con == null) {
        ...
        ArrayList<UacWebSystem> al = new ArrayList<UacWebSystem>();
        while (rs.next()) {
            UacWebSystem info = new UacWebSystem();
            setProperties(info, rs);
            al.add(info);
    ...
    public static void setProperties(UacWebSystem info, RecordSet rs){
	...
	public UacWebSystem get(String stWebSystemId) {
        Conditions conds = Conditions.newAndConditions();
        conds.add(new Condition("ST_WEB_SYSTEM_ID", Condition.OT_EQUAL, stWebSystemId));
        List<UacWebSystem> list = query(conds, null);
