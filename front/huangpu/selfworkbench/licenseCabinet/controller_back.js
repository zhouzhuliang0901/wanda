...
app.controller("licenseTakeController", function(
	...
	$scope.loginRequest = function(code, isRe) {
		...
		$http
			.jsonp("http://zwdt.huangpuqu.sh.cn:8080/ac/aci/window/getInfoByCodeForLogin.do", {
	...
	$scope.openBox = function(No, id) {
		appConfig.http(
			"getStuff.do", {
				isOpen: "true",
				stSelfApplyId: id,
				cupAddress:encodeURI($.config.get("address")||"") //新增证照柜地点
			},
			function() {
				var routerName = $location.path();
				$location.path("/openBox").search({
	...
	$scope.Continue = function() {
		...
		appConfig.http(
			"putMatter.do", {
				...
				cupAddress:encodeURI($.config.get("address")||"")
			},
			function(res) {
				layer.close(index);
				if(res.result === "ok") {
...
app.controller("openBoxController", function(
	...
	$scope.getResideQuantity = function() {
		...
		if($scope.tipsInfo == "放入材料"){
			appData.resideQuantity = 4
		}else if($scope.tipsInfo == "取出证照"){
			appData.resideQuantity = 5
		}
//		$.ajax({
//			type: "get",
