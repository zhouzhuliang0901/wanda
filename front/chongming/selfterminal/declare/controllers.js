	...
	OcxControl.Barcode(function(res) {
		...
		$http.jsonp(urlHost + "/aci/window/getInfoByCodeTest.do", {
			...
			.success(function(dataJsonp) {
				if(dataJsonp.result.success==201) {
...
app.controller("captureController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.recognition = function() { //人证数据对比
		...
		// 人证对比接口请求
		$.ajax({
			url: "http://10.244.108.35:8080/ac-product-ext/ext/aci/autoterminal/facecompare.do",
			type: "post",
			dataType: "json",
			data: {
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		...
		var fConfig = {
			...
			'userId': '',
			'source': '网上申请',
			'targetTypeName': $scope.targetTypeName, // 个人或法人
...
app.controller("materialPicController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.profileShow = function() {
		$.ajax({
			...
			url: 'http://10.244.108.35:8080/aci/autoterminal/forward.do?fmd=aci-archives&fdo=getLicenseStuffList',
...
app.controller("takePhotoController", function($scope, $route, $http, $location, data, $timeout, $routeParams) {
	...
	if($routeParams.flag == 'U') { // U盘上传
		...
		$scope.waitUploadImgUrl = data.profile[data.page].imageUrl;
		$scope.imgName = data.profile[data.page].stFilename;
		//	'http://hengshui.5uban.com/ac' + 
		$('#showImg').attr('src', 'http://10.244.108.35:8080' + $scope.waitUploadImgUrl);
	}
	...
	$scope.highCapture = function() { // 高拍仪拍照
		if($routeParams.flag == 'U') { // U盘上传
			...
			$.device.httpDownload( // 调用壳的方法下载服务器上的图片，再上传到另一接口中		 urlHost
				'http://10.244.108.35:8080' + $scope.waitUploadImgUrl,
				"C:\\waitUploadImg.jpg",
				//将选中图片下载
	...
	$scope.print = function() {
		...
		if($scope.statusText == '返回首页' && f == 2) {
			...
			OcxControl.receiptPrint(
				'自助申报申请回执\n' +
				'@qrcode@' + 'http://zwdt.sh.gov.cn/zwdtSW/sQ/aQ.do?id=' + $scope.applyNo + '&ob=' + data.idCardName + '@qrcode@\n' +
				'\n事项名称：' + $scope.allName +
				'\n事件编码：' + $scope.applyNo +
				'\n您可至 “一网通办” 总门户 “用户中心” 查询办理进度，或通过扫描上方二维码进行查询\n请您携带办理材料至 上海市崇明区翠竹路1501号 进行办理\n',
				function(success) {
