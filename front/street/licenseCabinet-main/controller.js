app.controller("mainController", function($rootScope, $scope, $http, appData, $location, appConfig, $state, $rootScope) {
	...
	$scope.materialShow = $.config.get('materialShow')=='Y'?true:false;
...
app.controller("putMaterialConfirmController", function($scope, $http, appData, $location, appConfig, $timeout) {
	...
	$scope.takeAndStore = function(stCabinetNo) {
		if(appData.isStaff == true) {
			...
			var result = {};
			for(var i = 0; i < appConfig.personnel.length; i++) {
				if(appConfig.personnel[i].Mac == $.config.get('uniqueId')) {
					result = appConfig.personnel[i];
				}
			}
			appConfig.http("store.do", {
					...
					stSenderName: encodeURIComponent(result.Name||""),
					stSenderId: result.Code||"",
					stSenderPhone: result.Tel||"",
			...
			$.ajax({
				...
				data: {
					...
					stCabinetNo: stCabinetNo,
					nmCertType: '1',
					stReceiverName: result.Name || '',
					stReceiverIdcard: result.Code || '',
					stReceiverPhone: result.Tel || '',
 ...
 * 保存设备信息
 */
app.controller("saveDeviceInfoController", function($scope, $http, appData, appConfig, $timeout, $location, $interval, $rootScope) {
	$scope.stCabinetNum = "";
	$scope.addCabinetInfo = function(){
		console.log($scope.nmCabinet);
		if($scope.nmCabinet=="" || $scope.nmCabinet == undefined ||$scope.nmCabinet == null){
			alert('柜子数不能为空')
			return;
		}
		if($scope.stContacts=="" || $scope.stContacts == undefined ||$scope.stContacts == null){
			alert('工作人员姓名不能为空')
			return;
		}
		if($scope.stPhone=="" || $scope.stPhone == undefined ||$scope.stPhone == null){
			alert('工作人员电话不能为空')
			return;
		}
		for(var i=1; i<=$scope.nmCabinet; i++){
			if(i>=10){
				if(i==$scope.nmCabinet){
					$scope.stCabinetNum = $scope.stCabinetNum + i;
				}else{
					$scope.stCabinetNum = $scope.stCabinetNum + i +',';
				}
			}else{
				$scope.stCabinetNum = $scope.stCabinetNum + '0'+i +',';
			}

		}
		console.log($scope.stCabinetNum)
		appConfig.http('addCabinetInfo.do',{
			stCabinetMac:appConfig.stMachineId,
			stCabinetNum:$scope.stCabinetNum,
			stCabinetNo:$scope.stCabinetNo||"",
			nmCabinet:$scope.nmCabinet,
			stCabinetStreet:encodeURI($.config.get("stCabinetStreet") || "863政务服务"),
			stStreetAddress:encodeURI($.config.get("stStreetAddress") || "联航路1518号"),
			stContacts:encodeURI($scope.stContacts),
