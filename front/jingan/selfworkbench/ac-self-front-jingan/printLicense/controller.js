...
app.controller("licenseMain", function($scope, $state, appData) {
	...
	$scope.current = 0;
//	$scope.lxmItem = perjsonStr.slice(0,1);
	$scope.stuffName = perjsonStr;
//	$scope.rsItem = perjsonStr.slice(4,7);
...
//				$scope.rsItem="";
	...
	$scope.Handle = function(url, banli,isSelf,stuffName) {
		trackEvent('证明开具-'+stuffName);
		appData.url = url;
		appData.type = banli;
		appData.person = $scope.person;
		if(isSelf == "1"){
	...
	$scope.choiceType = function(url,stuffName,code){
		if(code=="1111111111"){
			trackEvent('证明开具-'+stuffName);
			window.location.href = url;
		}
