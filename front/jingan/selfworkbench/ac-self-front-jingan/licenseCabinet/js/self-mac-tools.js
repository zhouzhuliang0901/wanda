...
jQuery.device = {
	...
	SarkInit: function() {
		try {
			window.external.SerialPort_Open("COM4", 115200, 8);
		} catch(e) {
	...
	SarkOpen: function(CabinetNum) {
		...
		var SarkNum = [
			//系统自带柜号
			"SET,D01,006,01,N,00,86",
			"SET,D02,006,01,N,00,87",
			"SET,D03,006,01,N,00,88",
			"SET,D04,006,01,N,00,89",
			"SET,D05,006,01,N,00,90",
			//"fff25505",
			//			"fff25506",
			//			"fff25507",
			//			"fff25508",
		...
		for(var i = 0; i < Custom.length; i++) {
			if(Custom[i] === CabinetNum) {
				hexStr = SarkNum[i];
				//发送指令
				window.external.SerialPort_WriteString(hexStr);
			}
		}
	},
	GoHome: function() {
