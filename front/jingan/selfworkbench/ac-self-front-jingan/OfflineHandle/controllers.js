...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		...
		var fConfig = {
			...
			stWindowNo: encodeURI($.config.get("uniqueId") || "C26"),
			loginName: "GZT001",
			getCertWay: $scope.getCertWay,
			stUserName: encodeURI(data.idCardName),
			stUnit: encodeURI(data.targetName || ""),
...
app.controller("infoFinishController", function($scope, $route, $http, $location, data, $timeout) {
	...
	//		} else {
	//			/**
	//			 * 说明: 以前使用lodop打印, 现在因为需要添加电子章, 就不能用lodop打印了
	//			 * 日期:2020.09.28
	//			 **/
	...
	$scope.submitApply = function() {
		...
		$http.jsonp(JA_Extranet_urlHost + '/aci/materialUp/toSubmit.do', {
			...
			data.applyNo = dataJson.applyNo;
			$scope.applyNo = data.applyNo;        
			recordUsingHistory('线下办理',  '办理',  $scope.allName,  data.idCardName,  data.idCardNum,  data.mobile,  $scope.applyNo,  data.departmentOrganName, data.itemNo);
			$scope.print();
		})
	}
	$scope.submitApply();
	...
	$scope.print = function() {
		...
		$scope.pdfPrintUrl = "http://xzfwzx.jingan.gov.cn:8080/ac/ext/ja/offLine/offLineCredential.do?stApplyId=" + data.applyId;
		$.device.httpDownload( // 调用壳的方法下载服务器上的图片，再上传到另一接口中		urlHost +
			$scope.pdfPrintUrl,
			$scope.path,
			//将选中图片下载
