app.directive("idcard", function() {
	return {
		...
		controller: function($scope, $location, $rootScope, $interval, $timeout) {
			...
			$scope.showImage = "../libs/common/images/wonders/idCard.gif";
...
//						},2000)
			...
			$scope.getImgbaseStr = function(idCardNo, name, imgUrl) {
				$.ajax({
					type: "post",
					url: "http://xzfwzx.jingan.gov.cn:8080/ac/aci/autoterminal/archives/getLicenseStuffList.do",
					data: {
						stIdNo: idCardNo,
						type: 0,
			...
			$scope.uploadIdCardImg = function(idCardNo, name, imgUrl) {
				var jsonData = {
					...
					//"http://hengshui.5uban.com/xhac/aci/autoterminal/archives/saveLicenseStuff.do"
				$.device.httpUpload(
					"http://xzfwzx.jingan.gov.cn:8080/ac/aci/autoterminal/archives/saveLicenseStuff.do",
					"FileData",
					imgUrl,
