...
jQuery.device = {
	...
	qrCodeOpen: function(callback) {
		try {
			var this_ = this
			this_.audioPPlay("Realtek High Definition Audio", window.external.GetCurrentPath() +
	...
	httpUploadSafe: function(url, fileKey, filePath, keyValueJsonStr, successCallback, errorCallback) {
		try {
			window.external.Http_Upload_Safe(url, fileKey, filePath, keyValueJsonStr,
				"httpUploadSuccessCallback", "httpUploadErrorCallback");
			window.httpUploadSuccessCallback = function(result) {
				successCallback(result);
			};
