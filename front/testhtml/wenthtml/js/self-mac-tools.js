...
function saveDeviceStatus(stOutDeviceCode, nmException, stCause, nmNotice, nmTotal, nmRemain, count) {
	$.ajax({
		type: "get",
		url: "http://10.2.104.246:8080/ac-self-manager/outdevicestatus/odeviceStatus/save.do",
		dataType: "jsonp",
		jsonp: "jsonpCallback",
		data: {
...
jQuery.device = {
	...
	qrCodeOpen: function(callback) {
		try {
			...
			saveDeviceStatus("二维码", 0, "正常", 0, 0, 0, 0);
		} catch(e) {
			saveDeviceStatus("二维码", 1, "未正常调用", 0, 0, 0, 0);
		}
	},
	...
	idCardOpen: function(callback) {
		try {
			...
			saveDeviceStatus("身份证", 1, "未正常调用", 0, 0, 0, 0);
		}
	},
	...
	cmCaptureShow: function(width, height, x, y) {
		try {
			...
			saveDeviceStatus("高拍仪", 0, "正常", 0, 0, 0, 0);
		} catch(e) {
			saveDeviceStatus("高拍仪", 1, "未正常打开", 0, 0, 0, 0);
		}
	},
