	...
	$scope.searchInsuredInfo = function() {
		$http.jsonp($.getConfigMsg.preUrl + "/aci/workPlatform/medicalInsurance/queryAccountInfo.do", {
			params: {
				...
				identNo: '430426199804106174' // appData.licenseNumber // "310104194906012828"
			}
...
app.controller('bookMakingChoose', function($scope, $http, $state, appData, $sce) {
	...
	$scope.searchInsuredInfo = function() {
		$http.jsonp($.getConfigMsg.preUrl + "/aci/workPlatform/medicalInsurance/queryAccountInfo.do", {
			params: {
				jsonpCallback: "JSON_CALLBACK",
				identNo: '430426199804106174'//'310227195507240051' // appData.licenseNumber // "310104194906012828"
			}
		}).success(function(returnData) {
			if(returnData[0].cbrxxs) {
				appData.zhh = returnData[0].cbrxxs[0].cbrxx[0].zhh;
	...
	$scope.updateRecordBook = function() {
		$http.jsonp($.getConfigMsg.preUrl + "/aci/workPlatform/medicalInsurance/updateRecordBook.do", {
			...
			if(!returnData[0].syscode) {
				...
				console.log(appData.newBookBh);
				console.log(appData.newBookKh);
				console.log(appData.userXb);
				console.log(appData.userName);
				console.log(appData.userAge);
...
//		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		...
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(145, 207, 427, 20, "网点：临汾街道社区事务受理中心");
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
...
//账户清算信息查询
app.controller('accountSettlement', function($scope, $state, appData, $sce, appFactory) {
	$scope.funName = appData.funName;
	var date = new Date();
	$scope.year = date.getFullYear();
	$scope.isLoding = false;
