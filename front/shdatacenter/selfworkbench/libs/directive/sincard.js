app.directive("sincard", function() {
	return {
		restrict: "E",
...
//						},2000)
			...
			$scope.getImgbaseStr = function(idCardNo, name, imgUrl) {
				$.ajax({
					type: "post",
					url: "http://172.16.125.53:8080/ac-product/aci/autoterminal/archives/getLicenseStuffList.do",
					data: {
						stIdNo: idCardNo,
						type: 0,
			...
			$scope.uploadIdCardImg = function(idCardNo, name, imgUrl) {
				...
				$.device.httpUpload(
					"http://172.16.125.53:8080/ac-product/aci/autoterminal/archives/saveLicenseStuff.do",
					"FileData",
					imgUrl,
