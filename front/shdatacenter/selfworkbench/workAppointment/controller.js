app.controller("appointmentQu", function($scope, $state, appFactory, appData, $timeout, $http){
	$scope.quList = [
		{
			quName:"黄浦区"
		},
	...
	$scope.operation = "请选择区:";
	$scope.itemProperty = "quName";
	$scope.check = function(item) {
		appFactory.qumethod(item.quName);
		$state.go("main");
...
app.controller("appointmentAuthentication", function($scope, $state, appData, appFactory) {
	...
	$scope.choiceType = function(type){
		if(type=='idcard'){
			...
			$scope.operation = "第二步：请将随申办中身份证二维码放置扫码区";
		}
