...
app.controller('login', function($scope, $http, $state, appData, appFactory, $timeout) {
	...
	//根据是否有一网通办账号
	$scope.isRegistration = function(idCard, name) {
		$scope.isLoding = false;
		$scope.loginType = "";
		$.ajax({
			type: "get",
			url: "http://hengshui.5uban.com/xhac/aci/workPlatform/isRegistration.do",
			/*url写异域的请求地址*/
			dataType: "jsonp",
			/*加上datatype*/
			jsonp: "jsonpCallback",
	...
	$scope.getResult = function(img){
		$scope.img = img;
		appData.idCardImg = $scope.img;
		$scope.isRegistration(appData.Number, appData.Name);
	} 
	$scope.prevStep = function() {
		$state.go("loginType");
	}
...
app.controller("recognition", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$.device.Camera_Show();
	$scope.recognitionOver = false;
	$scope.compare2PhotoOld = function() {
		$scope.isLoding = false;
		$.ajax({
			type: "post",
			url: "http://hengshui.5uban.com/xhac/aci/workPlatform/CA/compare2PhotoOld.do",
			dataType: "json",
			data: {
				userName: appData.Name,
				identNo: appData.Number,
				idCardPhoto: appData.idCardImg,
				capturePhoto: appData.faceImg
			},
			success: function(data) {
			...
			error: function(err) {
				$scope.recognitionOver = true;
				$scope.isAlert = true;
				$scope.msg = "认证接口有误，请重试！";
			}
		});
	}
	$scope.capture = function() {
		appData.faceImg = $.device.Camera_Base64();
		$scope.showImage ="data:image/png;base64,"+appData.faceImg;
		$scope.recognitionOver = true;
		$.device.Camera_Hide();
	}
...
app.controller("inputPhone", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		...
		$.ajax({
			type: "post",
			url: "http://hengshui.5uban.com/xhac/aci/workPlatform/CA/dataSynchronizationToCivilAdministration.do",
			dataType: "json",
			data: {
				userName: appData.Name,
				sex: ($scope.sex == "男") ? "1" : "2",
...
app.controller("info", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.stBirth = (appData.Number).substring(6, 10) + "-" + (appData.Number).substring(10, 12) + "-" + (appData.Number).substring(12, 14);
	...
	$scope.nextStep = function() {
		...
		$scope.sumbit = function() {
			$.ajax({
				type: "get",
				url: "http://hengshui.5uban.com/xhac/aci/workPlatform/userRegister.do",
				dataType: "jsonp",
				jsonp: "jsonpCallback",
				data: {
					userName: appData.Name,
					...
					password: appData.submitInfo.stPassword,
					type: '身份证'
				},
				success: function(data) {
					console.log(data);
