...
app.controller("infoJybtController", function($scope, $route, $http, $location, data,$rootScope, $timeout, appFactory) {
	...
	$scope.info = function() {
		...
		$http.jsonp("http://hengshui.5uban.com/xhac/aci/workPlatform/elderlyCard/getApplicantInfo.do?", {
			params: config
		}).success(function(dataJsonp) {
			setCenter();
			if(data.nation == "") {
					setSelectChecked("nations", dataJsonp.rtnData.nationality);
				} else {
					setSelectCheckedByText("nations", data.nation);
				}
			if(dataJsonp.rtnData) {
				data.rtnData = dataJsonp.rtnData;
				$scope.stMomile = dataJsonp.rtnData.cellno || data.mobile;
			}
			$scope.stRecipientMomile = $scope.stMomile;
			$scope.stRecipient = $scope.stName;
		}).error(function(err) {
			setCenter();
		...
		$scope.next1 = function(){
			...
			var manLicense = $.ajax({
					...
					success:function(dataJsonp){
						...
						$scope.stAddress = $scope.hjs;
						$scope.liveStAddress = $scope.hjs;
				},error:function(err){
	...
	$scope.next = function() {
		...
		$scope.nextload1 = function(){
			appFactory.pro_fetch(data.idCardNum, '310105109000100','YJS00010003','YJS0001','居民身份证','1', function(dataJson) {
				if(!dataJson) {
					$scope.SisAlert = true;
					$scope.Smsg = "没有该证照!";
				}
