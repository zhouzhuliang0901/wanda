app.controller("waiter", function($scope, $state, appData, $sce) {
	$scope.ywtbShow = function() {
		window.external.URL_OPEN(50, 100, 1800, 850, "http://zwdt.sh.gov.cn/govPortals/index.do");
		$scope.address = $sce.trustAsResourceUrl("http://zwdt.sh.gov.cn/govPortals/index.do");
	}
	$scope.tzsbShow = function() {
		window.external.URL_OPEN(50, 100, 1800, 850, "http://tzjc.scjgj.sh.gov.cn/secms/loginout.jsp");
...
app.controller('apply', function($state, $scope, appData, $http) {
	...
	$scope.encryptDataByRSA = function(){
		$http.jsonp('http://hengshui.5uban.com/xhac/aci/workPlatform/util/encryptDataByRSA.do',{
			...
			window.external.URL_OPEN(50, 150, 1800, 800, $scope.url + encodeURIComponent(appData.url));
		}).error(function(err){
