...
jQuery.device = {
	...
	qrCodeOpen: function(callback) {
		try {
			...
			window.qrScannerCallBack = function(value) {
				callback(value);
				$.log.debug('qrScannerCallBack执行'+value)
			};
			window.GetScannerCode = function(value) {
				callback(value);
				$.log.debug('GetScannerCode执行'+value)
			};
		} catch(e) {}
	},
	...
	fileBase64ToFile: function(base64Str,filePath){
		try{
			window.external.File_Base64ToFile(base64Str,filePath)
		}catch(e){}
	},
	// Office操作
	...
	officeSaveAs:function(filepath){
		try{
			window.external.Office_SaveAs(filepath)
		} catch(e) {
			alert(e)
		}
	},
	...
	fileCopy:function(src,dest){
		try{
			window.external.File_Copy(src,dest)
		}catch(e){
			alert('err:'+e)
		}
	},
