...
app.controller("listController", function($scope, $route, $location, $http, $rootScope, data) {
	...
	$scope.getItemOne = function() {
		...
		$scope.itemName = [{
			...
			},
			{
				...
				clNameCode: "<p>事项名称：特种设备使用登记</p><p>事项编码：310150679000</p>",
			...
			},
			{
				...
				clRange: "本指南适用于本市网络预约出租汽车驾驶员人员背景审查的办理。",
			...
			},
			{
				...
				clNameCode: "<p>事项名称：第一类医疗器械备案</p><p>事项编码：310150713000</p>",
				clDealAccording: "",
				clDealOrgan: "嘉定区行政服务中心",
				clApprovalConds: "申请材料齐全，符合法定形式，具体见申请材料目录。",
				clApprovalCount: "无数量限制",
			...
			},
			{
				...
				clNameCode: "<p>事项名称：技术合同认定登记</p><p>事项编码：310150054000</p>",
				clDealAccording: "",
				clDealOrgan: "嘉定区行政服务中心",
				clApprovalConds: "申请材料齐全，符合法定形式，具体见申请材料目录。",
				clApprovalCount: "无数量限制",
			...
			},
			{
				...
				clRange: "本指南适用于本市技术合同认定登记的审核办理。",
				clNameCode: "<p>事项名称：技术合同认定登记</p><p>事项编码：310150528002</p>",
				clDealAccording: "",
				clDealOrgan: "嘉定区行政服务中心",
				clApprovalConds: "申请材料齐全，符合法定形式，具体见申请材料目录。",
				clApprovalCount: "无数量限制",
	...
	$scope.getItemThree = function() {
		...
		{
			stItemName: "设立健身气功站点审批",
			organCode: "SHJTJD",
			itemNo: "310150487002",
//			itemTenNo: "310150487002",设立健身气功站点审批事项用不到该参数
			organName: "",
			ogranId: 28,
			organNodeId: 28,
			stItemId: "14177335-a2c7-497d-88db-0a5570fe91ca"
		}]
	}
...
app.controller("idCardController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.getIdcard = function(info, images) {
		...
		$.log.debug(data.startDay + '---------------------' + data.endDay)
	}
			...
			$.ajax({
				...
				success: function(dataJsonp) {
					if(dataJsonp.result.success == false) {
						...
						layer.msg('网络遇到了问题，请重试！');
						$timeout(function() {
							$location.path('/select');
						}, 500);
					}
					if(!dataJsonp.result.data.realname || !dataJsonp.result.data.idcard) {
						layer.msg('没有返回的姓名和身份证号')
					}
					if(!dataJsonp.result.data.VALIDSTARTDAY || !dataJsonp.result.data.VALIDENDDAY) {
						alert('没有获取到开始-结束时间')
					}
						if(dataJsonp.encrypt_identity) {
					data.encrypt_identity = ClearBr(dataJsonp.encrypt_identity);
				}
					...
					$.log.debug('返回信息：' + JSON.stringify(dataJsonp.result.data))
					$.log.debug('市民云信息：' + JSON.stringify(dataJsonp.result.data))
					$.log.debug('身份证：' + dataJsonp.result.data.idcard)
					$.log.debug('姓名' + dataJsonp.result.data.realname)
					$.log.debug('状态信息' + dataJonsp.result.success)
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout, appFactory) {
	...
	if(data.itemId == "c8b8e1f5-55bc-415a-b0e9-b39f27f473a3" && data.itemTenNo == "3120W0358000") {
		...
		/*if(data.idCardNum.slice(0,2)!= 31){
		...
		$scope.refresh = function() {
			...
			$http.jsonp(urlHost + '/aci/workPlatform/DZCert/queryMarriageCertificateInfo.do', {
				...
				if(!SCLASS || !SARCHIVESNO || !SFIRSTDATE) {
					layer.msg('未查询到驾驶证信息！');
				}
			...
			$http.jsonp(urlHost + '/aci/workPlatform/DZCert/queryMarriageCertificateInfo.do', {
				params: {
					...
					businessCode: data.applyId || '',
					identNo: data.encrypt_identity||data.idCardNum,
					catMainCode: '310105105000100', //证照目录编码的前半段
					jsonpCallback: "JSON_CALLBACK",
					//新增参数
					name: data.idCardName,
					startDay: data.startDay,
		...
		$http.jsonp(urlHost + '/aci/workPlatform/DZCert/queryMarriageCertificateInfo.do', {
			...
			$.log.debug('户籍地址:', JSON.stringify(dataJson))
			console.log('户籍地址:', dataJson['SADDR'])
			var SADDR = dataJson.sAddr || dataJson.SADDR
			if(!SADDR) {
				layer.msg('未查询到户籍地址！');
			}
		...
		$timeout(function() {
			if(data.idCardSex) {
				...
				if(data.idCardNum.slice(16, 17) % 2 == 0) {
					$('#sex').val('女');
				} else {
					$('#sex').val('男');
				}
		...
		$scope.goNext = function() {
			...
			$scope.saveSignature = function() {
				...
				data.signStr = $scope.signature.split(",")[1];
				//将base64图片保存为本地
				//data.signFilePath = $.device.currentPath() + "\\temp\\file\\sign.png"
				//$.device.fileBase64ToFile(data.signStr,data.signFilePath)
				//alert('图片保存成功'+data.signFilePath)
				$scope.isShowSign = false;
				$scope.signFinish()
			};
		}
		...
		$scope.signFinish = function() {
			...
			$http.jsonp(urlHost + '/aci/materialUp/saveNew.do?' + from, {
				...
				$scope.printWord = function() {
					$.ajax({
						url: "http://10.237.16.72/aci/autoterminal/getlicensePDF.do",
						//url: "http://180.169.7.194:8080/ac-product/aci/autoterminal/getlicensePDF.do",
						type: "post",
						dataType: 'json',
						data: {
							name: data.idCardName,
							...
							hjAddress: data.address,
							czAddress: data.liveAddress,
							carType: data.carType,
							licenseNo: data.driveNo,
							mobile: data.phone,
							date: data.driveDate,
							applyNo: data.No,
							signPng: data.signStr
						},
						success: function(dataJsonp) {
							console.log(dataJsonp)
							$scope.pdfBase64 = dataJsonp.data;
							data.pdfBase64 = $scope.pdfBase64;
							$scope.uploadOffice();
						},
						error: function(err) {
							layer.msg('获取pdf失败')
				...
				$scope.uploadOffice = function() {
					$scope.jsonData = {
						...
						stuffId: data.autoUpload[0].stuffId,
						reset: data.resetData,
						fileName: data.autoUpload[0].name + '.pdf',
						type: "1",
						itemId: data.itemId,
						stuffName: data.autoUpload[0].name,
					};
					...
					$.ajax({
						...
						data: {
							...
							stuffId: data.autoUpload[0].stuffId,
							reset: data.resetData,
							fileInput: 'data:image/png;base64,' + data.pdfBase64,
							fileName: data.autoUpload[0].name + '.pdf',
							type: "1",
							itemId: data.itemId,
							stuffName: data.autoUpload[0].name,
						...
						success: function(dataJsonp1) {
							...
							data.isUploadForPush.push({
								index: 0,
								stuffName: '上海市网络预约出租汽车驾驶员人员背景审查申请表',
								fileName: '上海市网络预约出租汽车驾驶员人员背景审查申请表',
								method: "U盘上传"
							});
							data.listImg[0] = {
								'activeImg': null,
				...
				$scope.idCardNumber = data.encrypt_identity||data.idCardNum;
				$scope.loadAjax3 = function() {
					$scope.isLoading = false;
					appFactory.pro_fetch($scope.idCardNumber, '310100208000100', data.applyId, 'jd02825', '申请人驾驶证.png', '申请人驾驶证', data.idCardName, data.startDay, data.endDay, function(dataJson, imgbase64) {
						$.log.debug('loadAjax3方法接收到的参数' + data.idCardName + data.startDay + data.endDay)
						try {
							//alert('进入callback')
							if(!dataJson || dataJson == '' || dataJson == [] || imgbase64['str'] == '' || !imgbase64['str']) {
								alert("没有驾驶证证照!请手动上传")
								data.listImg[3] = {
									'activeImg': null,
									'index': 3,
								...
								data.autoUpload[3].imgUrl = 'http://10.237.16.72' + dataJson[0].pictureUrlForBytes
								$scope.imgUrl = dataJson[0].pictureUrl;
								//----给listImg赋值
								data.listImg[3] = {
									'activeImg': null,
									'index': 3,
						...
						data.imgId3 = dataJson.rtnData.imgid;
					})
				}
				$scope.loadAjax2 = function() {
					$scope.isLoading = false;
					appFactory.pro_fetch($scope.idCardNumber, '310105105000100', data.applyId, 'jd02824', '申请人户口簿.png', '申请人户口簿', data.idCardName, data.startDay, data.endDay, function(dataJson, imgbase64) {
						...
						try {
							...
							if(!dataJson || dataJson == '' || dataJson == [] || imgbase64['str'] == '' || !imgbase64['str']) {
								...
								data.autoUpload[2].imgUrl = 'http://10.237.16.72' + dataJson[0].pictureUrlForBytes
								$scope.imgUrl = dataJson[0].pictureUrl;
								//----给listImg赋值
								data.listImg[2] = {
									'activeImg': null,
				...
				$scope.loadAjax1 = function() {
					...
					//					  data, code,applyId,stuffId,fileName,stuffName,callback, complete,callback1,error
					appFactory.pro_fetch($scope.idCardNumber, '310105109000100', data.applyId, 'jd02823', '申请人身份证.png', '申请人身份证', data.idCardName, data.startDay, data.endDay, function(dataJson, imgbase64) {
						...
						try {
							...
							if(!dataJson || dataJson == '' || dataJson == [] || imgbase64['str'] == '' || !imgbase64['str']) {
								...
								data.autoUpload[1].imgUrl = 'http://10.237.16.72' + dataJson[0].pictureUrlForBytes
								$scope.imgUrl = dataJson[0].pictureUrl;
								//----给listImg赋值
								data.listImg[1] = {
									'activeImg': null,
		...
		$scope.goNext = function() {
			...
			$http.jsonp(urlHost + '/aci/materialUp/saveNew.do?' + from, {
				...
				// 判断是否为设立健身气功站点审批事项
				if(data.itemId == "14177335-a2c7-497d-88db-0a5570fe91ca") {
					// 身份证上传
					$scope.autoUploadIdCardInfo = function() {
						$scope.isLoading = false;
						//data, code,applyId,stuffId,itemId,stuffName ,callback, callback1
						//data, code,applyId,stuffId,fileName,stuffName,callback, complete,callback1,error
						$scope.idCardNumber = data.encrypt_identity || data.idCardNum;
								...
								if(!dataJson || dataJson == '' || dataJson == [] || imgbase64['str'] == '' || !imgbase64['str']) {
									...
									if(data.isUpload.length <= 1) {
										data.isUpload.push({
											index: 1,
											stuffName: '申请人身份证',
											img: 'http://10.237.16.72' + dataJson[0].pictureUrlForBytes,
											method: "个人档案"
										});
										data.isUploadForPush.push({
											index: 1,
...
app.controller("materialListController", function($scope, $sce, $route, $http, $location, data, $timeout) {
	...
	$.log.debug('data.isUpload----' + JSON.stringify(data.isUpload))
	$.log.debug('data.listImg----' + JSON.stringify(data.listImg))
	//设置上传文件 按钮变化
	if(data.isUpload != "") {
		...
		$http.jsonp(urlHost + '/aci/materialUp/toSubmit.do', {
			...
			$scope.print = function() {
				...
				if($scope.statusText == '返回首页' && f == 2) {
					...
					lodop.SET_PRINT_STYLEA(0, "FontSize", 14);
					lodop.ADD_PRINT_TEXT(370, 28, 600, 30, "请您在星期一至星期五工作时间内（法定节假日除外）携带您的申报提交确认单至嘉定区行政服务中心综合服务区发证窗口（A01 A02）领取证照结果。");
					lodop.SET_PRINT_STYLEA(0, "FontSize", 14);
					lodop.ADD_PRINT_TEXT(500, 551, 168, 30, date.getFullYear() + "年" + month + "月" + date.getDate() + "日");
					lodop.SET_PRINT_STYLEA(0, "FontSize", 14);
