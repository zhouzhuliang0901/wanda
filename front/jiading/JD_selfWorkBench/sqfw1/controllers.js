...
app.controller("idCardController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.idCardName = "沈中楠";
	data.idCardNum = "310222196911170035";
	data.idCardSex = "男";
	data.address = "上海市嘉定区南翔镇德园路1437弄14号1104室";
	data.startDay = "2005-05-20";
	data.endDay = "2025-05-20";
	$location.path("/info");
			...
			$http.jsonp(urlHost + '/aci/materialUp/saveNew.do?' + from, {
				...
				$scope.printWord = function() {
					...
					$timeout(function() {
						if(data.companyName && data.companyNo) {
							...
							$.device.officeSetJpgValue("sign", data.signFilePath);
				...
				$scope.uploadOffice = function() {
					$scope.jsonData = {
						...
						fileName: data.autoUpload[0].name + '.doc',
						type: "2",
						itemId: data.itemId,
				...
				$scope.loadAjax3 = function() {
					$scope.isLoading = false;
					appFactory.pro_fetch(data.idCardNum,'310100208000100',data.applyId,'jd02825','申请人驾驶证.png','申请人驾驶证',data.idCardName,data.startDay,data.endDay,function(dataJson,imgbase64){
						$.log.debug('方法接收到的参数'+data.idCardName+data.startDay+data.endDay)
						try{
							//alert('进入callback')
				...
				$scope.loadAjax2 = function() {
					$scope.isLoading = false;
					appFactory.pro_fetch(data.idCardNum,'310105105000100',data.applyId,'jd02824','申请人户口簿.png','申请人户口簿',data.idCardName,data.startDay,data.endDay,function(dataJson,imgbase64){
...
//					  data, code,applyId,stuffId,fileName,stuffName,callback, complete,callback1,error
					appFactory.pro_fetch(data.idCardNum,'310105109000100',data.applyId,'jd02823','申请人身份证.png','申请人身份证',data.idCardName,data.startDay,data.endDay,function(dataJson,imgbase64){
