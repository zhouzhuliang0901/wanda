...
app.controller("main", function($scope, $state, appData, $sce) {
	...
	$scope.choiceType = function(type, name, ywlx) {
		...
		appData.type = type;
		appData.ywlx = ywlx;
		if($.device.vendor() == "wonders"||$.device.vendor() == "zhuofansoft"||$.device.vendor() == "epoint") {
			$state.go("loginType");
		} else {
...
app.controller("updateResidence", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	if($.device.vendor() == "wonders"||$.device.vendor() == "zhuofansoft"||$.device.vendor() == "epoint") {
		...
		$scope.sex = ((parseInt($scope.stIdCard.substring(16, 17)) % 2) == 0) ? "女" : "男";
		$scope.wandaQuery = function() {
			$.log.debug(i);
			$.device.dataCardQuery(appData.licenseNumber, appData.licenseName, function(res) {
				$scope.isLoading = false;
				$.log.debug("res" + res);
				res = JSON.parse(res);
				$.log.debug("resjson" + res);
	...
	$scope.nextStep = function() {
		if($.device.vendor() == "wonders"||$.device.vendor() == "zhuofansoft"||$.device.vendor() == "epoint") {
			...
			$timeout(function() {
				...
				recordUsingHistory('公安服务', '办理', $scope.funName, appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
				//行为分析(办理)
				trackEventForAffairs('', $scope.funName, "上海市公安局", appData.licenseName, appData.licenseNumber, '');
			}, 1000);
		} else {
			//卓凡
