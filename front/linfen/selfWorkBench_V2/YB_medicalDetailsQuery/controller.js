...
app.controller('main', function($state, $scope, appData, $location) {
	...
	$scope.funName = newPerjsonStr.medical_detail_query.stuffName;
	appData.type = newPerjsonStr.medical_detail_query.type;
	appData.ywlx = newPerjsonStr.medical_detail_query.ywlx;
...
app.controller('medicalDetails', function($scope, $state, appData, $sce, appFactory) {
	...
	recordUsingHistory('医保服务', '查询', '医保就医明细查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
//行为分析(查询)
	trackEventForQuery("医保就医明细查询", "", "查询", "上海市医疗保障局", $scope.licenseName, appData.licenseNumber, "");
	$scope.isScroll = function() {
