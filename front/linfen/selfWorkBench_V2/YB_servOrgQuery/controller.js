app.controller("main", function($scope, $state, appData, $sce) {
	...
	$scope.choiceType = function(type, name, ywlx) {
		...
		if(appData.type == "bookMaking") {
			...
			$state.go("yljgChoose");
		}else if(appData.type == "password"){
			window.location.href = "../YB_passwordUpdate/index.html";
		}else if(appData.type == "payStand"){
			window.location.href = "../YB_payStandard/index.html";
		}else if(appData.type == "calculator"){
			window.location.href = "../YB_medicalCostCalculator/index.html";
		}else {
			$state.go("loginType");
		}
	}
...
app.controller("info", function($scope, $state, appData, $sce, appFactory) {
	...
	$scope.print = function() {
		...
		LODOP.ADD_PRINT_TEXT(32, 250, 430, 50, "个人账户信息查询表(" + year + ")");
		...
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(125, 597, 154, 20, "单位码：");
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(145, 42, 427, 20, "单位名称：");
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		...
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(930, 43, 427, 20, "网点：临汾街道社区事务受理中心");
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		...
		recordUsingHistory('医保服务', '查询+打印', '医保个人信息', $scope.xm, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
		setTimeout(function() {
			$state.go("main");
		}, 3000);
	}
...
app.controller('accountSettlement', function($scope, $state, appData, $sce, appFactory, $timeout) {
	...
	$scope.print = function() {
		...
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(940, 230, 262, 20, "网点：临汾路街道社区事务受理服务中心");
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
...
//医疗服务机构
app.controller("servicesList", function($scope, $state, appData, $sce,$location) {
	$scope.operation = "请选择查询内容";
	$scope.stuffName = servicesList;
	if(appData.lastMenu == null || appData.lastMenu == undefined || appData.lastMenu == "") {
		appData.lastMenu = $location.search().lastMenu;
		appData.YHsign = "1";
	}
	...
	$scope.prevStep = function() {
		if(appData.YHsign == "1"){
			window.location.href = "../allOrganYH/index.html#/YB_List";
		}else if(appData.type == "chxpg" || appData.type == "chxhl") {
...
app.controller("ybzlydfwChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	appData.funName='医保诊疗项目约定服务医院查询条件';
	$scope.funName = appData.funName;
	appData.project= "";
	appData.area = "";
	$scope.ybydxm = ybydxm;
...
app.controller("ybzlydfwQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName='医保诊疗项目约定服务医院查询';
	$scope.concel = "false";
	$scope.isAlert = false;
	...
	$scope.queryDiagnosisAndTreatmentProject = function() {
		$.ajax({
			...
			success: function(dataJson) {
				...
				recordUsingHistory('医保服务', '查询', '医保诊疗项目约定服务医院查询', "", "", '', '', JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery('医保诊疗项目约定服务医院查询', "", "查询", "上海市医疗保障局", "", "", "");
			},
			error: function(err) {
				$scope.isAlert = true;
				$scope.msg = "未查询到信息,请重试";
...
app.controller("chxQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName='长期护理保险护理';
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
	...
	$scope.queryNursingInstitutions = function() {
		$.ajax({
			...
			data: {
				...
				area: appData.area, //所属区
				institutionName: appData.institutionName || "", //机构名称
				address: appData.address || "", //地址
			},
			success: function(dataJson) {
				$scope.isLoading = false;
...
app.controller("sybxjgwdQueryList", function($scope, $state, appData, $sce) {
	...
	$scope.queryCommercialInsurance = function() {
		$.ajax({
			...
			data: {
				area: appData.area || "", //所属区
				commercialInstitution: appData.commercialInstitution || "", //机构名称
			},
			success: function(dataJson) {
				$scope.isLoading = false;
...
app.controller("ybxgfwQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName='医保及相关服务机构查询';
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
	...
	$scope.queryDesignatedHospital = function() {
		$.ajax({
			...
			data: {
				...
				area: appData.area,//所属区
				hospitalName: appData.hospitalName || "", //医院名称
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].ddyys) {
