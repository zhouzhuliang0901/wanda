...
app.controller("choose", function($scope, $state, appData,$timeout,$sce) {
	...
	$scope.getAuthorizationToken = function() {
		...
		$.ajax({
			...
			//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/doLogin.do",
			dataType: "json",
			// jsonp: "jsonpCallback",
			data: {
	...
	$scope.selectSpaceName = function(spaceName){
		$scope.isLoading = true
			$scope.applicationInfo1= {
				"propertyName":encodeURI(spaceName),
				"pageSize":"100"
			}
			...
			$.ajax({
				...
				//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/getEnterpriseList.do",
				dataType: "json",
				// jsonp: "jsonpCallback",
				data: {
				...
				success: function(dataJson) {
					...
					$scope.isTip = true;
					$timeout(function() {
						$scope.spaceList = dataJson.data.content;
						$scope.isLoading = false;
					}, 1000)
				},
				error: function(err) {
	...
	$scope.spaceInfo = function(codeV){
		...
		$.ajax({
			...
			//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/getEnterpriseDetail.do",
			dataType: "json",
			// jsonp: "jsonpCallback",
			data: {
				Authorization:appData.AuthorizationToken,
...
app.controller("info", function($scope, $state, appData, $sce,$timeout) {
	...
	// $scope.spaceInfoList4 = spaceInfoList4.data.content//测试
	// $scope.isblank1 = isBlanklength($scope.spaceInfoList4.satisfactionForCsp)
	// $scope.isblank2 = isBlanklength($scope.spaceInfoList4.sectPerformanceForCsp)
	// $scope.isblank3 = isBlanklength($scope.spaceInfoList4.aLevelForCsp)
	...
	$scope.spaceInfo2 = function(){
		...
		$.ajax({
			...
			//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/getTotalCommunity.do",
			dataType: "json",
			// jsonp: "jsonpCallback",
			data: {
				Authorization:appData.AuthorizationToken,
	...
	$scope.spaceInfo3 = function(){
		...
		$.ajax({
			...
			//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/getDishonestScore.do",
			dataType: "json",
			// jsonp: "jsonpCallback",
			data: {
				Authorization:appData.AuthorizationToken,
	...
	$scope.spaceInfo4 = function(){
		...
		$.ajax({
			...
			//  url: "http://180.169.7.194:8081/ac-self/selfapi/PropertyInquiry/getPerformanceInformation.do",
			dataType: "json",
			// jsonp: "jsonpCallback",
			data: {
				Authorization:appData.AuthorizationToken,
