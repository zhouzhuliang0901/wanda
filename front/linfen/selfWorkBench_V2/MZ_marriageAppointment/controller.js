...
app.controller('cancel', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.getMatterCon = function() {
		var condFlag = false;
		do {
			if(isBlank($scope.manNumber)) {
				$scope.isAlert = true;
				$scope.msg = "男方证件号码为必填项！";
				return;
			}
			if(isBlank($scope.ladyNumber)) {
				$scope.isAlert = true;
				$scope.msg = "女方证件号码为必填项！";
				return;
			}
			if(isBlank($scope.suCode)) {
				$scope.isAlert = true;
				$scope.msg = "统一审批编码为必填项！";
				return;
			}
		} while (condFlag);
