...
app.controller('loginType', function($state, $scope, appData, $state, $location) {
	//$scope.funName = appData.funName;
	appData.funName = $scope.stuffName = "开具有无违法犯罪记录证明";
...
app.controller('login', function($scope, $http, $state, appData, appFactory) {
	...
	$scope.nextStep = function() {
		...
		$scope.token = function() {
			if(appData.ywlx == "print" || appData.type == "print") {
				...
				if(jQuery.getConfigMsg.ishttpUploadYH && scanImg1 != '') {
					$.ajax({
						...
						success: function(result) {
							...
							imgHTML += '<div class="img" id="' + appData.uploadStuffId + '"><img src="' + appData.imgStr + '" width="150" height="200" /></div>';
							$('.imgBox').html(imgHTML);
							$scope.isFinish = true;
						},
						error: function(err) {
