...
app.controller("guideline", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	//进入页面 从新赋值
//	console.log(perjsonStr);
//	console.log(perjsonStr[13]);
//	appData.id =perjsonStr[13].id;
//	appData.code =perjsonStr[13].code;
//	appData.type = "handle";
//	appData.stuffName =perjsonStr[13].stuffName;
//	appData.guideline =perjsonStr[13].guideline;
//	appData.API_ID = {
//			"A": perjsonStr[13].API_ID_A,
//			"B": perjsonStr[13].API_ID_B,
//			"C": perjsonStr[13].API_ID_C,
//			"D": perjsonStr[13].API_ID_D,
//		};
	removeAnimate($('.main2'));
		...
		$scope.param = {
			...
			"query": {
				...
				"condition_detail": { //查询条件
					"Q_EQ_MAN": (appData.submitInfo.stSex == '男')?appData.submitInfo.stName:appData.submitInfo.stWomen,
                    "Q_EQ_WOMAN":  (appData.submitInfo.stSex == '男')?appData.submitInfo.stWomen:appData.submitInfo.stName,
					"Q_EQ_DISTRICT": ""
				}
			}
		};
		...
		$scope.submit = function() {
			...
			$.ajax({
				...
				data: {
					...
					param: encodeURI(JSON.stringify($scope.param))
...
app.controller("preview", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	trackEvent("档案查询",appData.stuffName);
	$scope.stuffName = appData.stuffName;
	$scope.nextText = "打印";
	$scope.isAlert = false;
	$scope.concel = "false";
			...
			success: function(data) {
				...
				try {
					...
					if(data.pngData.length == 0) {
						...
						appData.printPdfBase64 = data.base64;
					}
				} catch(e) {
					console.log(e);
					$scope.isAlert = true;
	...
	$scope.print = function() {
		...
		//		};
		//		lodop.PRINT();
		$.device.urlPdfPrint($scope.printPdf,"D:\\pdfPrint.pdf",function(){
