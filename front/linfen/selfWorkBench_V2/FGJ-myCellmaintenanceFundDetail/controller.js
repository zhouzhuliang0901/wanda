	...
	layui.use(['form', 'layedit', 'laydate'], function() {
		...
		form.on('select(xiaoqu)', function(data) {
			if (data.value) {
				...
				$.customAjax.post($.getConfigMsg.preUrlSelf+'/selfapi/housingManagement/getUnitId.do', {
					tokenSNO: $rootScope.tokenSNO,
		...
		form.on('select(menpai)', function(data) {
			if (data.value) {
				...
				$.customAjax.post($.getConfigMsg.preUrlSelf+'/selfapi/housingManagement/getHouseId.do', {
					tokenSNO: $rootScope.tokenSNO,
					communityId: appData.communityId,
	...
	$scope.query = function(index, item) {
		...
		$.customAjax.post($.getConfigMsg.preUrlSelf + '/selfapi/housingManagement/checkHouse.do', {
			...
			if (res.code == '200') {
				...
				appData.infoHouId = item.houId;
				appData.houAddress = item.houAddress;
				$state.go('info');
			} else {
				$scope.isAlert = true;
				$scope.msg = res.msg;
...
app.controller('info', function($state, $scope, appData, $rootScope) {
	removeAnimate($('.scrollBox2'));
	addAnimate($('.scrollBox2'));
	$scope.houAddress = appData.houAddress;
	$scope.isAlert = false;
	$scope.concel = "false";
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
	...
	$scope.queryInfo = function(flag) {
		$scope.isLoading = true;
		$.customAjax.post($.getConfigMsg.preUrlSelf+'/selfapi/housingManagement/maintenanceFundDetail.do', {
			tokenSNO: $rootScope.tokenSNO,
			houId: appData.infoHouId,
			flag:flag,
		}, function(res) {
			console.log(res);
			$scope.isLoading = false;
			if (res.code == "200") {
				$scope.info = res.data.account;
				if(flag == "1"){
					//支出
					$scope.zcList = res.data.list;
				}else if(flag == "2"){
					$scope.srList = res.data.list;
				}else{
					$scope.allList = res.data.list;
				}
				//模块使用记录
				$scope.jsonStr = {
					SUCCESS: "true",
				...
				recordUsingHistory('房管局服务', '查询', appData.funName, appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
			} else {
				$scope.isAlert = true;
				$scope.msg = res.msg;
			}
