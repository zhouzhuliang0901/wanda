...
app.controller("guideline", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	$scope.funName = appData.funName = "退伍军人档案查询";
...
//		$scope.getUserInfoByAccessToken();
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($scope.rwbd)) {
				$scope.isAlert = true;
				$scope.msg = "请输入入伍部队";
				return
			}
			if(isBlank($scope.paramsInfo.stEnlistDate)) {
				$scope.isAlert = true;
				$scope.msg = "请选择入伍时间";
				return
			}
			if(isBlank($scope.rwqgzdw)) {
				$scope.isAlert = true;
				$scope.msg = "请输入入伍前工作单位";
				return
			}
		...
		let detailParams = {
			...
			rwqgzdw: $scope.paramsInfo.rwqgzdw || "",
			rwbd: $scope.paramsInfo.rwbd || "",
			rwsj: $scope.paramsInfo.stEnlistDate || "",
		}
