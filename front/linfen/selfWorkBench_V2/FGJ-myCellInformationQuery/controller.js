...
app.controller('info', function($state, $scope, appData, $rootScope) {
	...
	$scope.getOwnersCongress = function(id) {
		$scope.isLoading = true;
		$.customAjax.post($.getConfigMsg.preUrlSelf + '/selfapi/housingManagement/getOwnersCongress.do', {
			ownersCongressId: id,
			tokenSNO: $rootScope.tokenSNO
		}, function(res) {
			console.log(res);
			$scope.isLoading = false;
			if (res.code == "200") {
				$scope.ownerInfo = res.data;
				$scope.ownerInfo.directorName = getString($scope.ownerInfo.directorName);
				$scope.ownerInfo.viceDirectorName = getString($scope.ownerInfo.viceDirectorName);
			}
		}, function(err) {})
	}
	...
	$scope.queryInfo = function() {
		$scope.isLoading = true;
		$.customAjax.post($.getConfigMsg.preUrlSelf + '/selfapi/housingManagement/getCommunityInfo.do', {
			tokenSNO: $rootScope.tokenSNO,
			...
			$scope.isLoading = false;
			if (res.code == "200") {
				$scope.basicinfo = res.data.community[0];
				$scope.glcinfo = res.data.managementOffice[0];
				$scope.yzinfo = res.data.ownersCongress[0];
				$scope.wyinfo = res.data.property[0];
				$scope.getOwnersCongress($scope.yzinfo.ID);
				//模块使用记录
				$scope.jsonStr = {
					SUCCESS: "true",
					data: {
