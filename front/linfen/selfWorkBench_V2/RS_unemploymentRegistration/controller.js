app.controller('selectList', function($state, $scope, appData, $location, appFactory) {
	...
	$scope.getMatterCon = function(itemName, marriageType,code,content) {
		appData.funName = itemName;//事项名称
		appData.marriageType = marriageType;//事项类别
		appData.content = content;//办理须知
		$state.go("loginType")
...
app.controller('info4', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	$scope.funName = "来沪人员失业登记退出";
	// $scope.isLoading = true;
	// appData.licenseNumber = "341623199301038318";
	$scope.nextText = "提交"
	$scope.isShow1 = false;
	$scope.isShow2 = false;
	$scope.concel = "false";
	...
	// $scope.isLoading = true;
	$scope.quitList = quitList//退出原因列表
	$scope.stIdCard = appData.licenseNumber;//办理人身份证号码
	//获取用户id
	$scope.getUserInfoByAccessToken = function() {
		console.log("token:"+appData.token);
	...
	$scope.nextStep = function() {
		...
		$scope.isLoading = true;
		$scope.applicationInfo1 = {
			"user_id":"0000000003",//操作员代码
			"Cetf_id":$scope.stIdCard,//证件号码
			"tcyy":$scope.quitListName.key,//退出原因
	}
	console.log("提交参数"+ JSON.stringify(	$scope.applicationInfo1))
		$.ajax({
			type: "post",
			url: $.getConfigMsg.preUrlSelf + '/selfapi/UnemploymentRegistration/LD0009S2',
			// url: "http://localhost:8080/ac-self/selfapi/UnemploymentRegistration/LD0009S2",
			dataType: "json",
			data: {
				json: JSON.stringify($scope.applicationInfo1)
			},
...
app.controller("submit", function($scope, $state, appData, $sce, appFactory) {
	$scope.funName = appData.funName = "失业登记";
