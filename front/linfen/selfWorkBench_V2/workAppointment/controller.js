...
//选择区域
app.controller('choiceArea', function($scope, $location, instance, appFactory, $timeout, $http, $state) {
	$scope.isLoading = false;
	$scope.isAlert = false;
	$scope.concel = "false";
	$scope.current = 0;
	instance.areaName = "浦东新区"
	$scope.current2 = 0;
	instance.type = '区级行政服务中心';
	$scope.areaList = [{
			"areaCode": "14",
			"areaName": "浦东新区"
		}, {
	...
	$scope.typeList = ['区级行政服务中心', '社区事务受理服务中心', '区级部门窗口'];
	$scope.change = function(index, item) {
		$scope.current = index;
		$scope.areaName = instance.areaName = item.areaName;
	}

	$scope.change2 = function(index, item) {
		$scope.current2 = index;
		$scope.type = instance.type = item;
	}
	$scope.nextStep = function() {
		//		if($scope.type == "区级行政服务中心"){
		$state.go('choiceCenter')
		//		}else{
		//			$scope.isAlert = true;
		//			$scope.msg = "暂未开放";
...
//选择区域
app.controller('choiceCenter', function($scope, $location, instance, appFactory, $timeout, $http, $state) {
	$scope.areaName = instance.areaName;
	$scope.type = instance.type;
	$scope.isLoading = false;
	$scope.isAlert = false;
	$scope.concel = "false";
	//通过区域查询办理点
	$scope.fetchPlaceList = function() {
		appFactory.pro_fetch("fetchPlaceList.do", {
			"stParentDepartmentName": encodeURI(instance.areaName),
			"stOrganType": encodeURI(instance.type)
		}, function(res) {
			if(res.code == 200) {
				$scope.licenseList = res.data;
			} else {
				$scope.isAlert = true;
				$scope.msg = "暂未查询到信息";
				$scope.alertConfirm = function() {
	...
	$scope.getNotice = function(placeId) {
		appFactory.pro_fetch("getNotice.do", {
			"placeId": placeId
		}, function(res) {
			console.log(res);
			if(res.code == 200 && res.data.length == 0) {
				...
				$scope.isAlert = true;
				$scope.msg = res.data[0].stNoticeContent;
				$scope.alertConfirm = function() {
					$state.go('one');
				}
			} else {
	...
	$scope.choice = function(item) {
		instance.bldCode = item.windowCode;
		instance.bld = item.placeName;
		instance.bldAddress = item.placeAddress;
		$scope.getNotice(item.placeId);
	}
...
app.controller('onecontroller', function($scope, $location, instance, appFactory, $timeout, $http, $state) {
	...
	$scope.bldCode = instance.bldCode;
	$scope.list = [];
	$scope.itemList = [];
	...
	$scope.orderTimes = function(date) { //获取预约时段
		...
		appFactory.pro_fetch("getResourceList.do", {
			...
			for(var j = 0; j < $scope.showDataMake.length; j++) {
				let dateO = $scope.showDataMake[j].stPeriod.split('~')[0];
				let date = $scope.showDataMake[j].stPeriod.split('~')[1];
				console.log(date);
				$scope.SXO = dateO.match(/(\S*):/)[1];
				$scope.SX = date.match(/(\S*):30/)[1];
				console.log($scope.SXO);
				console.log($scope.SX);
				if($scope.SXO < 12 && $scope.SX > 12) {
					$scope.showDataMake[j].sxm = "全天";
				} else if($scope.SX > 12) {
...
app.controller('fourcontroller', function($scope, $location, instance, $state, appData) {
	...
	$scope.choiceType = function(type) {
		if(type == 'idcard') {
			...
			$scope.operation = "第二步：请将随申办中随申码或身份证二维码放置扫码区";
			// $state.go(".second");
			setTimeout(function() {
				console.info('a');
	...
	$scope.readCloud = function(data) {
		...
		appData.licenseNumber = data.result.data.idcard;
...
app.controller('sixcontroller', function($scope, $location, $timeout, appData, instance) {
	...
	if(appData.zctepn === 0) {
		...
		$scope.completeTips1 = "请于" + instance.date + " " + instance.time + " 前往" + instance.bld + "办理";
		$scope.completeTips2 = "地址：" + instance.bldAddress;
		//模块使用记录
		$scope.jsonStr = {
			SUCCESS: "true",
			data: {
				name: instance.stItemName,
			}
		}
		recordUsingHistory('自助预约', '办理', instance.stItemName, appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
