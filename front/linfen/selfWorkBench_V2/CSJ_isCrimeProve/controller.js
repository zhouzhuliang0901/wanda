...
app.controller('choiceMode', function($state, $scope, appData, $timeout, appFactory, $http) {
	$scope.funName = appData.funName = "长三角有无违法犯罪记录证明开具";
	appData.itemCode = $scope.itemCode = "312000218000";
	$scope.isAlert = false;
	$scope.concel = "false";
	...
	$scope.choiceType = function(type) {
		...
		if(appData.type == "sh") {
			if(appData.modeType == "print") {
				window.location.href = '../GA_isCrimeProve/index.html#/loginType?sign=1&type=print';
			} else {
				window.location.href = '../GA_isCrimeProve/index.html#/loginType?sign=1';
			}
		} else if(appData.type == "ah" || appData.type == "zj") {
			$scope.isAlert = true;
			$scope.msg = "该省市暂未开通开具违法证明，请敬请等待！"
		} else if(appData.type == "js") {
			$state.go("loginType");
		}
	}
	$scope.prevStep = function() {
...
//		$scope.getAccessToken($rootScope.tokenSNO);
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($scope.liveAddress)) {
				$scope.isAlert = true;
				$scope.msg = "请输入现居住地";
				return
			}
			if(isBlank($scope.hjAddress)) {
				$scope.isAlert = true;
				$scope.msg = "请输入户籍所在地";
				return
			}
			...
			if(isBlank($scope.sqly)) {
				$scope.isAlert = true;
				$scope.msg = "请选择申请理由";
				return
			}
			if(isBlank($('#dtStartDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择证明开始日期";
				return
			}
			if(isBlank($('#dtEndDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择证明结束日期";
				return
			}
			if($("#license_type .choice").text() < 1) {
				$scope.isAlert = true;
				$scope.msg = "请选择证明类型！";
				return;
			}
		...
		for(var i = 0; i < list.length; i++) {
			if(list[i].innerHTML != undefined) {
				zmlxNames.push(list[i].innerHTML);
				var id = getZmlx(list[i].innerHTML);
				zmlxItems.push(id);
				if(zmlxValue == "") {
					zmlxValue = id;
				} else {
					zmlxValue = zmlxValue + "," + id
				}
			}
		}
		appData.mobile = $scope.stMobile;
		appData.dtStartDate = $('#dtStartDate').val();
		appData.dtStartDateUnix = Date.parse(new Date(appData.dtStartDate));
		appData.dtEndDate = $('#dtEndDate').val();
		appData.dtEndDateUnix = Date.parse(new Date(appData.dtEndDate));
		appData.paramStr = {
			"data": {
				"applyNo": "",
				...
				"info": {
					...
					"EndTime": {
						"value": appData.dtEndDate,
						"unix": appData.dtEndDateUnix
					},
					"bls": {
						"name": "江苏省",
						"value": "3"
					...
					"StartTime": {
						"value": appData.dtStartDate,
						"unix": appData.dtStartDateUnix
					},
					"nowAddress": $scope.liveAddress,
					"wffzjlzl": {
						"names": zmlxNames,
						"value": zmlxValue,
						"items": zmlxItems
					},
					"hjAddress": $scope.hjAddress,
				...
				if(jQuery.getConfigMsg.ishttpUploadYH && scanImg1 != '') {
					$.ajax({
						...
						success: function(result) {
							...
							imgHTML += '<div class="img" id="' + result.stuffId + '"><img src="' + appData.imgStr + '" width="150" height="200" /></div>';
							$('.imgBox').html(imgHTML);
							$scope.isFinish = true;
						},
						error: function(err) {
