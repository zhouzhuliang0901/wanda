...
app.controller('login', function($scope, $http, $state, appData, appFactory, $timeout) {
	...
	$scope.idcardLogin = function(info, images) {
		if(info) {
			...
			appData.ValidtermOfStart = info.ValidtermOfStart;
			appData.ValidtermOfEnd = info.ValidtermOfEnd;
			appData.Address = info.Address;
			$state.go('recognition');

//			appData.Number = '310228198808070818';
//			appData.Name = '陈雷';
//			$state.go('info');
...
app.controller("recognition", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	if(window.innerWidth < 1600) {
		$scope.__cameraPos.width = 640;
		$scope.__cameraPos.height = 480;
		$scope.__cameraPos.x = 325;
		$scope.__cameraPos.y = 240;
	}
//	if(acBridgeMac.imagePath() == "zhuofansoft_gzt") {
//				$scope.__cameraPos.x = 240;
//				$scope.__cameraPos.y = 640;
//	}
	...
	$scope.getTokenSNO = function() {
		var rec = $.ajax({
			...
			jsonp: "jsonpCallback",
			data: {
				name: encodeURI(appData.Name),
				idCard: appData.Number,
				facePhoto: appData.faceImg,
				copyIDPhoto: appData.idCardImg || appData.faceImg,
				certStartTime:appData.ValidtermOfStart || "",
				certEndTime:encodeURI(appData.ValidtermOfEnd || "")
			},
...
//	addAnimate($('.stable'))
	...
	$scope.mAddress=appData.Address||'';
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.SUCCESS == "true") {
					...
					recordUsingHistory('经信委服务', '办理', '市民网上实名认证服务', appData.Name, appData.Number, $scope.count, '', JSON.stringify($scope.jsonStr));
					//行为分析(办理)
					trackEventForAffairs("", "市民网上实名认证服务", "上海市经济和信息化委员会", appData.Name, appData.Number, "");
				} else {
					if(data.result == "-1200") {
						$scope.isAlert = true;
