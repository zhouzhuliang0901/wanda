...
function getIdcardInfo() {
	...
	try {
		idCardData.Name = window.external.GetConfig("hardwareidCardName");
	} catch(e) {}
	try {
		idCardData.Number = window.external.GetConfig("hardwareidCardNumber");
	} catch(e) {}
	try {
		idCardData.ValidtermOfStart = window.external.GetConfig("hardwareidCardDateStart");
	} catch(e) {}
	try {
		idCardData.ValidtermOfEnd = window.external.GetConfig("hardwareidCardDataEnd");
	} catch(e) {}
	if(idCardData.Name!=''&&idCardData.Number!=''){
		return JSON.stringify(idCardData);
	}else{
		alert('配置文件出错');
	}
}
