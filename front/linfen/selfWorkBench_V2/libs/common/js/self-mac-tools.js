...
function trackEvent1(eventName) {
	...
	//	]);
	//	_vlstatId.push(['netFlag', eventName], ['ext1', '工作台']);
	//	(function() {
...
function saveDeviceStatus(stOutDeviceCode, nmException, stCause, nmNotice, nmTotal, nmRemain, count) {
	$.ajax({
		type: "get",
		url: $.getConfigMsg.preUrlSelfApi + "/outdevicestatus/odeviceStatus/save.do",
		dataType: "jsonp",
		jsonp: "jsonpCallback",
		data: {
			stDeviceId: $.config.get("uniqueId"),
			stOutDeviceCode: encodeURI(stOutDeviceCode),
			nmException: nmException,
			stCause: encodeURI(stCause),
			nmNotice: nmNotice,
			nmTotal: nmTotal,
			nmRemain: nmRemain,
			count: count
		},
		succes: function(dataJson) {
		...
		error: function(err) {
			console.log("odeviceStatus/save.do err");
		}
	});
}
/**
		...
		try {
			...
			if (jQuery.lodop.needCLodop()) {
				...
				if ((!LODOP) || (!LODOP.VERSION)) {
					...
					document.body.innerHTML = (is64IE ? strHtm64_Install : strHtmInstall) + document.body
						.innerHTML;
					return LODOP;
				}
			}
			if (LODOP.VERSION < "6.2.2.6") {
				if (!needCLodop())
					document.body.innerHTML = (is64IE ? strHtm64_Update : strHtmUpdate) + document.body
					.innerHTML;
			}
...
jQuery.device = {
	...
	GoHome: function() {
		try {
			console.log('万达:返回首页');
			var goHomeProjectFileName='selfWorkBench_V2/';
			try {
			...
			if (acBridgeMac.vendor() == "nhdevice") {
				if(nhdeviceSum.Phaseiidevice()){
					nhdeviceSum.nhGoHome();
				}else{
					window.external.GoHome();
				}
			} else if (('index-shsdsx' == yhPublicItemdeviceName) || ('csjindex' == yhPublicItemdeviceName) || (
					'index-yh-yb' == yhPublicItemdeviceName) || ('indexMainPageYH/index.html' ==
					yhPublicItemdeviceName)) {
				var test = window.location.href,
					midpiece = '';
				midpiece = goHomeProjectFileName+"index.html#/";
				if ('indexMainPageYH/index.html' == yhPublicItemdeviceName) midpiece = goHomeProjectFileName;
				test = test.split('selfWorkBench_V2')[0] + midpiece + yhPublicItemdeviceName;
				window.location.href = test;
			} else if (acBridgeMac.vendor() == "zhdevice") {
				var test = window.location.href;
				var zhdeviceUrlSuffixName = 'index-zh';
				if (localStorage.getItem("zhdeviceUrlSuffixName") != null) {
					zhdeviceUrlSuffixName = localStorage.getItem("zhdeviceUrlSuffixName");
				}
				test = test.split('selfWorkBench_V2')[0] + goHomeProjectFileName+"index.html#/" +
					zhdeviceUrlSuffixName;
				window.location.href = test;
			} else if (acBridgeMac.vendor() == 'pfdevice') {
				var test = window.location.href;
				test = test.split('selfWorkBench_V2')[0] + goHomeProjectFileName+"index.html#/index-pf"
				window.location.href = test;
			} else if (acBridgeMac.vendor() == 'jtdevice') {
				var test = window.location.href;
				test = test.split('selfWorkBench_V2')[0] + goHomeProjectFileName+"index.html#/index-jtyh"
				window.location.href = test;
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				var test = window.location.href;
				var nsdeviceUrlSuffixName = 'index-ns';
				if (localStorage.getItem("nsdeviceUrlSuffixName") != null) {
					nsdeviceUrlSuffixName = localStorage.getItem("nsdeviceUrlSuffixName");
				}
				test = test.split('selfWorkBench_V2')[0] + goHomeProjectFileName+"index.html#/" +
					nsdeviceUrlSuffixName;
				window.location.href = test;
	...
	Camera_Show: function() {
		try {
			 ...
			 */
			var this_ = this
			this_.audioPPlay("Realtek High Definition Audio", this_.currentPath() +
				"\\resources\\audio\\recognition.wav");
	...
	Face_Show: function(width, height, x, y, callback) {
		try {
			saveDeviceStatus("camera", 0, "正常", 0, 0, 0, 0);
			...
			if (acBridgeMac.vendor() == 'wonders' && window.external.GetConfig('liveDetection') == 'N') {
			...
			} else if (acBridgeMac.vendor() == 'wonders' && window.external.GetConfig('liveDetection') ==
				'hxdevice') {
				JS_OpenDevice();
				window.JS_CaptureFinish = function(code, imagePath1, imagePath2) {
					if (0 == code) {
						// 活体抓拍成功，活体;
						// 更新活体拍照图像;
						var new_image_path = imagePath2;
						$.device.fileBase64(new_image_path, function(val) {
							callback(val);
						})
					} else {
						$.log.debug("活体抓拍超时");
				...
				if(nhdeviceSum.Phaseiidevice()){
					nhdeviceSum.getFaceImage(callback)
				}else{
					onPageLoad();
				...
				$.deviceBocom.faceShow(430, 275, 385, 300, function(value) {
					callback(JSON.parse(value.Data).ImageStream)
				});
			} else if (acBridgeMac.vendor() == 'ghdevice') {
				SetProperty(); //设置窗口
				setTimeout(function() {
				...
				setTimeout(function() {
					starttakepicture(function(val) {
						ghWinPathtoBase64('C:\\\\1.jpg', callback)
					}); //开始拍照
				}, 8000)
				setTimeout(function() {
					this_.Face_Close();
				...
				$.nsdeviceSum.Camera_Show(560, 400, 330, 300, function(value) {
					if (value.Result == 0) {
						setTimeout(function() {
							$.nsdeviceSum.Camera_Base64(function(value) {
								if (value.Result == 0) {
									callback(value.Base64);
								}
								$.nsdeviceSum.Camera_Hide(function(result) {})
							})
						}, 3000);
					}
				})
			...
			saveDeviceStatus("camera", 1, "未正常调用", 0, 0, 0, 0);
		}
	},
	//关闭活体检测
	Face_Close: function() {
	...
	qrCodeOpen: function(callback) {
		try {
			...
			if (acBridgeMac.vendor() == 'jhdevice') {
				...
				$.deviceBocom.qrCodeOpen(callback);
			} else if (acBridgeMac.vendor() == 'ghdevice') {
				readcode();
				window.ghqrScannerCallBack = function(value) {
					callback(value);
				};
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				$.nsdeviceSum.qrCodeOpen(function(value) {
					if (value.Result == 0) {
						callback(value.Data.BarcodeData)
					}
				})
			} else {
	...
	qrCodeClose: function() {
		try {
			...
			if (this.vendor() == 'nhdevice') {
				...
				//closedevicebarcodegh();
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				$.nsdeviceSum.qrCodeClose(function(result) {})
			} else {
	...
	idCardOpen: function(callback) {
		try {
			...
			} else if (this.vendor() == 'jtdevice') {
				$.deviceBocom.idCardOpen(function(value) {
					callback(JSON.stringify(value))
				});
			} else if (acBridgeMac.vendor() == 'ghdevice') {
				opendev();
				window.ghidCardCallBack = function(value) {
					callback(value);
				};
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				$.nsdeviceSum.idCardOpen(function(value) {
					if (value.Result == 0) {
	...
	idCardClose: function() {
		try {
			...
			if (this.vendor() == 'nhdevice') {
				...
				$.deviceBocom.idCardClose();
			} else if (this.vendor() == 'ghdevice') {
				//closedevice();
			} else if (this.vendor() == 'nsdevice') {
				$.nsdeviceSum.idCardClose(function(value) {});
			} else {
	...
	ssCardOpen: function(callback) {
		try {
			...
			saveDeviceStatus("SocialCard", 0, "正常", 0, 0, 0, 0);
		} catch (e) {
			saveDeviceStatus("SocialCard", 1, "未正常调用", 0, 0, 0, 0);
		}
	},
	...
	openUrlWindow: function(url) {
		try {
			if (this.vendor() == 'wonders') {
				...
				InnerChromeBrowserUtil.CreateNewChromeForm(window.screen.availWidth - 280, window.screen
					.availHeight - 280, 180, 180, url, 1)
			} else if (this.vendor() == 'epoint') {
	...
	medicalPrint: function(newBookBh, newBookKh, userXb, userName, userAge) {
		try {
			if (this.vendor() == 'wonders') {
				...
				//saveDeviceStatus("医保记录册", 0, "正常", 0, 0, 0, 1);
				//				setTimeout(function() {
				...
				var txt1 = txt0 + txt1 + txt2 + txt3 + txt4 + txt5 + txt6 + txt7 + txt11 + txt12 + txt13 +
					txt14 + txt15 + txt16 + txt17;
				txt =
					'<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body>' +
					'</head><body>' + txt1 + '</body></html>';
				...
				$.ajax({
					...
					success: function(rtnData) {
						...
						window.external.printCardJYC(
							"http://10.81.16.48:8080/epoint-web-zwfwznsb/rest/auditattach/readAttach?attachguid=" +
							taskdocattachguid);
					},
					error: function(err) {
						alert("err" + err)
				...
				openMedicalPrinter(newBookBh, newBookKh, userXb, userName, userAge);
			}
		} catch (error) {}
	},
	// 高拍仪
	cmCaptureShow: function(width, height, x, y) {
		try {
			...
			if (acBridgeMac.vendor() == 'ghdevice') {
				...
				zhcmCapture.cmCaptureShow_zh(200, 250, 390, 390, function(res) {});
			} else if (acBridgeMac.vendor() == "nhdevice") {
				if(nhdeviceSum.Phaseiidevice()){
					nhdeviceSum.nhopenSnCamera();
				}
			} else {
	...
	cmCaptureCaptureUrl: function(callback) {
		try {
			if (acBridgeMac.vendor() == 'ghdevice') {
				...
				setTimeout(function() {
					zhcmCapture.cmCaptureCaptureBase64_zh(function(value) {
						callback(value);
					});
				}, 1)
			} else {
	...
	cmCaptureCaptureBase64: function(callback) {
		try {
			if (acBridgeMac.vendor() == 'ghdevice') {
				var paths = Capture();
				ghWinPathtoBase64(paths, function(value) {
					callback(value)
				})
			} else if (acBridgeMac.vendor() == "zhdevice") {
				zhcmCapture.cmCaptureCaptureBase64_zh(function(value) {
					callback(value.replace(/^data:image\/\w+;base64,/, ""))
				});
			} else if (acBridgeMac.vendor() == "nhdevice") {
				if(nhdeviceSum.Phaseiidevice()){
					nhdeviceSum.nhsnCameraBeginSna(callback);
				}
			} else {
	...
	cmCaptureHide: function() {
		try {
			if (this.vendor() == 'ghdevice') {
				ghCaptureClosedevice();
			} else if (acBridgeMac.vendor() == "zhdevice") {
				zhcmCapture.cmCaptureHide_zh(function() {});
			} else if (acBridgeMac.vendor() == "nhdevice") {
				if(nhdeviceSum.Phaseiidevice()){
					nhdeviceSum.nhcloseSnCamera();
				}
			} else {
	...
	audioSound: function(type) {
		try {
			...
			if (acBridgeMac.vendor() == 'wonders') {
				//身份证阅读器
				if (type == 'IdCard') {
					window.external.Hd_Audio_PPlay("Realtek High Definition Audio", this_.currentPath() +
						"\\resources\\audio\\idcard.wav");
				}
				//高拍仪
				else if (type == 'CmCapture') {
					this_.audioPPlay("Realtek High Definition Audio", this_.currentPath() +
						"\\resources\\audio\\cmCapture.wav");
				}
				//二维码扫码枪
				...
				//居住证擦写设备
					...
					this_.audioPPlay("Realtek High Definition Audio", this_.currentPath() +
						"\\resources\\audio\\residenceVisa.wav");
				} else if (type == "sscard") {
					this_.audioPPlay("Realtek High Definition Audio", this_.currentPath() +
						"\\resources\\audio\\sscard.wav");
				} else if (type == "Camera") {
	...
	fileBase64: function(filePath, callback) {
		try {
			if (this.vendor() == 'jhdevice') {
				...
				callback($.deviceBocom.fileBase64(filePath));
			} else if (acBridgeMac.vendor() == 'ghdevice') {
				ghWinPathtoBase64(filePath, callback);
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				callback($.nsdeviceSum.getbase64(filePath))
			} else {
	...
	urlPdfPrint: function(url, filePath, callback, urlbase64) {
		try {
			...
			if (acBridgeMac.vendor() == 'jhdevice') {
				...
				$.deviceBocom.printPdf(url, function(value) {});
			} else if (acBridgeMac.vendor() == 'ghdevice') {
				if (urlbase64 != undefined) {
					opendevPdfPrint();
					setTimeout(function() {
						base64tofile(urlbase64);
					}, 1000)
				}
			} else if (acBridgeMac.vendor() == 'nsdevice') {
				$.nsdeviceSum.printerPrint(urlbase64, function(result) {
					console.log("printerPrint done:" + JSON.stringify(result))
				})
			} else {
	...
	printGetLodop: function(type) {
		try {
			...
			if (acBridgeMac.vendor() == "nsdevice") {
...
jQuery.config = {
	get: function(key) {
		try {
			if (key == "uniqueId" && acBridgeMac.vendor() == "zhdevice") {
				return JSON.parse(window.AppHost.getManage().getMachineInfo())['networkinterface'][0].mac.match(
					new RegExp("\\w{1," + 2 + "}", "g")).join("-");
			...
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "ghdevice") {
				return getlocalmacgh();
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "zhuofansoft") {
				return eval('(' + LocalFileUtil.GetMac() + ')').macs.slice(0,17).replace(/\:/g,"-");
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "epoint") {
				return window.external.GetMacAddress().replace(/:/g, '-');
			} else {
	...
	load: function(key, callback) {
		try {
			if (key == "uniqueId" && acBridgeMac.vendor() == "zhdevice") {
				callback(JSON.parse(window.AppHost.getManage().getMachineInfo())['networkinterface'][0].mac
					.match(new RegExp("\\w{1," + 2 + "}", "g")).join("-"));
			} else if (key == "isLxmPrintMachine" && acBridgeMac.vendor() == "zhdevice") {
				try {
			...
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "nsdevice") {
				$.nsdeviceSum.getConfig(function(value) {
					if (value.Result == 0) {
						callback(value.Mac)
					}
				})
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "ghdevice") {
				callback(getlocalmacgh())
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "nhdevice") {
				nhdeviceSum.nhgetMac(callback)
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "zhuofansoft") {
				callback(eval('(' + LocalFileUtil.GetMac() + ')').macs.slice(0,17).replace(/\:/g,"-"))
			} else if (key == "uniqueId" && acBridgeMac.vendor() == "epoint") {
				callback(window.external.GetMacAddress().replace(/:/g, '-'))
			} else {
