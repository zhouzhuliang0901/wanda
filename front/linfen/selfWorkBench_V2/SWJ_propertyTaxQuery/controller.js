app.controller('guideline', function($state, $scope, appData, $http) {
	$scope.funName = "个人住房房产税应税及已缴税情况查询";
	$scope.isAlert = false;
...
app.controller('choose', function($state, $scope, appData, $location, $timeout, $rootScope) {
	...
	$scope.change3 = function(name, index, id) {
		$scope.current3 = index;
		appData.cardType = $scope.id;
	}
	$scope.prevStep = function() {
		$state.go("guideline");
	}
	$scope.nextStep = function() {
		...
		appData.licenseNumber = $scope.cardNo;
