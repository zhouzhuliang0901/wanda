...
//			return ''
		...
		if($scope.stMateIdCard) {
			try {
				$scope.stMateBirth = ($scope.stMateIdCard).substring(6, 10) + "-" + ($scope.stMateIdCard).substring(10, 12) + "-" + ($scope.stMateIdCard).substring(12, 14);
			} catch(e) {
				$scope.stMateBirth = "";
			}
		} else {
		...
		$scope.param = {
			...
			"query": {
				...
				"condition_detail": { //查询条件
					"Q_EQ_NAME_MALE": (appData.submitInfo.stSex == "男") ? appData.submitInfo.stName : appData.submitInfo.stMateName,
					"Q_EQ_NAME_FEMALE": (appData.submitInfo.stSex == "女") ? appData.submitInfo.stName : appData.submitInfo.stMateName,
		...
		//				"slrxm": "社区开发",
		//				"org_name": "自助终端",
		//				"org_code": "20190926002",
		//				"code": appData.id,
		//				"ssdagmc": "崇明区档案馆",
		//				"condition_detail": {
		//					"Q_LIKE_NAME_MALE": "伍德罗·帕克斯（WOODREW PARKS）",
		//					"Q_LIKE_NAME_FEMALE": "贺健",
		//					"Q_LIKE_ID_NO_MALE": "E841928",
		//					"Q_LIKE_IDTYPE_MALE": "护照",
		//					"Q_EQ_DISTRICT": "shdaj"
		//				}
		//			}
		//		}
