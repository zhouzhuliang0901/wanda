...
app.controller('main', function($state, $scope, appData, $http, $location) {
	...
	// 以下三个变量参数用于接口查询, 不可更改!
	appData.itemName = '居住证积分查询';
	appData.code = 'xxxx';
	appData.type = 'residencePermit';
    //显示社保卡登录选项
...
app.controller("social", function($scope, $http, $state, appData, appFactory, $timeout) {
	...
	$scope.humanSocietyPrint = function() {
		$.ajax({
			type: "get",
			url: $scope.configUrl + '/aci/workPlatform/humanSociety/humanSocietyPrint.do',
			/*url写异域的请求地址*/
			dataType: "json",
			/*加上datatype*/
			jsonp: "jsonpCallback",
	...
	$scope.humanSocietyQuery = function() {
		...
		$.ajax({
			...
			success: function(dataJson) {
				console.log(dataJson);
				if(dataJson.tybm != null && dataJson.tybm != '' && dataJson.success == "1") {
		...
		//		$http.jsonp($.getConfigMsg.preUrl + '/aci/workPlatform/humanSociety/humanSocietyQuery.do', {
		//			params: {
		//				jsonpCallback: "JSON_CALLBACK",
		//				type: appData.type,
		//				zjhm: appData.licenseNumber,
	...
	$scope.queryResidenceLicenseScore = function() {
		$.ajax({
			...
			success: function(dataJson) {
				...
				recordUsingHistory('人社服务', '查询', appData.funName, appData.licenseName, appData.licenseNumber, '', "", JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery(appData.funName, "", "查询", "上海市人力资源社会保障局", appData.licenseName, appData.licenseNumber, "");
