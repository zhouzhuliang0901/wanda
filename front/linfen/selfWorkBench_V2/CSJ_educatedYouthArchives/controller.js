...
app.controller('info', function($state, $scope, appData, $timeout, appFactory, $http) {
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if (isBlank($scope.stSsxxAddress)) {
				$scope.isAlert = true;
				$scope.msg = "请输入上山下乡地点";
				return
			}
			if (isBlank($scope.paramsInfo.stSsxxDate)) {
				$scope.isAlert = true;
				$scope.msg = "请选择上山下乡时间";
				return
			}
...
app.controller('submit', function($state, $scope, appData) {
	...
	recordUsingHistory('长三角服务', '办理', appData.funName, appData.licenseName, appData.licenseNumber, '', $scope
		.applyNo, JSON.stringify($scope.jsonStr));
	//行为分析(办理)
