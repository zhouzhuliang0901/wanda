...
app.controller("info", function($scope, $state, appData, $sce, appFactory, $timeout) {
	...
	console.log($scope.nextShowname);
	$timeout(function() {
	...
	$scope.accumulationFundInfo = function() {
		...
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrl + "/selfapi/SJInfoQuery/queryAccumulationFund.do",
			dataType: "json",
			jsonp: "jsonpCallback",
			data: {
			...
			success: function(dataJsonp) {
				if(dataJsonp.head.rst.buscode == "000000") {
					...
					recordUsingHistory('住建服务', '查询+打印', '个人住房公积金信息查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
					//行为分析(查询)
