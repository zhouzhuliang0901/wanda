...
app.controller('info', function($state, $scope, appData, $rootScope, $timeout, appFactory, $http) {
	...
	$scope.$watch('outCity', function(val) {
		if(val) {
			...
			if(val.name.indexOf('省本级') == -1){
				$scope.isOutBenShen = false;
			}else{
				$scope.isOutBenShen = true;
			}
		}
	})
	...
	$scope.$watch('inCity', function(val) {
		if(val) {
			...
			if(val.name.indexOf('省本级') == -1){
				$scope.isInBenShen = false;
			}else{
				$scope.isInBenShen = true;
			}
		}
	})
	//提交
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($scope.outCounty) && $scope.isOutShangHai == false && ($scope.outCity.name.indexOf('省本级') == -1)) {
				$scope.isAlert = true;
			...
			if(isBlank($scope.inCounty) && $scope.isInShangHai == false && ($scope.inCity.name.indexOf('省本级') == -1)) {
				$scope.isAlert = true;
		...
		$.ajax({
			...
			data: {
				...
				userName: encodeURI($scope.paramsInfo.userName),
				toArea: $scope.toArea,
			...
			success: function(res) {
				...
				if(res.isSuccess == true) {
					...
					$scope.isAlert = true;
					if(!isBlank(res.jsonFromResult)){
						$scope.msg = res.jsonFromResult.resultMessage;
					}else if(!isBlank(res.jsonToResult)){
						$scope.msg = res.jsonToResult.resultMessage;
					}
				}
			},
			error: function(err) {
