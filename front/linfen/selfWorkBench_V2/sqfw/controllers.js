...
app.controller("citizenController", function($scope, $route, $http, $location, $rootScope, data, $timeout, appFactory) {
	var name = data.itemName;
	$scope.showImg="../libs/common/images/dicon/"+acBridgeMac.vendor()+"/qrCode-self.gif";
	$scope.itemName = name;
	$scope.statusName = data.statusName;
	$scope.SisAlert = false;
	$scope.concel = "false";
		...
		if(code.indexOf("http") != -1) {
			...
			$.ajax({
				...
				success: function(dataJonsp) {
					$scope.loading = false;
					$.log.debug(code);
					if(dataJonsp != null && dataJonsp != undefined && dataJonsp.data.encrypted == true) {
						$scope.tipsText = "正在加载数据，请稍候...";
						$scope.getAccessToken(dataJonsp.data.biz_response.tokenSNO);
					} else {
						$scope.SisAlert = true;
						$scope.Smsg = "扫码失败请重试"
						$timeout(function() {
...
app.controller("infoFinishController", function($scope, $route, $http, $location, $rootScope, data, $timeout, appFactory) {
	...
	$scope.urladdress = urlHost + "/aci/selfWorkBench/sqfw/index.html#/qrCode?applyNo=" + $scope.applyNo + "&itemName=" + data.itemName + "&name=" + data.idCardName;
	//	}
	var name = data.itemName;
	$scope.itemName = name;
