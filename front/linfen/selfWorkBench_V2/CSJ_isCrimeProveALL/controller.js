...
app.controller('choiceMode', function($state, $scope, appData, $timeout, appFactory, $http) {
	$scope.funName = appData.funName = "有无违法犯罪记录证明开具";
...
app.controller("guideline", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		if(appData.type == "sh") {
			...
			if(appData.modeType == "print"){
				window.location.href = '../CSJ_isCrimeProve/index.html#/loginType?type='+appData.type+'&sign=1&modeType=print';
			}else{
				window.location.href = '../CSJ_isCrimeProve/index.html#/loginType?type='+appData.type+'&sign=1';
			}
		}
	}
