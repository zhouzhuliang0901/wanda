...
app.controller("guideline", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	$scope.funName = appData.funName = "再生子女档案查询";
...
//		$scope.getUserInfoByAccessToken();
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($scope.man)) {
				$scope.isAlert = true;
				$scope.msg = "请输入男方姓名";
				return
			}
			if(isBlank($scope.paramsInfo.man_borndate)) {
				$scope.isAlert = true;
				$scope.msg = "请选择男方出生日期";
				return
			}
			if(isBlank($scope.woman)) {
				$scope.isAlert = true;
				$scope.msg = "请输入女方姓名";
				return
			}
			if(isBlank($scope.paramsInfo.woman_borndate)) {
				$scope.isAlert = true;
				$scope.msg = "请选择女方出生日期";
				return
			}
		...
		let detailParams = {
			man: $scope.paramsInfo.man,
			woman: $scope.paramsInfo.woman,
			man_borndate: $scope.paramsInfo.man_borndate || "",
			woman_borndate: $scope.paramsInfo.woman_borndate || "",
		}
		...
		$.ajax({
			...
			data: {
				...
				Addressee: encodeURI(basicParams.Addressee || ''),
				Mailingaddress: encodeURI(basicParams.Mailingaddress || ''),
				yzbm: basicParams.yzbm || '',
				itemCode: '3120901600000'
			},
