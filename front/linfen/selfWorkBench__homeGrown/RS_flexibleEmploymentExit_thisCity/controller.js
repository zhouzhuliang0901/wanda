...
app.controller('login', function($scope, $http, $state, appData, appFactory) {
	...
	$scope.handleCondition = function() {
		...
		appFactory.pro_fetch("LD0015Q3", "ld", appData.applyNo, JSON.stringify($scope.paramStr), function(dataJson) {
...
app.controller('info', function($state, $scope, appData, $timeout, appFactory, $http) {
	...
	$scope.change = function(type, index) {
		if(type == "xbbttsbz") {
			$scope.xbbttsbz = index;
		} else if(type == "sbbttsbz") {
			$scope.sbbttsbz = index;
		} else if(type == "jybttsbz") {
			$scope.jybttsbz = index;
		}
	}
	...
	$timeout(function() {
		$('.form_datetime')
			...
			.on('changeDate', function(ev) {
				if(ev) {
					jyzzrq = $scope.jyzzrq = formatDateCustom(ev.date.valueOf());
					jyzzrq = (jyzzrq.replace(/-/g, ""));
					$scope.paramStr = {
						person_name: encodeURI(appData.licenseName),
					...
					appFactory.pro_fetch("LD0015J3", "ld", appData.applyNo, JSON.stringify($scope.paramStr), function(dataJson) {
						console.log(dataJson);
						$scope.isLoading = false;
						if(!isBlank(dataJson)) {
							$scope.head = dataJson.data.msg.head;
							...
							if(!isBlank($scope.head)) {
								if($scope.head.rst.buscode == "000000") {
									...
									$scope.jybttsbz = $scope.checkResult.jybttsbz;
									$scope.sbbttsbz = $scope.checkResult.sbbttsbz;
									$scope.xbbttsbz = $scope.checkResult.xbbttsbz;
								} else {
									$scope.isAlert = true;
	...
	$scope.nextStep = function() {
		...
		$scope.saveApplyInfo = function() {
			...
			$scope.saveParamStr = {
				person_name: encodeURI(appData.licenseName),
				cetf_id: appData.licenseNumber,
				jybttsbz: $scope.jybttsbz,
				sbbttsbz: $scope.sbbttsbz,
				xbbttsbz: $scope.xbbttsbz,
				pid: appData.hylbResult.pid.toString(),
				czygh: "",
				jyzzrq: jyzzrq,
			}
			console.log($scope.saveParamStr);
			appFactory.pro_fetch("LD0015S2", "ld", appData.applyNo, JSON.stringify($scope.saveParamStr), function(dataJson) {
				console.log(dataJson);
				if(!isBlank(dataJson)) {
					$scope.head = dataJson.data.msg.head;
					$scope.body = dataJson.data.msg.body;
