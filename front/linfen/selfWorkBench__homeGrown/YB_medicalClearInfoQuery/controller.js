...
app.controller('main', function($state, $scope, appData, $location) {
	...
	$scope.funName = newPerjsonStr.account_clear_info_query.stuffName;
	appData.type = newPerjsonStr.account_clear_info_query.type;
	appData.ywlx = newPerjsonStr.account_clear_info_query.ywlx;
...
app.controller('accountSettlement', function($scope, $state, appData, $sce, appFactory, $timeout) {
	...
	$scope.isShowPrint = false;
	//获取网点信息
	$scope.qutletsMsg=$.getConfigMsg.qutletsVal;
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		$state.go('main');
	}
	...
	$scope.print = function() {
		...
		//var LODOP = $.device.printGetLodop('');
		var Html = "<style>body{margin: 60px 0px 0px 40px;} table {font-size: 12px;border-collapse: collapse;border-bottom: 1px solid;border-right: 1px solid;text-align: center;width: 700px;}" +
			...
			"</tr>" +
			"</table>";
		var addxinxi='<p style="width: 680px;font-size: 12px;margin:30px 0px 0px 10px;"><span style="float: left;">渠道：“一网通办”自助终端</span>'+
		'<span style="margin-left: 40px;">网点:'+($scope.qutletsMsg || "临汾路街道社区事务受理服务中心")+'</span>'+
		'<span style="float: right;">查询时间:'+time+'</span></p>'+
		'<p style="margin:30px 0px 0px 20px;font-size: 12px;">*本查询仅做信息查询使用</p>';
		Html=Html+addxinxi;
...
//		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
//		LODOP.ADD_PRINT_TEXT(940, 230, 262, 20, "网点：" + ($scope.qutletsMsg || "临汾路街道社区事务受理服务中心"));
//		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
...
//		LODOP.PRINT();
		...
		recordUsingHistory('医保服务', '查询+打印', '帐户清算信息查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
		trackEventForQuery("帐户清算信息查询", "", "打印", "上海市医疗保障局", $scope.licenseName, appData.licenseNumber, "");
		$timeout(function() {
			$state.go("main");
		}, 3000);
	}
