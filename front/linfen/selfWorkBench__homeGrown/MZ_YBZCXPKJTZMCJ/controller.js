...
app.controller('info', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.hjProvinceList=changeListname($scope.hjProvinceList,'value');
	$scope.jzProvinceList = filterByName($rootScope.allList, "");
	$scope.jzProvinceList=changeListname($scope.jzProvinceList,"value");
	console.log($scope.hjProvinceList[0]);
	$scope.hjProvince = $scope.hjProvinceList[0];
	$scope.jzProvince = $scope.jzProvinceList[0];
	$scope.hjProvincecu=$scope.hjProvince.name;
	$scope.jzProvincecu=$scope.jzProvince.name;
...
app.controller('infoChoose', function($state, $scope, appData, $location, $rootScope, $timeout) {
	...
	$scope.countryList = changeListname($scope.countryList,"value");
	...
	$timeout(function() {
		$scope.$watch("county", function(val) {
			if(val) {
				...
				$scope.streetList = changeListname($scope.streetList,"value");
			}
		});
	...
	$timeout(function() {
		...
		$scope.$watch("Province", function(val) {
			...
			if(val) {
				...
				$scope.citycu="";
				$scope.cityList =changeListname($scope.cityList,"value");
				if(val.value == "上海市") {
					$scope.active = false;
				} else {
					$scope.active = true;
