...
app.controller("infoJybtController", function($scope, $route, $http, $location, data, $rootScope, $timeout, appFactory) {
	...
	$scope.next = function() {
		...
		$scope.nextload1 = function() {
			$scope.isLoading = false;
			appFactory.pro_fetch(data.idCardNum, data.idCardName,'就业补贴',data.VALIDSTARTDAYTRAT,data.VALIDENDDAY,'310105109000100', 'YJS00010003', 'YJS0001', '居民身份证', '1', function(dataJson) {
				if(!dataJson) {
					$scope.SisAlert = true;
					$scope.Smsg = "没有该证照!";
				}
