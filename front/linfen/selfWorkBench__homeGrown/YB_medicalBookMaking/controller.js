...
app.controller('bookMakingReason', function($scope, $http, $state, appData, $sce, appFactory) {
	...
	var OCXDomeID=$.clearCache.getOCXName();
	console.log(OCXDomeID);
	$scope.cont=0;
	...
	$scope.updateRecordBook = function() {
		...
		$http.jsonp($.getConfigMsg.preUrl + "/aci/workPlatform/medicalInsurance/updateRecordBook.do", {
			...
			if(returnData == undefined || returnData == "" || returnData == null){
				...
				recordUsingHistory('医保服务', '打印', '办理就医记录册的申领、更换、补发', $scope.name, $scope.Number, '', appData.imageData, JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery("办理就医记录册的申领、更换、补发", "", "打印", "上海市医疗保障局", $scope.name, $scope.Number, "");
                //清楚缓存
                $.clearCache.getUSB(OCXDomeID);
                $.clearCache.clear(OCXDomeID);
				$scope.confirm = function() {
					...
					setTimeout(function() {
						$state.go("main");
						$.clearCache.closeClear(OCXDomeID);
					}, 9000);
				};
				$scope.confirm();
