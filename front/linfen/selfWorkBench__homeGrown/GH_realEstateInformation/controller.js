app.controller('loginType', function($state, $scope, appData, $location) {
	...
	$scope.prevStep = function() {
		window.location.href = "../GH_allItem/index.html";
	}
});
	...
	$scope.nextStep = function(){
		...
		$.device.httpDownload( // 调用壳的方法下载服务器上的图片，再上传到另一接口中		urlHost +
			...
			function(result) {
				...
				recordUsingHistory('规划资源局服务', '查询+打印', appData.funName, appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery(appData.funName, '', "打印", "上海市规划和自然资源局", appData.licenseName, appData.licenseNumber, "");
			},
