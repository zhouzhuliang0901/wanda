...
// 原来的guidelineController控制器
app.controller("mainController", function($scope, $route, $http, $location, $sce, data, $timeout, $rootScope, appFactory) {
	data.itemName = "敬老卡申领";
...
app.controller("selectController", function($scope, $route, $http, $location, data, $rootScope, $timeout, appFactory) {
	...
	$scope.prev = function() {
		if(data.itemName == "灵活就业两项补贴") {
			...
			// 原来的路由跳转 $location.path("/guideline");
			$location.path("/main");
...
//		测试数据
//		data.idCardName = '陈雷';
//		data.idCardNum = '310228198808070818';
//		$scope.choice();
...
//	$scope.getIdcard();
	...
	//	data.idCardName = "陈雷" //"邹天奇"; //
	//	data.idCardNum = "310228198808070818";
	//	//  "310109194911262065";//"430426199804106174"; ////"310104197308010412"; //""; //"";//"320831199503150013";//"310105197805313613"; //
...
app.controller("infoController", function($scope, $route, $http, $location, data, $rootScope, $timeout, appFactory) {
	...
	$scope.next = function() {
		...
		$scope.nextload1 = function() {
			$scope.isLoading = false;
			appFactory.pro_fetch(data.idCardNum, data.idCardName, '敬老卡申领', data.VALIDSTARTDAYTRAT, data.VALIDENDDAY, '310105109000100', 'GA2011_010001', 'GA2011_01', '居民身份证', '0', function(dataJson) {
...
//	 根据屏幕宽高调整高拍仪位置
	if(window.innerWidth >= 1600) {
		...
		$.device.cmCaptureShow(510, 450, 70, 360);
	}
...
app.controller("infoFinishController", function($scope, $route, $http, $location, $rootScope, data, $timeout, appFactory) {
	...
	if(data.itemName == "灵活就业两项补贴") {
		...
		//行为分析(办理)
        trackEventForAffairs(data.applyNo, '敬老卡申领', "上海市公安局", data.idCardName, data.idCardNum, data.mobile);
	} else if(data.itemName == "生育保险") {
