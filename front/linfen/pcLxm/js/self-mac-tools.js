...
jQuery.device = {
	...
	GoHome: function() {
		try {
			...
			if(acBridgeMac.vendor() == "nhdevice"){
			...
			} else if(('index-shsdsx'==yhPublicItemdeviceName)||('csjindex'==yhPublicItemdeviceName)||('index-yh-yb'==yhPublicItemdeviceName)||('indexMainPageYH/index.html'==yhPublicItemdeviceName)){
				var test = window.location.href,midpiece='';
				midpiece="selfWorkBench_V2/index.html#/";
				if('indexMainPageYH/index.html'==yhPublicItemdeviceName)midpiece="selfWorkBench_V2/";
				...
				test = test.split('selfWorkBench_V2')[0] + "selfWorkBench_V2/index.html#/"+zhdeviceUrlSuffixName;
				window.location.href = test;
				...
				test = test.split('selfWorkBench_V2')[0] + "selfWorkBench_V2/index.html#/"+ghdeviceUrlSuffixNameList;
				window.location.href = test;
			} else{
	...
	Face_Show: function(width, height, x, y, callback) {
		try {
			...
			if(acBridgeMac.vendor() == 'wonders' && window.external.GetConfig('liveDetection') == 'N') {
				...
				$.nsdeviceSum.Camera_Show(400, 300, 300, 300, function(value) {
					if(value.Result == 0) {
						setTimeout(function() {
	...
	idCardOpen: function(callback) {
		try {
			...
			if(this.vendor() == 'jhdevice') {
				...
				$.deviceBocom.idCardOpen(function(value){callback(JSON.stringify(value))});
			} else if(acBridgeMac.vendor() == 'ghdevice') {
				opendev();
