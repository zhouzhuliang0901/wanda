app.controller('loginType', function($state, $scope, appData, $location, appFactory) {
	$.getConfigMsg.preUrlSelf = "http://10.2.104.102:8081/ac-self"
...
//				$scope.getUserInfoByAccessToken();
	...
	$scope.nextStep = function() {
		...
		//上传材料
		//		appFactory.upload_file($scope.licenseNumber, appData.licenseName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, '310105105000100', "CL00330002","户口簿", function(dataJson) {
		...
		try {
			appData.isUpload.push({
				...
				status: 1,
				method: "高拍仪",
				imgId: "1", //dataJson1.data.attachid
...
app.controller("applyInfo", function($scope, $state, appData, $sce, appFactory) {
	...
	$scope.nextStep = function() {
		...
		$scope.uploadFile1 = function() {
			//上传材料
			//			appFactory.upload_file($scope.licenseNumber, appData.licenseName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, '310105109000100', "CL00330001","中华人民共和国居民身份证", function(dataJson) {
			//				if(dataJson == "" || dataJson == null || dataJson == undefined) {
			...
			try {
				appData.isUpload.push({
					...
					status: 1,
					method: "高拍仪",
					imgId: "0", //dataJson1.data.attachid
		...
		$scope.uploadFile2 = function() {
			//上传材料
			//			appFactory.upload_file($scope.licenseNumber, appData.licenseName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, '310101360000100', "CL00330003","中华人民共和国残疾人证", function(dataJson) {
			//				if(dataJson == "" || dataJson == null || dataJson == undefined) {
			...
			try {
				appData.isUpload.push({
					...
					status: 1,
					method: "高拍仪",
					imgId: "2", //dataJson1.data.attachid
