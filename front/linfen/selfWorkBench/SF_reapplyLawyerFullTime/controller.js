app.controller('personChoice', function($scope, $state, appData, $sce, appFactory) {
	$scope.isAlert = false;
	appData.funName = "重新申请律师专职执业";
	appData.itemCode = "310100284000-08";
...
app.controller('info', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.practiceTypeList = practiceType;
	//个人信息
	...
	$scope.yzyjg = appData.yzyjg;
	$scope.nzyjg = appData.nzyjg;
	$scope.firstDate = appData.firstDate;
	$scope.firstCity = appData.firstCity
	...
	$scope.gwgj = appData.gwgj;
	$scope.bldCode = appData.bldCode;
	$scope.stPhone = appData.stPhone;
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($('#firstDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择首次执业日期！";
				return;
			}
			if(isBlank($scope.firstCity)) {
				$scope.isAlert = true;
				$scope.msg = "请输入首次执业城市！";
				return;
			}
			if(isBlank($scope.practiceType)) {
				$scope.isAlert = true;
				$scope.msg = "请选择执业类别！";
				return;
			}
		...
		appData.firstDate = $('#firstDate').val();
		// 获取某个时间格式的时间戳
		appData.firstDateUnix = Date.parse(new Date(appData.firstDate));
		appData.firstCity = $scope.firstCity;
		appData.practiceType = $scope.practiceType;
		...
		appData.stPhone = $scope.stPhone;
		appData.gwgj = $scope.gwgj;
		appData.yzyjg = $scope.yzyjg;
		//  拟职业机构   办理点
		appData.nzyjg = $scope.nzyjg;
...
app.controller('pickUpMethod', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		...
		$scope.params = {
			"data": {
				...
				"info": {
					...
					"applicant_firstPracticeDate": {
						"unix": appData.firstDateUnix,
						"value": appData.firstDate
					},
					"applicant_firstPracticeCity": appData.firstCity,
					"applicant_foreignLawyerQualification": appData.gwlszg,
					"applicant_foreignNationlity": appData.gwgj,
					"applicant_gender": {
						"name": appData.stSex,
					...
					"applicant_originalPracticeOrganization": appData.yzyjg,
					...
					"applicant_practiceType": {
						"name": appData.practiceType.name,
						"value": appData.practiceType.id
					},
		...
		$scope.saveApplyInfo = function() {
			$.ajax({
				type: "get",
				url: $.getConfigMsg.preUrlSelf + "/selfapi/lawyerPractice/reapplyLawyerPractice.do",
				dataType: "jsonp",
				jsonp: "jsonpCallback",
				data: {
