...
app.controller("infoSybxController", function($scope, $route, $http, $location, data, $rootScope, $timeout, appFactory) {
	...
	$scope.info = function() {
		...
		$scope.next1 = function() {
			//男方户口本
			var nConfig = {
				identNo: data.encrypt_identity || $scope.nIdCard, //data.idCardNum,
	...
	$scope.next = function() {
		...
		$scope.nextload6 = function() {
			appFactory.pro_fetch($scope.idCardNum, data.idCardName, '生育保险', data.VALIDSTARTDAYTRAT, data.VALIDENDDAY, '310105127000101', 'YJS00020008', 'YJS0002', '结婚证', '1', function(dataJson) {
				try {
					if(!dataJson) {
						$scope.SisAlert = true;
						$scope.Smsg = "没有该证照!";
		...
		$scope.nextload3 = function() {
			//判断是否有女方户籍证明
			appFactory.pro_fetch($scope.idCardNum, data.idCardName, '生育保险', data.VALIDSTARTDAYTRAT, data.VALIDENDDAY, '310105105000100', 'YJS00020002', 'YJS0002', '女方户籍证明', '1', function(dataJson) {
				try {
					if(!dataJson) {
						$scope.SisAlert = true;
						$scope.Smsg = "没有该证照!";
		...
		$scope.nextload1 = function() {
			...
			appFactory.pro_fetch($scope.idCardNum, data.idCardName, '生育保险', data.VALIDSTARTDAYTRAT, data.VALIDENDDAY, '310105109000100', 'YJS00020001', 'YJS0002', '女方居民身份证', '1', function(dataJson) {
				try {
					if(!dataJson) {
						$scope.SisAlert = true;
						$scope.Smsg = "没有该证照!";
