		...
		$http.jsonp($.getConfigMsg.preUrl + '/aci/workPlatform/util/encryptDataByRSA.do', {
			...
			switch(appData.type) {
				case "ylj":
					...
					recordUsingHistory('三金查询', '查询', '养老金查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
					//行为分析(查询)
					trackEventForQuery("养老金查询", "", "查询", "上海市人力资源社会保障局", appData.licenseName, appData.licenseNumber, "");
					break;
				case "gjj":
					...
					recordUsingHistory('三金查询', '查询', '公积金查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
					//行为分析(查询)
					trackEventForQuery("公积金查询", "", "查询", "上海市住房城乡建设管理委", appData.licenseName, appData.licenseNumber, "");
					break;
				case "ybj":
					...
					recordUsingHistory('三金查询', '查询', '医保金查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
					//行为分析(查询)
					trackEventForQuery("医保金查询", "", "查询", "上海市医保局", appData.licenseName, appData.licenseNumber, "");
					reak;
				case "jjxx":
...
app.controller("print", function($scope, $state, $timeout, appData, $sce, $location) {
	...
	$scope.print = function() {
		...
		function print1() {
			...
			LODOP.ADD_PRINT_TEXT(100, 55, 188, 20, "渠道：“一网通办”自助终端");
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(100, 312, 262, 20, "网点："+($.config.get('Outlets')||"临汾路街道社区事务受理服务中心"));
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		...
		$timeout(function() {
			...
			recordUsingHistory('三金查询', '打印', '公积金查询', $scope.licenseName, $scope.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
			//行为分析(查询)
			trackEventForQuery("公积金查询", "", "打印", "上海市住房城乡建设管理委", appData.licenseName, appData.licenseNumber, "");
			$state.go("main");
		}, 3000);
	}
	$scope.alertConfirm = function() {
