...
app.controller("info", function($scope, $state, appData, $sce, appFactory, $timeout) {
	...
	$scope.getPensionInfo = function() {
		...
		recordUsingHistory('人社服务', '查询', '养老金查询', $scope.licenseName, $scope.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
		//行为分析(查询)
	...
	$scope.queryMedicalDetails = function() {
		...
		recordUsingHistory('医保服务', '查询', '医保金查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
		trackEventForQuery('医保金查询', '', '查询', '上海市医疗保障局', appData.licenseName, appData.licenseNumber, '');
	}
...
app.controller("print", function($scope, $state, $timeout, appData, $sce, $location) {
	...
	//打印
	$scope.print = function() {
		if(isBlank($scope.basicInfo)) {
			$scope.isAlert = true;
			$scope.msg = "未查询到信息不能打印"
			$scope.alertConfirm = function() {
				$state.go("info");
			}
		}
