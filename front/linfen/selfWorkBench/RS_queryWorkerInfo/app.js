...
app.factory("appFactory", function($http, $rootScope) {
	...
	var getRSDictionary = function(proId, cityId, streetId,callback) {
		$http.jsonp($.getConfigMsg.preUrlSelf+"/selfapi/pensionAdjustment/RSDictionary.do", {
			params: {
				proId: proId,
				cityId: cityId,
				streetId: streetId,
	...
	let getList = function() {
		getRSDictionary("", "", "", function(dataJson) {
			$rootScope.provinceList = dataJson;
		});
	}
	getList();
	return {
		pro_fetch: product,
		getDictionary: getRSDictionary
	}
})