	...
	$scope.Upload = function() {
		if(appData.handleTypeId == "01") {
			$scope.Upload1 = function() {
				appFactory.pro_fetch($scope.licenseNumber, appData.licenseName, appData.funName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, '310100488000100', 'GA00300004', 'GA0030', '住所证明', '1', function(dataJson) {
		...
		appFactory.pro_fetch($scope.licenseNumber, appData.licenseName, appData.funName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, '310105109000100', 'YTGA003001', 'GA0030', '有效身份证明', '1', function(dataJson) {
			if(dataJson == "" || dataJson == null || dataJson == undefined) {
				layer.msg("未能从电子证照获取到身份证照上传");
				$timeout(function() {
					$state.go("materialList");
