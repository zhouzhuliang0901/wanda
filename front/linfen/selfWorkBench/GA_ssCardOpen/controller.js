...
app.controller("submit", function($scope, $state, appData, $sce, appFactory) {
	...
	$scope.nextStep = function() {
		if(!$scope.count) {
			layer.msg("请填写银行卡号后4位");
			return;
		}
		if(!$scope.isContinue) {
			layer.msg("对不起，您无法办理社保卡开通服务")
			return;
		}
		if(!$scope.isOpen) {
			layer.msg("对不起，您无法激活社保卡")
			return;
		}
		$scope.isLoading = true;
		$.ajax({
			...
			data: {
				...
				zxyy: "", // 注销原因
				kplzzt: appData.zt,
				dbr_xm: appData.fatherName || "",
				dbr_lxdh: appData.dbr_lxdh || "",
				dbr_zjlx: "01",
				dbr_zjhm: appData.fatherNumber || "",
				bz: "",
			},
			success: function(dataJson) {
				$scope.isLoading = false;
		...
		recordUsingHistory('公安服务', '办理', "新版社保卡开通服务", appData.sonlicenseName || appData.licenseName, appData.sonlicenseNumber || appData.licenseNumber, '', "", JSON.stringify($scope.jsonStr));
		//行为分析(办理)
		trackEventForAffairs("", "新版社保卡开通服务", "上海市公安局", appData.sonlicenseName || appData.licenseName, appData.sonlicenseNumber || appData.licenseNumber, "");
	}
