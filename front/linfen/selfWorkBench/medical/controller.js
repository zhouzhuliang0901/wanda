app.controller("main", function($scope, $state, appData, $sce) {
	...
	$scope.choiceType = function(type, name, ywlx) {
		...
		} else if(appData.type == "services") {
			$state.go("servicesList");
		} else if(appData.type == "medicalInstitution") {
			$state.go("yljgChoose");
		} else if(appData.ywlx == "") {
...
app.controller('loginType', function($state, $scope, appData, $location) {
	...
	if(appData.type == "" || appData.type == undefined) {
		...
		if(appData.type == "info") {
			$scope.funName = appData.funName = "医保个人信息";
		}
	}
...
app.controller('loginType1', function($state, $scope, appData, $location) {
	...
	if(appData.type == "" || appData.type == undefined) {
		appData.type = $location.search().type;
		$scope.funName = appData.funName = "综合减负试算";
	}
...
app.controller('login', function($scope, $http, $state, appData, appFactory, $rootScope) {
	$scope.funName = appData.funName;
	$scope.isAlert = false;
	$scope.msg = "";
...
app.controller('swipeAgentIdcard', function($scope, $http, $state, appData, appFactory) {
	...
	$scope.citizenLogin = function(info) {
		if(info) {
			if(appData.qrCodeType == "suishenma") {
				appData.agentName = info.zwdtsw_name;
				appData.agentNumber = info.zwdtsw_cert_id;
				$scope.searchInsuredInfo();
			} else {
				var idcardInfo = info.result.data;
	...
	$scope.updateRecordBook = function() {
		$http.jsonp($.getConfigMsg.preUrl + "/aci/workPlatform/medicalInsurance/updateRecordBook.do", {
			...
			if(!returnData[0].syscode) {
				...
				recordUsingHistory('医保服务', '打印', '办理就医记录册的申领、更换、补发', $scope.name, $scope.Number, '', '', JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery("办理就医记录册的申领、更换、补发", "", "打印", "上海市医疗保障局", $scope.name, $scope.Number, "");
				//清除医保记录册缓存
				try {
					if(DSPrinterLibOCX) {
						GetUSB();
...
app.controller("info", function($scope, $state, appData, $sce, appFactory) {
		...
		$.ajax({
			...
			success: function(dataJson) {
				...
				if(dataJson.length > 0) {
					...
					appData.tableInfo = "<style>table{font-size:12px;border-collapse: collapse; width:680px} table td{border:1px solid;height:18px;} table th{text-align:right;border:1px solid;height:18px;width:40%;}</style><table>" +
						...
						"<tr><th>门急诊当年账户支付累计：</th><td>" + toDecimal2(info.mjz_dnzh_lj) + "</td><th>门诊大病统筹段历年账户支付累计：</th><td>" + toDecimal2(info.db_tcdlnzh_lj) + "</td></tr>" +
						"<tr><th>门急诊历年账户支付累计：</th><td>" + toDecimal2(info.mjz_lnzh_lj) + "</td><th>门诊大病统筹段现金支付累计：</th><td>" + toDecimal2(info.db_tcdxj_lj) + "</td></tr>" +
						"<tr><th>门急诊及药店购药自负段累计：</th><td>" + toDecimal2(info.mjz_zfdxj_lj) + "</td><th>门诊大病附加段历年账户支付累计：</th><td>" + toDecimal2(info.db_fjdlnzh_lj) + "</td></tr>" +
						"<tr><th>门急诊自负段历年账户支付累计：</th><td>" + toDecimal2(info.mjz_zfdlnzh_lj) + "</td><th>门诊大病统筹支付累计：</th><td>" + toDecimal2(info.db_tc_lj) + "</td></tr>" +
						"<tr><th>门急诊附加段历年账户支付累计：</th><td>" + toDecimal2(info.mjz_fjdlnzh_lj) + "</td><th>门诊大病附加段现金支付累计：</th><td>" + toDecimal2(info.db_fjdxj_lj) + "</td></tr>" +
						"<tr><th>门急诊附加段现金累计：</th><td>" + toDecimal2(info.mjz_fjdxj_lj) + "</td><th>门诊大病附加支付累计：</th><td>" + toDecimal2(info.db_fj_lj) + "</td></tr>" +
						"<tr><th>门急诊统筹支付累计：</th><td>" + toDecimal2(info.mjz_tc_lj) + "</td><th>门诊大病分类自负累计：</th><td>" + toDecimal2(info.db_flzf_lj) + "</td></tr>" +
						"<tr><th>门急诊分类自负段累计：</th><td>" + toDecimal2(info.mjz_flzf_lj) + "</td><th></th><td></td></tr>" +
						"<tr><th>门急诊附加支付累计：</th><td>" + toDecimal2(info.mjz_fj_lj) + "</td><th>家床起付段历年账户支付累计：</th><td>" + toDecimal2(info.jc_qfdlnzh_lj) + "</td></tr>" +
						"<tr><th>住院起付段历年账户支付累计：</th><td>" + toDecimal2(info.zy_qfdlnzh_lj) + "</td><th>家床起付段现金支付累计：</th><td>" + toDecimal2(info.jc_qfdxj_lj) + "</td></tr>" +
						"<tr><th>住院起付段现金支付累计：</th><td>" + toDecimal2(info.zy_qfdxj_lj) + "</td><th>家床统筹段历年账户支付累计：</th><td>" + toDecimal2(info.jc_tcdlnzh_lj) + "</td></tr>" +
						"<tr><th>住院统筹段历年账户支付累计：</th><td>" + toDecimal2(info.zy_tcdlnzh_lj) + "</td><th>家床统筹支付累计：</th><td>" + toDecimal2(info.jc_tc_lj) + "</td></tr>" +
						"<tr><th>住院统筹段现金支付累计：</th><td>" + toDecimal2(info.zy_tcdxj_lj) + "</td><th>家床附加段历年账户支付累计：</th><td>" + toDecimal2(info.jc_fjdlnzh_lj) + "</td></tr>" +
						"<tr><th>住院统筹支付累计：</th><td>" + toDecimal2(info.zy_tc_lj) + "</td><th>家床附加段现金支付累计：</th><td>" + toDecimal2(info.jc_fjdxj_lj) + "</td></tr>" +
						"<tr><th>住院附加段历年账户支付累计：</th><td>" + toDecimal2(info.zy_fjdlnzh_lj) + "</td><th>家床附加支付累计：</th><td>" + toDecimal2(info.jc_fj_lj) + "</td></tr>" +
						"<tr><th>住院附加段现金支付累计：</th><td>" + toDecimal2(info.zy_fjdxj_lj) + "</td><th>家床分类自负累计：</th><td>" + toDecimal2(info.jc_flzf_lj) + "</td></tr>" +
						"<tr><th>住院附加支付累计：</th><td>" + toDecimal2(info.zy_fj_lj) + "</td><th></th><td></td></tr>" +
						"<tr><th>住院分类自负累计：</th><td>" + toDecimal2(info.zy_flzf_lj) + "</td><th>高价药门急诊当年账户累计：</th><td>" + toDecimal2(info.gjy_mjz_dnzh_lj) + "</td></tr>" +
						"<tr><th>住院天数：</th><td>" + toDecimal2(info.zyts) + "</td><th>高价药门急诊历年账户累计：</th><td>" + toDecimal2(info.gjy_mjz_lnzh_lj) + "</td></tr>" +
						"<tr><th></th><td></td><th>高价药门急诊自负段现金支付累计：</th><td>" + toDecimal2(info.gjy_mjz_zfdxj_lj) + "</td></tr>" +
						"<tr><th>急观起付段历年账户支付累计：</th><td>" + toDecimal2(info.jg_qfdlnzh_lj) + "</td><th>高价药门急诊附加段现金累计：</th><td>" + toDecimal2(info.gjy_mjz_fjdxj_lj) + "</td></tr>" +
						"<tr><th>急观起付段现金支付累计：</th><td>" + toDecimal2(info.jg_qfdxj_lj) + "</td><th>高价药门急诊附加累计：</th><td>" + toDecimal2(info.gjy_mjz_fj_lj) + "</td></tr>" +
						"<tr><th>急观统筹段历年账户支付累计：</th><td>" + toDecimal2(info.jg_tcdlnzh_lj) + "</td><th>高价药门急诊统筹段现金累计：</th><td>" + toDecimal2(info.gjy_mjz_tcdxj_lj) + "</td></tr>" +
						"<tr><th>急观统筹段现金支付累计：</th><td>" + toDecimal2(info.jg_tcdxj_lj) + "</td><th>高价药门急诊统筹累计：</th><td>" + toDecimal2(info.gjy_mjz_tc_lj) + "</td></tr>" +
						"<tr><th>急观统筹支付累计：</th><td>" + toDecimal2(info.jg_tc_lj) + "</td><th>高价药门急诊自负段历年账户累计：</th><td>" + toDecimal2(info.gjy_mjz_zfdlnzh_lj) + "</td></tr>" +
						"<tr><th>急观附加段历年账户支付累计：</th><td>" + toDecimal2(info.jg_fjdlnzh_lj) + "</td><th>高价药门急诊附加段历年账户累计：</th><td>" + toDecimal2(info.gjy_mjz_fjdlnzh_lj) + "</td></tr>" +
						"<tr><th>急观附加段现金支付累计：</th><td>" + toDecimal2(info.jg_fjdxj_lj) + "</td><th></th><td></td></tr>" +
						"<tr><th>急观附加支付累计：</th><td>" + toDecimal2(info.jg_fj_lj) + "</td><th>高价药住院起付段历年账户支付累计：</th><td>" + toDecimal2(info.gjy_zy_qfdlnzh_lj) + "</td></tr>" +
						"<tr><th>急观分类自负累计：</th><td>" + toDecimal2(info.jg_flzf_lj) + "</td><th>高价药住院起付段现金支付累计：</th><td>" + toDecimal2(info.gjy_zy_qfdxj_lj) + "</td></tr>" +
						"<tr><th></th><td></td><th>高价药住院统筹段历年账户支付累计：</th><td>" + toDecimal2(info.gjy_zy_tcdlnzh_lj) + "</td></tr>" +
						"<tr><th>药店购药当年账户支付累计：</th><td>" + toDecimal2(info.yd_dnzh_lj) + "</td><th>高价药住院统筹段现金支付累计：</th><td>" + toDecimal2(info.gjy_zy_tcdxj_lj) + "</td></tr>" +
						"<tr><th>药店购药历年账户支付累计：</th><td>" + toDecimal2(info.yd_lnzh_lj) + "</td><th>高价药住院统筹累计：</th><td>" + toDecimal2(info.gjy_zy_tc_lj) + "</td></tr>" +
						"<tr><th>药店购药分类自负累计：</th><td>" + toDecimal2(info.gy_flzf_lj) + "</td><th>高价药住院附加段历年账户支付累计：</th><td>" + toDecimal2(info.gjy_zy_fjdlnzh_lj) + "</td></tr>" +
						"<tr><th>药店购药自负段现金支付累计：</th><td>" + toDecimal2(info.gy_zfdxj_lj) + "</td><th>高价药住院附加段现金支付累计：</th><td>" + toDecimal2(info.gjy_zy_fjdxj_lj) + "</td></tr>" +
						"<tr><th></th><td></td><th>高价药住院附加累计：</th><td>" + toDecimal2(info.gjy_zy_fj_lj) + "</td></tr>" +
						"<tr><th>长护险统筹段现金累计：</th><td>" + toDecimal2(info.chx_tcdxj_lj) + "</td><th></th><td></td></tr>" +
						"<tr><th>长护险统筹累计：</th><td>" + toDecimal2(info.chx_tc_lj) + "</td><th>商业保险历年账户支付金额：</th><td>" + toDecimal2(info.sybx_qnzh_zf) + "</td></tr>" +
						"<tr><th>长护险次数：</th><td>" + toDecimal2(info.chxcs) + "</td><th></th><td></td></tr>" +
						"</table>";
				}
			},
			error: function(err) {
	...
	$scope.print = function() {
		...
		setTimeout(function() {
			...
			LODOP.ADD_PRINT_TEXT(22, 250, 430, 50, "个人"+(appData.infoyear==tYear?"":"历史")+"账户信息查询表(" + appData.infoyear + ")");
			LODOP.SET_PRINT_STYLE("BOLD", 1);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 20);
			//		LODOP.ADD_PRINT_TEXT(83, 43, 154, 20, "卡号：" + $scope.zhh);
			//		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(73, 43, 115, 20, "医保办法：" + $scope.ybbf);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(73, 207, 142, 20, "姓名：" + $scope.xm);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(73, 371, 226, 20, "身份证号：" + appData.licenseNumber);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(73, 598, 111, 20, "人员类别：" + $scope.rylb);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(95, 43, 154, 20, "卡状态：" + $scope.zhzt);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(95, 207, 142, 20, "账户状态：" + $scope.fcqk);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(95, 371, 221, 20, "保健对象：" + $scope.bjdx);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(95, 597, 154, 20, "特殊人员：" + $scope.tsry);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(115, 43, 154, 20, "封存情况：" + $scope.fcqk);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
//			LODOP.ADD_PRINT_TEXT(115, 207, 162, 20, "公务员情况：" + $scope.gwyqk);
//			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(115, 371, 221, 20, "职退情况：" + $scope.ztqk);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(115, 597, 154, 20, "单位码：" + appData.dwbm);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(135, 42, 427, 20, "单位名称：" + appData.dwmc);
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(965, 43, 200, 22, "区县：" + ($.config.get('districtAndCounty') || "静安区医保中心"));
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(965, 260, 270, 22, "渠道：“一网通办”智能终端");
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(965, 530, 309, 22, "打印时间：" + date.getFullYear() + "年" + month + "月" + date.getDate() + "日");
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(990, 43, 427, 20, "网点：" + ($.config.get('Outlets') || "临汾街道社区事务受理中心"));
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TEXT(1020, 55, 200, 20, "*本查询仅做信息查询使用");
			LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
			LODOP.ADD_PRINT_TABLE(160, 43, 1300, 1400, appData.tableInfo);
			LODOP.PRINT();
		}, 100);
		//模块使用记录
		$scope.jsonStr = {
			...
			success: function(dataJson) {
				...
				if(dataJson) {
					...
					if(dataJson[0].errmsg) {
						...
						$scope.msg = "您的减负金额为" + dataJson[0].jfje + "元";
				...
				recordUsingHistory('医保服务', '查询', '医保综合减负试算', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
				//行为分析(查询)
				trackEventForQuery("医保综合减负试算", "", "查询", "上海市医疗保障局", $scope.licenseName, appData.licenseNumber, "");
			},
			error: function(err) {
				$scope.isLoding = true;
				$scope.isAlert = true;
...
app.controller('medicalDetails', function($scope, $state, appData, $sce, appFactory) {
	...
	//行为分析(查询)
	trackEventForQuery("医保就医明细", "", "查询", "上海市医疗保障局", $scope.licenseName, appData.licenseNumber, "");
	$scope.isScroll = function() {
...
app.controller('accountSettlement', function($scope, $state, appData, $sce, appFactory, $timeout) {
			...
			success: function(dataJson) {
				...
				if(dataJson.buscode == "FFF200") {
					...
					try {
						...
						if(($scope.year == 2021) && ($scope.months > 6)) {
							$scope.ifSeven = false;
						} else {
							$scope.ifSeven = true;
						}
	...
	$scope.print = function() {
		...
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		LODOP.ADD_PRINT_TEXT(940, 230, 262, 20, "网点：" + ($.config.get('Outlets') || "临汾路街道社区事务受理服务中心"));
		LODOP.SET_PRINT_STYLEA(0, "FontSize", 10);
		...
		//行为分析(查询)
		trackEventForQuery("账户清算信息", "", "打印", "上海市医疗保障局", $scope.licenseName, appData.licenseNumber, "");
		$timeout(function() {
			$state.go("main");
		}, 3000);
	}
...
app.controller('treatmentDetails', function($scope, $state, appData, $sce, appFactory) {
	...
	//模块使用记录
	$scope.usingHistory = function() {
		recordUsingHistory('医保服务', '查询', '参保人员待遇', '', '', '', '', '');
	}
	$scope.usingHistory();
	//行为分析(查询)
	trackEventForQuery("参保人员待遇", "", "查询", "上海市医疗保障局", "", "", "");
	$scope.prevStep = function() {
		$state.go("reduceChoose");
	}
	$scope.isScroll = function() {
...
//医疗服务机构
app.controller("servicesList", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.stuffName = servicesList;
	$scope.lastMenu = appData.lastMenu || false;
	appData.lastMenu = "";
	$scope.modelName = [{
			"stuffName": "长护险护理机构查询",
			"type": "chxhl",
	...
	$scope.choiceType = function(type, name, ywlx) {
		...
		if(type == "cqhlbxhl") {
			...
			$state.go(appData.type + "Choose");
		}
	}

	$scope.choiceType2 = function(type, name) {
		appData.funName = name;
		trackEvent(name)
		appData.type = type;
		$state.go(appData.type + "Choose");
	...
	$scope.isScroll();
	$scope.prevStep = function() {
		if(appData.type == "chxpg" || appData.type == "chxhl") {
...
//医保诊疗项目约定服务医院查询条件
app.controller("ybzlydfwChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.project = "";
	appData.area = "";
	...
	$scope.nextStep = function() {
		//		var condFlag = false;
		//		do {
		//			if(appData.project == "" || appData.project == null || appData.project ==undefined){
		//				$scope.isAlert = true;
		//				$scope.msg = "请选择医保约定项目！";
		//				return;
		//			}
		...
		//			}
		//		} while (condFlag);
		appData.institutionName = $scope.institutionName;
		$state.go("ybzlydfwQueryList");
	}
});
//医保诊疗项目约定服务医院查询结果
app.controller("ybzlydfwQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	...
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		$state.go("ybzlydfwChoose");
	}
	$scope.queryDiagnosisAndTreatmentProject = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryDiagnosisAndTreatmentProject.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				project: appData.project || "", //医保约定项目
				area: appData.area || "", //所属区
				institutionName: encodeURI(appData.institutionName || ""), //机构名称
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].ybzlydfwyys) {
					$scope.queryList = dataJson[0].ybzlydfwyys[0].ybzlydfwyy;
	...
	$scope.queryDiagnosisAndTreatmentProject();
	$scope.isScroll = function() {
		new iScroll("wrapper", {
			vScrollbar: true,
			hScrollbar: false,
...
//长护险护理机构查询条件
app.controller("chxhlChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.serviceMode = "";
	appData.area = "";
	...
	$scope.areaList = areaList2;
	$scope.change = function(name, index, id) {
		$scope.current = null;
		$scope.current = index;
		appData.serviceMode = id;
	};
	$scope.change2 = function(name, index, id) {
		$scope.current2 = null;
		$scope.current2 = index;
	...
	$scope.isScroll();
	$scope.prevStep = function() {
		appData.lastMenu = true;
		$state.go("servicesList");
	}
	$scope.nextStep = function() {
		var condFlag = false;
		do {
			if(appData.serviceMode == "" || appData.serviceMode == null || appData.serviceMode == undefined) {
				$scope.isAlert = true;
				$scope.msg = "请选择服务形式！";
				return;
			}
		...
		appData.institutionName = $scope.institutionName;
		appData.address = appData.address;
		$state.go("chxQueryList");
	}
});

//长护险评估机构查询条件
app.controller("chxpgChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.area = "";
	$scope.concel = "false";
...
//长期护理保险护理机构查询结果
app.controller("chxQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		if(appData.type == "chxhl") {
			$state.go("chxhlChoose");
		} else if(appData.type == "chxpg") {
			$state.go("chxpgChoose");
		}
	}
	//长护险护理
	$scope.queryNursingInstitutions = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryNursingInstitutions.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				serviceMode: appData.serviceMode, //服务形式
				area: appData.area, //所属区
				institutionName: encodeURI(appData.institutionName || ""), //机构名称
				address: encodeURI(appData.address || ""), //地址
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].chxpgjgxxs) {
					$scope.queryList = dataJson[0].chxpgjgxxs[0].chxpgjgxx;
					$scope.listLength = $scope.queryList.length;
				} else {
					$scope.isAlert = true;
					$scope.msg = "无查询结果";
	...
	//长护险评估
	$scope.queryAssessmentInstitutions = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryAssessmentInstitutions.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				area: appData.area, //所属区
	...
	if(appData.type == "chxhl") {
		$scope.queryNursingInstitutions();
	} else if(appData.type == "chxpg") {
		$scope.queryAssessmentInstitutions();
	}
	$scope.isScroll = function() {
		new iScroll("wrapper", {
			vScrollbar: true,
...
//商业保险机构网点信息查询查询条件
app.controller("sybxjgwdChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.area = "";
	appData.commercialInstitution = "";
	$scope.areaList = areaList;
	$scope.organList = [{
		"organName": "中国人寿保险",
		"organCode": "A",
	...
	$scope.change = function(name, index, id) {
		$scope.current = null;
		$scope.current = index;
		appData.commercialInstitution = id;
	};
	$scope.isScroll = function() {
	...
	$scope.nextStep = function() {
		$state.go("sybxjgwdQueryList");
	}
});
//商业保险机构网点信息查询查询结果
app.controller("sybxjgwdQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		$state.go("sybxjgwdChoose");
	}
	$scope.queryCommercialInsurance = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryCommercialInsurance.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				area: appData.area || "", //所属区
				commercialInstitution: encodeURI(appData.commercialInstitution || ""), //机构名称
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].sybxjgwdxxs) {
					$scope.queryList = dataJson[0].sybxjgwdxxs[0].sybxjgwdxx;
					$scope.listLength = $scope.queryList.length;
				} else {
					$scope.isAlert = true;
					$scope.msg = "无查询结果";
	...
	$scope.queryCommercialInsurance();
	$scope.isScroll = function() {
		new iScroll("wrapper", {
			vScrollbar: true,
			hScrollbar: false,
...
//本市开展异地就医的医院查询条件
app.controller("bskzydjyChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.settlementLevel = "";
	...
	$scope.reduceLevel = reduceLevel;
	$scope.hospitalList = hospitalList;
	$scope.change2 = function(name, index, id) {
		$scope.current2 = null;
		$scope.current2 = index;
		appData.settlementLevel = id;
	};
	$scope.change = function(name, index, id) {
		$scope.current = null;
		$scope.current = index;
		appData.classification = id;
	};
	$scope.isScroll = function() {
	...
	$scope.nextStep = function() {
		var condFlag = false;
		do {
			if(appData.classification == "" || appData.classification == null || appData.classification == undefined) {
				$scope.isAlert = true;
				$scope.msg = "请选择医院分类！";
				return;
			}
		} while (condFlag);
		appData.hospitalName = $scope.hospitalName;
		appData.address = $scope.address;
		$state.go("bskzydjyQueryList");
	}
});
//本市开展异地就医的医院查询查询结果
app.controller("bskzydjyQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		$state.go("bskzydjyChoose");
	}
	$scope.queryHospitalForSeekMedicalInDifferentPlaces = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryHospitalForSeekMedicalInDifferentPlaces.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				classification: appData.classification || "", //医院分类
				settlementLevel: appData.settlementLevel || "", //医院结算等级
				hospitalName: encodeURI(appData.hospitalName || ""), //医院名称
				address: encodeURI(appData.address || ""), //地址
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].ydyljgxxs) {
					$scope.queryList = dataJson[0].ydyljgxxs[0].ydyljgxx;
					$scope.listLength = $scope.queryList.length;
				} else {
					$scope.isAlert = true;
					$scope.msg = "无查询结果";
	...
	$scope.queryHospitalForSeekMedicalInDifferentPlaces();
	$scope.isScroll = function() {
		new iScroll("wrapper", {
			vScrollbar: true,
			hScrollbar: false,
...
//医保及相关服务机构查询查询条件
app.controller("ybxgfwChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	appData.serviceInstitution = "";
	appData.area = "";
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.alertConfirm = function() {
	...
	$scope.nextStep = function() {
		var condFlag = false;
		do {
			if(appData.serviceInstitution == "" || appData.serviceInstitution == null || appData.serviceInstitution == undefined) {
				$scope.isAlert = true;
				$scope.msg = "请选择服务机构！";
				return;
			}
		...
		appData.institutionName = $scope.institutionName;
		appData.address = $scope.address;
		$state.go("ybxgfwQueryList");
	}
});
//医保及相关服务机构查询查询结果
app.controller("ybxgfwQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
		$state.go("ybxgfwChoose");
	}
	$scope.queryServiceInstitution = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/serviceInstitution/queryServiceInstitution.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				serviceInstitution: appData.serviceInstitution, //医院分类
				area: appData.area || "", //医院结算等级
				institutionName: encodeURI(appData.institutionName || ""), //医院名称
				address: encodeURI(appData.address || ""), //地址
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].fwjgs) {
					$scope.queryList = dataJson[0].fwjgs[0].fwjg;
					$scope.listLength = $scope.queryList.length;
				} else {
					$scope.isAlert = true;
					$scope.msg = "无查询结果";
...
//医疗机构查询条件
app.controller("yljgChoose", function($scope, $state, appData, $sce) {
	$scope.operation = "请选择查询内容";
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	appData.hospitalClass = "";
	appData.area = "";
	$scope.alertConfirm = function() {
		$scope.isAlert = false;
	}
	$scope.hospitalClassList = [{
		"name": "一级医院",
		"id": "1",
	}, {
		"name": "二级医院",
	...
	$scope.nextStep = function() {
		var condFlag = false;
		do {
			if(appData.hospitalClass == "" || appData.hospitalClass == null || appData.hospitalClass == undefined) {
				$scope.isAlert = true;
				$scope.msg = "请选择医院等级！";
				return;
			}
		...
		appData.hospitalName = $scope.hospitalName;
		$state.go("yljgQueryList");
	}
});
////医疗机构查询查询结果
app.controller("yljgQueryList", function($scope, $state, appData, $sce) {
	$scope.funName = appData.funName;
	$scope.concel = "false";
	$scope.isAlert = false;
	$scope.isLoading = true;
	...
	$scope.queryDesignatedHospital = function() {
		$.ajax({
			type: "get",
			url: $.getConfigMsg.preUrlSelf + "/selfapi/MedicalInstitution/queryDesignatedHospital.do",
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			data: {
				hospitalClass: appData.hospitalClass, //医院等级
				area: appData.area, //所属区
				hospitalName: encodeURI(appData.hospitalName||""), //医院名称
			},
			success: function(dataJson) {
				$scope.isLoading = false;
				if(dataJson[0].ddyys) {
					$scope.queryList = dataJson[0].ddyys[0].ddyy;
					$scope.listLength = $scope.queryList.length;
				} else {
					$scope.isAlert = true;
					$scope.msg = "无查询结果";
