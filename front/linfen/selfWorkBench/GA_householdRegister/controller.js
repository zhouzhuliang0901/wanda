...
//信息填写
app.controller('info', function($state, $scope, appData, $http, $interval, $timeout, appFactory) {
	$scope.funName = appData.funName;
	...
	$scope.nextStep = function() {
		...
		$.ajax({
			...
			jsonp: "jsonpCallback",
			data: {
				appName: "73d33360-2a1a-4314-80d1-c64d9d2b4be5",
			...
			success: function(dataJson) {
				if(dataJson.success == true && dataJson.data.isSuccess == true) {
					...
					$scope.isAlert = true;
					$scope.msg = dataJson.msg || dataJson.data.msg;
					return;
				}
			},
			error: function(err) {
