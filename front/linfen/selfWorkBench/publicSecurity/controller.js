app.controller("main", function($scope, $state, appData, $sce,$timeout) {
	...
	$timeout(function() {
		if($.getConfigMsg.isCommunity == 'Y' || $.getConfigMsg.isCommunity == null || $.getConfigMsg.isCommunity == undefined) {
			$('.linkt:nth-of-type(1) .stuff').css("line-height", "130px");
			$('.linkt:nth-of-type(2) .stuff').css("line-height", "130px");
			$('.linkt:nth-of-type(3) .stuff').css("line-height", "130px");
		} else {
			$('.linkt:nth-of-type(1) .stuff').css("line-height","58px");
			$('.linkt:nth-of-type(2) .stuff').css("line-height","58px");
			$('.linkt:nth-of-type(3) .stuff').css("line-height","58px");
		}
	});
	$scope.choiceType = function(type, name, ywlx, url) {
		appData.funName = name;
		appData.type = type;
		if(type == "exitAndEntry" || type == "isCrime") {
			$state.go("guideline");
	...
	$scope.toPrint = function(type, name, ywlx) {
		...
		if(type == "exitAndEntry" || type == "isCrime") {
			...
			window.location.href = type + "#/loginType?type=print";
		}
	}
...
app.controller('exitAndEntryChoose', function($state, $scope, appData, $http, $interval, $timeout) {
	...
	if(appData.zwdtsw_user_id != undefined && appData.zwdtsw_user_id != null && appData.zwdtsw_user_id != "") {} else {
...
app.controller("updateResidence", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		...
		//行为分析(查询)
		trackEventForQuery("居住证有效期更新", "", "打印", "上海市公安局", $scope.stName, $scope.stIdCard, "");
		$scope.isAlert = true;
		$scope.msg = "<p>正在更新卡面，请等待…</p><p>（大约需要20秒）</p>";
		$timeout(function() {
			window.external.DataCard_Print(135, 85, 300, 400, "<style>td{padding:0px;text-align: justify; -ms-text-justify: inter-ideograph; -ms-text-align-last: justify;}th{width: 180px;text-align: left;}</style><table cellspacing='0' style='font-family:黑体;font-size: 10px;font-weight: bold;'><tr><td>居住地</td><td>:</td><th rowspan='2'>" + $scope.liveAddr + "</th></tr><tr><td>地址</td><td>:</td></tr><tr><td>有效期限</td><td>:</td><th>" + $scope.validdate + "</th></tr></table>");
			saveDeviceStatus("ResidenceErased", 0, "正常", 0, 0, 0, 0);
			$.device.GoHome();
		}, 100);
	}
});