...
app.controller("archivesMain", function($scope, $state, appData) {
	...
	$scope.toPrint = function(id, code, stuffName, guideline, a, b, c, d) {
		...
		appData.type = "print";
		if(stuffName == "承包地确权登记档案") {
			window.location.href = "../DA_cbdqqdj/index.html"
		} else if(stuffName == "农民建房审批档案") {
			window.location.href = "../DA_farmerBuildApproval/index.html"
		} else if(stuffName == "上海市三八红旗手档案") {
			window.location.href = "../DA_march8RedFlagBearer/index.html"
		} else {
			$state.go("loginType");
		}
	}
...
app.controller("guideline", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	if($scope.stuffName == "涉外婚姻登记档案查询") {
		$scope.info = "<p>1、主管部门：市档案局</p>" +
			...
			"<p>3、申请材料：申请对象需提供以下材料的原件：</p>" +
			"<p>（1）有效身份证明原件（居民身份证、护照）；</p>" +
		...
		$scope.info = "<p>1、主管部门：市档案局</p>" +
			...
			"<table cellspacing='0' style='border: #477cbe 1px solid;'><tr><th style='width:8%'>序号</th><th style='width:19%'>档案馆</th><th>咨询电话</th><th>地址</th></tr><tr><td>1</td><td>上海市档案馆</td><td>63336633-1515</td><td>中山东二路9号（200002）</td></tr>" +
...
app.controller('archivesLoginType', function($state, $scope, appData, $http) {
	...
	$scope.choice = function() {
		if($scope.stuffName == "婚姻登记档案查询") {
			$state.go("infoMarry");
		} else if($scope.stuffName == "独生子女证档案查询") {
			$state.go("IdCardType");
		...
		} else if($scope.stuffName == "知青返城档案查询") {
		...
		} else if($scope.stuffName == "复员退伍军人档案查询") {
...
app.controller("infoMarry", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		...
		$scope.param = {
			...
			"query": {
				...
				"condition_detail": { //查询条件
					"Q_LIKE_MAN": (appData.submitInfo.stSex == '男')?appData.submitInfo.stName:appData.submitInfo.stWomen,
					"Q_LIKE_WOMAN":  (appData.submitInfo.stSex == '男')?appData.submitInfo.stWomen:appData.submitInfo.stName,
					"Q_EQ_DISTRICT": ""
				}
			}
		};
		...
		//							"Q_LIKE_MAN": "蔡晔",
		//							"Q_LIKE_WOMAN": "梁艳",
		//							//					"Q_LIKE_MANBORNDATE": "xxxx",
		//							//					"Q_LIKE_WOMANBORNDATE": "xxxx",
		...
		$scope.submit = function() {
			...
			$.ajax({
				...
				success: function(dataJson) {
					...
					//					if(dataJsons[0].hasOwnProperty("datas")) {
					//						$scope.isAlert = true;
					//						$scope.msg = "提交成功";
					//						$scope.alertConfirm = function() {
					//							$state.go("submit");
...
app.controller("preview", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
			...
			success: function(data) {
				...
				try {
					...
					if(data.pngData.length == 0) {
						...
						$scope.printPdf = $.getConfigMsg.preUrl+data.str;
					}
				} catch(e) {
					console.log(e);
					$scope.isAlert = true;
	...
	$scope.print = function() {
		...
		//		for(var i in $scope.totalList) {
		//			console.log($scope.totalList[i].png);
		//			if($scope.totalList.length > 1) {
		//				console.log($scope.totalList.length);
		//				lodop.NewPage();
		//			}
		//			lodop.ADD_PRINT_IMAGE(0, 0, 800, 1300, "<img border='0' src='" + $scope.totalList[i].png + "'>");
		...
		$.device.httpDownload( // 调用壳的方法下载服务器上的图片，再上传到另一接口中		urlHost + 
			...
			function(result) {
				...
				recordUsingHistory('档案查询', '查询+打印', appData.stuffName, appData.archivesName, appData.archivesNumber, '', '', JSON.stringify($scope.jsonStr));
				//行为分析(查询)
