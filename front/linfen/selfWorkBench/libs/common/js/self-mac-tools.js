...
function saveDeviceStatus(stOutDeviceCode, nmException, stCause, nmNotice, nmTotal, nmRemain, count) {
	$.ajax({
		...
		data: {
			...
			stOutDeviceCode: stOutDeviceCode,
...
jQuery.device = {
	...
	Face_Show: function(width, height, x, y, callback) {
		try {
			...
			if(window.external.GetConfig('liveDetection') == 'N') {
				...
				setTimeout(function() {
					let CameraName = window.external.GetConfig('camera');
					let ResolvingPowerIndex = window.external.GetConfig('resolution') || 1;
					window.external.VideoCapture_Link(CameraName, ResolvingPowerIndex);
				}, 2000)
				setTimeout(function() {
					window.external.VideoCapture_Show();
				...
				setTimeout(function() {
					let dataBase64 = window.external.VideoCapture_Capture_Base64();
					callback(dataBase64)
				}, 8000)
				setTimeout(function() {
					window.external.VideoCapture_UnLink();
	...
	medicalPrint: function(newBookBh, newBookKh, userXb, userName, userAge) {
		try {
			...
			let lodop = $.device.printGetLodop('medical');
			let bookMedicalName = window.external.GetConfig('medicalName') || "DASCOM DS-7860";
			if(window.external.GetConfig('liveDetection') == 'N') {
				window.external.Medical_Open();
				window.external.Medical_Move();
			} else {
				let bookMedicalPort = "COM" + (window.external.GetConfig('bookMedicalPort') || "4");
				$.device.serialPortOpen(bookMedicalPort, 9600, 8, function() {}) //开启串口
			}
			$.log.debug("打开的医保打印机-----------" + bookMedicalName);
			if(bookMedicalName == "DASCOM DS-7860") {
				$.log.debug("开始打印-----------DASCOM DS-7860");
				lodop.SET_PRINTER_INDEX("DASCOM DS-7860");
				lodop.ADD_PRINT_TEXT(230, 140, 670, 50, newBookBh);
				lodop.SET_PRINT_STYLEA(0, "FontSize", 16);
				lodop.ADD_PRINT_TEXT(270, 140, 670, 30, userName);
				lodop.SET_PRINT_STYLEA(0, "FontSize", 16);
				lodop.ADD_PRINT_TEXT(305, 140, 670, 30, userXb);
				lodop.SET_PRINT_STYLEA(0, "FontSize", 16);
				lodop.ADD_PRINT_TEXT(340, 140, 670, 30, newBookKh);
				lodop.SET_PRINT_STYLEA(0, "FontSize", 16);
				lodop.ADD_PRINT_TEXT(375, 140, 670, 30, userAge);
				lodop.SET_PRINT_STYLEA(0, "FontSize", 16);
				lodop.PRINT(); //打印
				setTimeout(function() {
	...
	urlPdfPrint: function(url, filePath, callback) {
		try {
			var this_ = this
			this_.senseSend("O(00,12,2)E")
			setTimeout(function() {
