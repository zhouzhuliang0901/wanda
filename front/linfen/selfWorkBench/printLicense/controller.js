...
app.controller("licenseMain", function($scope, $state, appData) {
	...
	$scope.Handle = function(url, banli, stuffName) {
		appData.url = url;
		appData.type = banli;
		appData.stuffName = stuffName;
		appData.person = $scope.person;
...
app.controller('apply', function($state, $scope, appData, $http) {
	...
	$scope.encryptDataByRSA = function() {
		$http.jsonp('http://hengshui.5uban.com/xhac/aci/workPlatform/util/encryptDataByRSA.do', {
			...
			recordUsingHistory('证明开具', '办理', appData.stuffName, $scope.licenseName, $scope.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
			//行为分析(办理)
			trackEventForAffairs("", appData.funName, "上海市电子证照库", appData.licenseName, appData.licenseNumber, "");
			window.external.URL_OPEN(200, 180, 1500, 700, $scope.url + encodeURIComponent(appData.url));
		}).error(function(err) {
...
app.controller("elicensePreview", function($scope, $state, appData, $timeout) {
	...
	$scope.print = function() {
		...
		$timeout(function() {
			...
			recordUsingHistory('证明开具', '查询+打印', appData.stuffName, $scope.licenseName, $scope.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
			//行为分析(查询)
			trackEventForQuery(appData.stuffName, "", "打印", "上海市电子证照库", $scope.licenseName, appData.licenseNumber, "");
