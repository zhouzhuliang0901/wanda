app.controller('personChoice', function($scope, $state, appData, $sce, appFactory) {
	$scope.isAlert = false;
	appData.funName = "首次申请律师专职执业审核";
	appData.itemCode = "310100284000-01";
...
app.controller('info', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.politicalList = political;
	$scope.educationList = education;
	$scope.credentialsTypeList = credentialsType;
	$scope.credentialsWayList = credentialsWay;
	//个人信息
	...
	$scope.political = appData.political;
	$scope.education = appData.education;
	$scope.graduation = appData.graduation;
	$scope.specialty = appData.specialty;
	$scope.hjProvince = appData.hjProvince;
	$scope.hjCity = appData.hjCity;
	$scope.hjCounty = appData.hjCounty;
	$scope.jzProvince = appData.jzProvince;
	$scope.jzCity = appData.jzCity;
	$scope.jzCounty = appData.jzCounty;
	$scope.address = appData.address;
	$scope.postCode = appData.postCode;
	$scope.score = appData.score;
	$scope.email = appData.email;
	$scope.sxjg = appData.sxjg;
	$scope.nzyjg = appData.nzyjg;
	$scope.internshipStartDate = appData.internshipStartDate;
	$scope.internshipEndDate = appData.internshipEndDate
	$scope.credentialsType = appData.credentialsType;
	$scope.credentialsCode = appData.credentialsCode;
	$scope.credentialsWay = appData.credentialsWay;
	$scope.credentialsDate = appData.credentialsDate;
	$scope.gwlszg = appData.gwlszg;
	$scope.gwxw = appData.gwxw;
	$scope.bldCode = appData.bldCode;
	$scope.nationList = nations;
	$scope.hjProvinceList = $rootScope.allList;
	$scope.jzProvinceList = $rootScope.allList;
	//实习机构
	$scope.sxjgList = mechanism;
	//拟执业机构
	$scope.nzyjgList = mechanism;
	//办理点
	$scope.bldList = bld;
	//监听省市 区域变化
	$timeout(function() {
		...
		$scope.$watch("hjProvince", function(val) {
			if(val) {
				$scope.hjCityList = "";
				appFactory.getDictionaries(val.code, function(dataJson) {
					$scope.hjCityList = dataJson.data;
				});
			}
		});
		$scope.$watch("hjCity", function(val) {
			if(val) {
				$scope.hjCountyList = "";
				appFactory.getDictionaries(val.code, function(dataJson) {
					$scope.hjCountyList = dataJson.data;
					if($scope.hjCountyList.length == 0) {
						$scope.hjActive = true;
					} else {
		...
		$scope.$watch("jzProvince", function(val) {
			if(val) {
				$scope.jzCityList = "";
				appFactory.getDictionaries(val.code, function(dataJson) {
					$scope.jzCityList = dataJson.data;
				});
			}
		});
		$scope.$watch("jzCity", function(val) {
			if(val) {
				$scope.jzCountyList = "";
				appFactory.getDictionaries(val.code, function(dataJson) {
					$scope.jzCountyList = dataJson.data;
					if($scope.jzCountyList.length == 0) {
						$scope.jzActive = true;
					} else {
		...
		$scope.$watch("sxjg", function(val) {
			if(val) {
				console.log($scope.sxjg);
				$scope.sxjgId = getKeyByName(mechanism, val);
				console.log($scope.sxjgId);
			}
		});
		$scope.$watch("nzyjg", function(val) {
			if(val) {
				console.log($scope.nzyjg);
				$scope.nzyjgId = getKeyByName(mechanism, val);
				console.log($scope.nzyjgId);
			}
		});
	}, 100);
		...
		var rec = $.ajax({
			...
			success: function(res) {
				...
				if(res.SUCCESS === true) {
					...
					$scope.getUserInfoByAccessToken(token);
				} else {
					$scope.isAlert = true;
					$scope.msg = '未获得对应的用户标识！';
					$scope.alertConfirm = function() {
	...
	$scope.nextStep = function() {
		...
		do {
			...
			if(isBlank($scope.political)) {
				$scope.isAlert = true;
				$scope.msg = "请选择政治面貌！";
				return;
			}
			if(isBlank($scope.education)) {
				$scope.isAlert = true;
				$scope.msg = "请选择学历！";
				return;
			}
			if(isBlank($scope.graduation)) {
				$scope.isAlert = true;
				$scope.msg = "请输入毕业院校！";
				return;
			}
			...
			if(isBlank($scope.hjCounty) && $scope.hjActive == false) {
				$scope.isAlert = true;
				$scope.msg = "请选择户籍地址（区）！";
				return;
			}
			...
			if(isBlank($scope.jzCounty) && $scope.jzActive == false) {
				$scope.isAlert = true;
				$scope.msg = "请选择考试省市（区）！";
				return;
			}
			if(isBlank($scope.score)) {
				$scope.isAlert = true;
				$scope.msg = "请输入考试成绩！";
				return;
			}
			if(isBlank($scope.sxjg)) {
				$scope.isAlert = true;
				$scope.msg = "请选择实习机构！";
				return;
			}
			if(isBlank($scope.nzyjg)) {
				$scope.isAlert = true;
				$scope.msg = "请选择拟执业机构！";
				return;
			}
			if(isBlank($('#internshipStartDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择实习开始日期！";
				return;
			}
			if(isBlank($('#internshipEndDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择实习结束日期！";
				return;
			}
			if(isBlank($scope.credentialsType)) {
				$scope.isAlert = true;
				$scope.msg = "请选择资格证类别！";
				return;
			}
			if(isBlank($scope.credentialsCode)) {
				$scope.isAlert = true;
				$scope.msg = "请输入资格证号！";
				return;
			}
			if(isBlank($scope.credentialsWay)) {
				$scope.isAlert = true;
				$scope.msg = "请选择资格证取得方式！";
				return;
			}
			if(isBlank($('#credentialsDate').val())) {
				$scope.isAlert = true;
				$scope.msg = "请选择资格证取得日期！";
				return;
			}
		...
		appData.political = $scope.political;
		appData.education = $scope.education;
		appData.graduation = $scope.graduation;
		appData.specialty = $scope.specialty;
		...
		appData.score = $scope.score;
		appData.email = $scope.email;
		appData.sxjg = $scope.sxjg;
		appData.nzyjg = $scope.nzyjg;
		appData.internshipStartDate = $('#internshipStartDate').val();
		// 获取某个时间格式的时间戳
		appData.internshipStartDateUnix = Date.parse(new Date(appData.internshipStartDate));
		appData.internshipEndDate = $('#internshipEndDate').val();
		appData.internshipEndDateUnix = Date.parse(new Date(appData.internshipEndDate));
		appData.credentialsType = $scope.credentialsType;
		appData.credentialsCode = $scope.credentialsCode;
		appData.credentialsWay = $scope.credentialsWay;
		appData.credentialsDate = $('#credentialsDate').val();
		appData.credentialsDateUnix = Date.parse(new Date(appData.credentialsDate));
		appData.gwlszg = $scope.gwlszg;
		appData.gwxw = $scope.gwxw;
		//实习机构  拟职业机构   办理点
		appData.sxjg = $scope.sxjg;
		appData.sxjgId = $scope.sxjgId;
		appData.nzyjg = $scope.nzyjg;
		appData.nzyjgId = $scope.nzyjgId;
		appData.bldCode = $scope.bldCode;
		//户口省市提交字段处理
		if(isBlank(appData.hjCounty)) {
			appData.hukou = {
				"items": [appData.hjProvince.code, appData.hjCity.code],
				"names": [appData.hjProvince.name, appData.hjCity.name],
				"value": appData.hjProvince.code + " " + appData.hjCity.code
			}
		} else {
			appData.hukou = {
				"items": [appData.hjProvince.code, appData.hjCity.code, appData.hjCounty.code],
				"names": [appData.hjProvince.name, appData.hjCity.name, appData.hjCounty.name],
				"value": appData.hjProvince.code + " " + appData.hjCity.code + " " + appData.hjCounty.code
			}
		}
		//考试省市提交字段处理
		if(isBlank(appData.jzCounty)) {
			appData.examCity = {
				"items": [appData.jzProvince.code, appData.jzCity.code],
				"names": [appData.jzProvince.name, appData.jzCity.name],
				"value": appData.jzProvince.code + " " + appData.jzCity.code
			}
		} else {
			appData.examCity = {
				"items": [appData.jzProvince.code, appData.jzCity.code, appData.jzCounty.code],
				"names": [appData.jzProvince.name, appData.jzCity.name, appData.jzCounty.name],
				"value": appData.jzProvince.code + " " + appData.jzCity.code + " " + appData.jzCounty.code
			}
		}
		if(appData.handle == "agent") {
			$state.go("agentInfo");
...
app.controller('pickUpMethod', function($state, $scope, appData, $location, appFactory, $timeout, $rootScope) {
	...
	$scope.nextStep = function() {
		...
		$scope.params = {
			"data": {
				...
				"info": {
					...
					"applicant_beginDate": {
						"unix": appData.internshipStartDateUnix,
						"value": appData.internshipStartDate
					},
					"applicant_birthday": appData.dtBirth,
					"applicant_discipline": appData.specialty,
					"applicant_education": {
						"name": appData.education.name,
						"value": appData.education.id
					},
					"applicant_endDate": {
						"unix": appData.internshipEndDateUnix,
						"value": appData.internshipEndDate
					},
					"applicant_examCity": appData.examCity,
					"applicant_examScore": appData.score,
					"applicant_foreignLawyerQualification": appData.gwlszg,
					"applicant_gender": {
						"name": appData.stSex,
						"value": appData.stSex == "男" ? "a" : "b"
					},
					"applicant_graduatedFrom": appData.graduation,
					"applicant_hukou": appData.hukou,
					...
					"applicant_phone": appData.stMobile,
					"applicant_politic": {
						"name": appData.political.name,
						"value": appData.political.id
					},
					"applicant_practiceOrg": {
						"items": [appData.sxjgId],
						"names": [appData.sxjg],
						"value": appData.sxjgId
					},
					"applicant_qualificationDate": {
						"unix": appData.credentialsDateUnix,
						"value": appData.credentialsDate
					},
					"applicant_qualificationNumber": appData.credentialsCode,
					"applicant_qualificationType": {
						"name": appData.credentialsType.name,
						"value": appData.credentialsType.id
					},
					"applicant_qualificationWay": {
						"name": appData.credentialsWay.name,
						"value": appData.credentialsWay.id
					},
					"applicant_studyAbroadCountryAndDegree": appData.gwxw,
					...
					"base_willOrganization": {
						"items": [appData.nzyjgId],
						"names": [appData.nzyjg],
						"value": appData.nzyjgId
					},
		...
		$scope.saveApplyInfo = function() {
			$.ajax({
				type: "get",
				url: $.getConfigMsg.preUrlSelf + "/selfapi/lawyerPractice/applyLawyerPracticeForTheFirst.do",
				dataType: "jsonp",
				jsonp: "jsonpCallback",
				data: {
				...
				success: function(dataJson) {
					...
					if(dataJson.isSuccess == true) {
						...
						$scope.msg = "保存办件信息成功";
						//上传材料
						appFactory.pro_fetch($scope.licenseNumber, appData.licenseName, appData.funName, appData.VALIDSTARTDAY, appData.VALIDENDDAY, appData.applyNo, function(dataJson, dataJson1) {
...
app.controller("materialList", function($scope, $state, $http, appData, $timeout, appFactory, $rootScope) {
	...
	$scope.btn = function() {
		...
		$scope.getItemStuffs = function() {
			$.ajax({
				type: "get",
				url: $.getConfigMsg.preUrl + "/aci/getItemStuffs.do",
				dataType: "jsonp",
				jsonp: "jsonpCallback",
				data: {
					itemCode: appData.itemCode
...
app.controller("submit", function($scope, $state, appData, $sce, appFactory) {
	...
	trackEventForAffairs($scope.applyNo,'办理','上海市公安局',appData.licenseName,appData.licenseNumber,appData.stMobile)
	var date = new Date();
