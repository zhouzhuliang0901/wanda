...
app.controller("info", function($scope, $state, appData, $sce, appFactory) {
	...
	$scope.funName = "同日出生查询";
	$scope.isAlert = false;
	$scope.searchInfo = false;
	$scope.man = false;
	$scope.woman = false;
	$scope.concel = "false";
	$scope.SexList = [{"sex":'男'},{"sex":'女'}];
	...
	if ($scope.Sex == '女') {
		setTimeout(function(){
			$("#nations").find("option[value='object:7']").attr('selected','selected');
		},300)
	} else {
		setTimeout(function(){
			$("#nations").find("option[value='object:6']").attr('selected','selected');
		},300)
	}
	...
	$scope.search = function() {
		...
		recordUsingHistory('公安服务', '查询', '同日出生人数查询', appData.licenseName, appData.licenseNumber, '', '', JSON.stringify($scope.jsonStr));
		trackEventForQuery('同日出生人数查询','','查询','上海市公安局',appData.licenseName,appData.licenseNumber,'');
	}
	$scope.prevStep = function() {
		$state.go("loginType");
	}
