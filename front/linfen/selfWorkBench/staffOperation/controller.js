...
app.controller("medicalBookDetails", function($scope, $state, appData, appFactory, $http, $timeout, $rootScope) {
	...
	$scope.print = function(jlch, kh, xm, xb, sfzh) {
		...
		var age = sfzh.substring(6, 10) + "年" + sfzh.substring(10, 12) + "月";
		$scope.confirm = function() {
			$.device.medicalPrint(jlch, kh, xb, xm, age);
			...
			recordUsingHistory('工作人员操作', '打印', '办理门急诊就医记录册(补打)', xm, sfzh, '', '', JSON.stringify($scope.jsonStr));
			setTimeout(function() {
