...
app.controller("licenseTakeController", function(
	...
	$scope.loginRequest = function(code, isRe) {
		...
		$http
			.jsonp("http://31.0.178.111/autoTeminal/aci/window/getQrCodeInfoByElectronicCert.do", {
				params: $scope.httpConfig,
				timeout: 10000
			})
			.success(function(data) {
				if(data.result.data!=undefined&&data.result.data.idcard!="") {
	...
	$scope.readIdCard = function() {
		$.device.idCardOpen(function(value) {
			...
			if(list != null) {
				...
				appData.ImageUrlStr = $.device.fileBase64(appData.idCardImg);
				//				$scope.getImgbaseStr(
...
app.controller("faceVerificationController", function(
	...
	$.device.Camera_Link("USB Camera", 1); //初始化摄像头
	$.device.Camera_Show();
...
app.controller("verificationAwaitController", function(
	...
	$scope.recognition = function() {
		...
		$scope.recognitionAjax = $.ajax({
			type: "post",
			url: "http://31.0.178.111:8088/ac-product-ext/ext/aci/autoterminal/facecompare.do",
			async: true,
			data: {
...
app.controller("scanCodeController", function(
	...
	$scope.zfbs = function(str) {
		...
		console.log(str.substr(-(x.toString().length), x.toString().length));
		if(x == str.substr(-(x.toString().length), x.toString().length)) {
			return true;
		} else {
			return false;
		}
