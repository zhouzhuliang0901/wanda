...
//		urlHost + '/aci/declare/getAllItemListForPage.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api/selfapi/guidance/getAllItemListForPage', {
			params: config
		}).success(function(dataJson) {
...
//		urlHost + '/aci/declare/getOrganListForDeclarePage.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api/selfapi/guidance/getOrganListForDeclarePage', {
			params: organConfig
		}).success(function(dataJson) {
			console.log(dataJson)
			$scope.isLoading = true;
	...
	/**
	 * func: 通过部门organId获取部门事项
	 * author: 李华熙
	 * date: 2019/10/14
	 * */
	$scope.getItemByOrganId = function(organId){
		$scope.isLoading = false;
		$scope.current = 1;
		var oConfig = {
			jsonpCallback: "JSON_CALLBACK",
...
//		urlHost + '/aci/declare/getItemListByOrganCodeForPage.do'
	...
	$scope.toMaterials = function(itemName, organCode, organName, itemNo, organId) {
		removeAnimate($('.linkbox1'))
		if($scope.scrollMove){
		...
		if(itemName){
			...
			$scope.getItemByOrganId(organId);
		}
		addAnimate($('.linkbox1'))
	};
