 ...
 */
jQuery.device = {
	//wonders zhuofansoft epoint
	vendor: function() {
		if(new RegExp("^SelfTerminal.ObjectForScriptingHelper").test(window.external)) {
			return "wonders";
		} else if(window.BSID18 instanceof Object) {
			return "zhuofansoft";
		} else if(new RegExp("^WinEtouch.frmEtouch").test(window.external)) {
			return "epoint";
		}
	},
	SarkInit: function() {
	...
	Camera_Init: function(width, height, x, y) {
		try {
			...
			if(this.vendor() == 'wonders') {
				...
				window._PayCamLive_OpenForm_Params_ = {
					'width': width,
					'height': height,
					'x': x,
					'y': y
				}
				PayCamLive.OpenForm(width, height, x, y);
				PayCamLive.StartVedio();
			} else if(this.vendor() == 'epoint') {
				window._openCommonLive_Params_ = x + " " + y + " " + width + " " + height + " shanghai 0.4";
				window.external.openCommonLive(window._openCommonLive_Params_);
			}
		} catch(error) {}
	},
	Camera_Link: function(CameraName, ResolvingPowerIndex) {
		try {
			...
			} else if(this.vendor() == 'zhuofansoft') {} else if(this.vendor() == 'epoint') {}
		} catch(error) {}
	},
	Camera_Show: function() {
		try {
			if(this.vendor() == 'wonders') {
				...
				PayCamLive.OpenForm(window._PayCamLive_OpenForm_Params_.width, window._PayCamLive_OpenForm_Params_.height, window._PayCamLive_OpenForm_Params_.x, window._PayCamLive_OpenForm_Params_.y);
			} else if(this.vendor() == 'epoint') {
				window.external.openCommonLive(window._openCommonLive_Params_);
			}
		} catch(error) {
	...
	Camera_Hide: function() {
		try {
			if(this.vendor() == 'wonders') {
				...
				PayCamLive.StopVedio();
				PayCamLive.CloseWindow();
			} else if(this.vendor() == 'epoint') {
				window.external.hideCommonLive();
				window.external.closeCommonLive();
			}
		} catch(error) {}
	},
	...
	Camera_Base64: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				/**关闭准备好的摄像头*/
				callback(window.external.VideoCapture_Capture_Base64());
			} else if(this.vendor() == 'zhuofansoft') {
				var data = PayCamLive.GetManualCatchImage();
				data = JSON.parse(data);
				callback(data.image);
			} else if(this.vendor() == 'epoint') {
				window.returnCommonLivefile = function(value) {
					var data = JSON.parse(value);
					callback(data.content);
				};
				window.external.scanCommonLive();
			}
		} catch(error) {}
	},
	//活体检测
	Face_Show: function(width, height, x, y, callback) {
		try {
			if(this.vendor() == 'wonders') {
				...
				PayCamLive.OpenForm(width, height, x, y);
				PayCamLive.StartVedio();
				PayCamLive.StartFaceDetection();
				window.PayCam_LiveDetectStatusEvent = function(value) {
					if(value == 3) {
						var d = PayCamLive.GetQualifiedImageByIndex(0);
						var data = JSON.parse(d);
						PayCamLive.StopLiveDetection();
						PayCamLive.StopVedio();
						callback(data.image);
					}
				};
			} else if(this.vendor() == 'epoint') {
				window.returnCommonLivefile_huoti = function(value) {
					var data = JSON.parse(value);
					callback(data.content);
				};
				window.external.openCommonLive(x + " " + y + " " + width + " " + height + " shanghai 0.4");
			}
		} catch(error) {}
	},
	//关闭活体检测
	...
	qrCodeOpen: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				...
				SprScan.OpenAlway();
				window.EP_QR_SCAN = {};
				window.EP_QR_SCAN.SprScan_Callback = function(data) {
					callback(data);
				}
			} else if(this.vendor() == 'epoint') {
				window.external.ScanBarCode();
				window.Scanewm = function(value) {
					var data = JSON.parse(value);
					callback(data.content);
				}
			}
	...
	qrCodeClose: function() {
		try {
			if(this.vendor() == 'wonders') {
				...
				window.external.StopScanBarCode();
			}
		} catch(e) {}
	},
	...
	idCardOpen: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				...
				BSID18.ReadIDCardInfoBackAndHold("idCardCallBack");
				window.idCardCallBack = function(data) {
					if(data.success) {
						var idata = {};
						idata.Name = data.idCardInfo.name;
						idata.Sex = data.idCardInfo.sex;
						idata.Number = data.idCardInfo.number;
						idata.Address = data.idCardInfo.address;
						idata.CardImagePath = data.idCardInfo.Image;
						idata.People = data.idCardInfo.nation;
						var oBirth = data.idCardInfo.birthday;
						var birth = oBirth.substring(0, 4) + "-" + oBirth.substring(4, 6) + "-" + oBirth.substring(6, 8);
						idata.Birthday = birth;
						var timelimit = data.idCardInfo.timeLimit;
						var times = timelimit.split("-");
						var start = times[0].substring(0, 4) + "-" + times[0].substring(4, 6) + "-" + times[0].substring(6, 8);
						var end = times[1].substring(0, 4) + "-" + times[1].substring(4, 6) + "-" + times[1].substring(6, 8);
						idata.ValidtermOfStart = start;
						idata.ValidtermOfEnd = end;
						callback(JSON.stringify(idata));
					} else {
						callback("{}");
					}
				}
				...
				window.returnCardID = function(value) {
					var data = JSON.parse(value);
					var idata = {};
					idata.name = data.name;
					idata.Number = data.code;
					idata.CardImagePath = data.pic;
					idata.People = data.folk;
					callback(JSON.stringify(idata));
				}
			}
		} catch(e) {}
	...
	cmCaptureShow: function(width, height, x, y) {
		try {
			if(this.vendor() == 'wonders') {
				...
				EloamCom.openVideo(width, height, x, y, 0);
				EloamCom.SetDeskew(true);
			} else if(this.vendor() == 'epoint') {
				window.external.openVideoLT(width + " " + height + " " + x + " " + y + " " + " 1");
				window.external.showVideoLT();
			}
		} catch(e) {}
	},
	...
	cmCaptureCaptureImageModel: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				...
				window.returnLTfile = function(value) {
					var data = JSON.parse(value);
					callback(data.content);
				};
				window.external.scanLT();
			}
		} catch(e) {}
	},
	cmCaptureCaptureUrl: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				callback(window.external.CmCapture_Capture_Url());
			} else if(this.vendor() == 'zhuofansoft') {
				callback("");
			} else if(this.vendor() == 'epoint') {
				window.returnLTfile = function(value) {
	...
	cmCaptureCaptureBase64: function(callback) {
		try {
			if(this.vendor() == 'wonders') {
				callback(window.external.CmCapture_Capture_Base64());
			} else if(this.vendor() == 'zhuofansoft') {
				callback("");
			} else if(this.vendor() == 'epoint') {
				window.returnLTfile = function(value) {
	...
	cmCaptureHide: function() {
		try {
			if(this.vendor() == 'wonders') {
				...
				window.external.hideVideoLT();
				window.external.closeVideoLT()
			}
		} catch(e) {}
	},
