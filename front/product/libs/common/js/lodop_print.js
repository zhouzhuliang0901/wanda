(function (w) {
    ...
    w.LODOP_PRINT.idcardPrint = function (frontImg, backImg, printQuantity) {
        lodop.ADD_PRINT_IMAGE(
            140,
            220,
            880,
            1000,
            "<img border='0' style='width:320px;height:205px;' src='" +
            frontImg +
            "'>"
        );
        lodop.ADD_PRINT_IMAGE(
         ...
         * 日期: 2019.08.13
         */
        lodop.SET_PRINT_COPIES(printQuantity);
        lodop.PRINT();
        // lodop.PREVIEW();
    };
    // 材料复印
    w.LODOP_PRINT.materialPrint = function (img, printQuantity) {
        lodop.ADD_PRINT_IMAGE(
            0,
            0,
            880,
    ...
    w.LODOP_PRINT.faceRecognition = function (photograph, portrait, info) {
        var SerialNumber = function () {
            ...
            if (localStorage.getItem("SerialTimer") == null) {
                //第一次打印注入时间戳
                localStorage.setItem("SerialTimer", Date.parse(new Date()));
            }
            if (
                localStorage.getItem("SerialTimer") != null &&
                Date.parse(new Date()) - localStorage.getItem("SerialTimer") >
                1000 * 3600 * 12
            ) {
                //时间戳存在并已经过12个小时，则清空流水号以及时间戳
            ...
            if (localStorage.getItem("SerialNumber") == null) {
                ...
                //流水号保有原格式递增
                var num =
                    parseInt(localStorage.getItem("SerialNumber")) + 1 + "";
        ...
        printBtn = function () {
            ...
            lodop.ADD_PRINT_TEXT(
                ...
                200,
                80,
                NowDate + localStorage.getItem("SerialNumber")
            );
            ...
            lodop.ADD_PRINT_IMAGE(
                ...
                "100%",
                "100%",
                "<img border='0' style='width:350px;height:256px;' src='" +
                photograph +
                "'>"
            ); //核验照片
