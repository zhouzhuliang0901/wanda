...
//				$scope.dept = true;
	...
	$scope.personalItem = function() {
		...
		$scope.deptindex = 0;
		$scope.current = 0;
		//		urlHost + '/aci/declare/getItemTheme.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api/selfapi/onlineapply/getItemTheme', {
			params: {
	...
	$scope.getDepartment = function() {
		...
		//		urlHost + '/aci/declare/getDeptInArea.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api/selfapi/onlineapply/getItemTheme', {
			params: {
				jsonpCallback: "JSON_CALLBACK",
				type: '3'
...
//			$scope.itemName = dataJson.data;
	...
	$scope.getItemListByType = function(code, type) {
		...
		//		urlHost + '/aci/declare/getItemInTheme.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api/selfapi/onlineapply/getItemInTheme', {
			params: tConfig
		}).success(function(dataJson) {
			$scope.itemName = dataJson.data;
			addAnimate($('.linkbox1'))
	 ...
	 * date: 2019/10/14
	 **/
	$scope.getSearchDepartment = function (themeCode, type) {
		var tConfig = {
		jsonpCallback: "JSON_CALLBACK",
			//			areaCode: $scope.areaCode,
			themeCode: themeCode,
	...
	$scope.toItemTypeMaterials = function(name, code, type) {
		...
		if(type) {
			...
			if(code == "全部" && type == "1") {
				...
				$scope.getSearchDepartment(name, type);
			}
		} else {
			data.themeCode = name;
	...
	$scope.toMaterials = function(itemName, organCode, organName, itemNo, itemTypeName) {
		addAnimate($('.linkbox1'))
		$scope.isLoading = false;
		if(itemName) {
...
//		urlHost + '/aci/declare/getWindowItemStatusList.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api//selfapi/onlineapply/getWindowItemStatusList', {
			params: oConfig
		}).success(function(dataJson) {
...
//		urlHost + '/aci/declare/getGuideInfoByZhallId.do'
		$http.jsonp('http://10.2.101.72:8080/ac-self-api//selfapi/guidance/getGuideInfoByZhallId', {
			params: oConfig
		}).success(function(dataJson) {
...
app.controller("materialPicController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		data.selectImg = "http://218.202.254.222" + $scope.imgUrls[$scope.current].pictureUrlForBytes;
...
app.controller("infoFinishController", function($scope, $route, $http, $location, data, $timeout) {
	...
	var code = "http://218.202.254.222/aci/workBench_1/sqfw/index.html#/qrCode?applyNo=" + data.applyNo;
	var date = new Date();
