...
var EloamGlobalRet =OCX_Scan.InitDevs(); //初始化高拍仪
...
var OcxControl = {
    ...
    reSaveGray: function (fileName, success) { //图片灰化
    ...
    scanSaveRotate: function (fileName, success) { //图片宽大于高时 顺时针旋转90度
        var EloamGlobal = document.getElementById("OCX_Scan");
        var ret = EloamGlobal.InitDevs();
        var srcImg = EloamGlobal.CreateImageFromFile(fileName, 0);
        var h = srcImg.GetHeight();
        var w = srcImg.GetWidth();
        if (w > h) {
            srcImg.Rotate(-90.00, 0x80, 1);
            srcImg.Save(fileName, 0);
            success && success(fileName);
        }
    },
    mergeImages: function (imgsObj, success, error) { //合并图像
...
//			 OCX_idCard.CloseConnection();
	  		 OCX_idCard.Open("IdCardReader",100000);
//			 OCX_idCard.ReadImage(100000, 100000, that.variable.idCardSaveUrl);
    ...
    Barcode: function (response, error) {
        ...
        var result = OCX_Barcode.Open("BCR", 10000);
        if (result == -1) {
            error && error("Device open failed!");
        }
    },
    ...
    scanSave2: function (response, error) {
        //保存图片  返回图片名称
        var that = this;
        var image = that.variable.EloamVideo.CreateImage(0, EloamView.GetView());
        if (image) {
    ...
    laserPrint: function (type, printNums, response, error) {
        ...
        OCX_LaserPrint.attachEvent("OpenCompleted", function () {
            console.log("OpenCompleted");
            var status = OCX_LaserPrint.PrintRawData('papercount=1;file1=d:/wondersPrint/' + type + '.html');
    ...
    cameraOpen: function (pos, response, error) {
        ...
        if (result == -1) {
            error && error("camera open is failed!");
        } else if (result == 0) {
    ...
    cameraClose: function (response, error) {
        if (OCX_Camera) {
            ...
            console.log("StopDisplay" + result);
            if (result == 0) {
    ...
    Light: {
        ...
        qrcodeLightClose: function () {
            OCX_Light.SetGuidLigtEx("SCANNER", "OFF");
        },
        //关闭所有灯
        allLightClose: function () {
            this.Light.idCardLightClose();
            this.Light.qrcodeLightClose();
            this.Light.laserPrint();
            this.Light.receiptprinterLightClose();
            this.Light.documentprinterLightClose();
            this.Light.ukeyLightClose();
        }
    },
    OCXDeviceClose: function () {
        // 关闭所有设备
        try {
            var idcardResult = OCX_idCard.CloseConnection();
        } catch (error) {

        }

        try {
            var idcardBarcode = OCX_Barcode.CloseConnection();

        } catch (error) {

        }
        try {
            var idcardPrint = OCX_ReceiptPrint.CloseConnection();
        ...
        try {
            var idcardShield = OCX_Shield.CloseConnection();
