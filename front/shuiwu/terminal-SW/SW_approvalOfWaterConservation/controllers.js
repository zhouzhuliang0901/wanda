...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [
		{"stStatusName":"新办","itemId":"77ad4264-6d5f-4025-9927-ec3679ab2228"},
		{"stStatusName":"依申请变更","itemId":"61bb3f6a-7a73-4e25-b9a9-10deb1e69db7"}];
	$scope.statusItemList = [{
			"stStatusName": "保持设施的审批",
			"stStatusIndex": 1
		},
		{
			"stStatusName": "保持设施的验收",
			"stStatusIndex": 2
		},
	];
...
//	$scope.getSearchMatter();
	...
	$scope.getSubItem = function(type,itemId) {
		...
		data.itemName = type === "新办" ? data.itemName+"-新办" : data.itemName+"-依申请变更";
	}
...
//		$location.path("/select");
		...
		+"&href=SW_approvalOfWaterConservation"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {// 准专门处理水务局河道管理范围内建设项目的审核事项
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100548000-02-s" : "310100548000-01-s";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.selected_division = $scope.DIVISION[0]
	//分类
	$scope.TYPE = TYPE
	$scope.selected_type = $scope.TYPE[0]
	...
	var info = {
			"ASSESSCORPORATION": $scope.ASSESSCORPORATION,
			"expId": "",
			"MONITORCORPORATION": $scope.MONITORCORPORATION,
			...
			"stuff516": [],
			"mdata": "",
			"userType": "法人",
			"ST_ORG_TYPE": {
			...
			"SUPERVISIONCORPORATION": $scope.SUPERVISIONCORPORATION,
			...
			"STARTTIME": {
				"value": $("#STARTTIME").val(),
				"unix": Date.parse(new Date($('#STARTTIME').val()))
			},
			"DWTEL": $scope.DWTEL,
			"stuff2211": [],
			"stuff2213": [],
			...
			"DIVISION": {
				"name": $scope.selected_division.name,
				"value": $scope.selected_division.value
			},
			...
			"ENDTIME": {
				"value": $("#ENDTIME").val(),
				"unix": Date.parse(new Date($('#ENDTIME').val()))
			},
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"stuffPreFill": {},
			"APPPOST": $scope.APPPOST,
			"CONSTRUCTIONCORPORATION": $scope.CONSTRUCTIONCORPORATION,
			"mobile": $scope.mobile
		}
		var fConfig =
			{
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100548000-02-y" : "310100548000-01-y";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
