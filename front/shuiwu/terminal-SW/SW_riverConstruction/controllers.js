...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "河道管理范围内建设项目施工方案的审核";
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "9d0630b6-157d-44d6-8ccd-f50a7d326588"
		},
		{
			"stStatusName": "延续",
			"itemId": "fd96946d-9746-44f1-841e-d694a01bb78e"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_riverConstruction"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	if(data.statusName == "新办"){
		data.itemCode = "310100523000-01";
	}else{
		data.itemCode = "310100523000-02";
	}
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"mdata": "",
			"userType": "法人",
			"ST_ORG_TYPE": {
				"name": $scope.selected_org_type.name,
				"value": $scope.selected_org_type.value
			},
			"SGLINKEMAIL": $scope.SGLINKEMAIL,
			"PROJECTSCOPE": $scope.PROJECTSCOPE,
			"username": data.idCardName,
			"licenseNo": data.idCardNum,
			"eformCode": data.itemCode,
			"SGCOMPANY": $scope.SGCOMPANY,
			"COMPANYCODE": $scope.COMPANYCODE,
			"SGLINKPHONE": $scope.SGLINKPHONE,
			"JLCOMPANY": $scope.JLCOMPANY,
			...
			"SGLINKMAN": $scope.SGLINKMAN,
			"ARTIFICIALPERSON": $scope.ARTIFICIALPERSON,
			"PROJECTNAME": $scope.PROJECTNAME,
			"DWTEL": $scope.DWTEL,
			"SGLINKMPHONE": $scope.SGLINKMPHONE,
			"LINKMPHONE": $scope.LINKMPHONE,
			"SGLINKFAX": $scope.SGLINKFAX,
			"PROJECTADDRESS": $scope.PROJECTADDRESS,
			"ARTFAX": $scope.ARTFAX,
			"SJCOMPANY": $scope.SJCOMPANY,
