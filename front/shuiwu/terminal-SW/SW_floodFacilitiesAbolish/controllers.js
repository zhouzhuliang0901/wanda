...
app.controller("guidelineController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "防汛工程设施废除的审批";
	$scope.guideTitle = data.itemName;
	data.itemId = "637de0b2-d4e2-4463-adf9-e157a17be17e";// 获取相对应的办事指南
...
//		$location.path("/select");
		window.location.replace("../login/index.html#/select?itemName="+data.itemName
		+"&statusIndex="+data.statusIndex+"&statusName="+data.statusName
		+"&nmBelong="+$scope.nmBelong
		+"&href=SW_floodFacilitiesAbolish"+"&type="+data.type+"&deal="+data.deal.indexOf("自然人"));
	}
});
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout) {
	...
	// 证件类型
	$scope.alicenseType = alicenseType;
	$scope.mlicenseType = $scope.alicenseType[0];
	...
	$scope.goNext = function() {
		...
		var info = {
					...
					"stuff19n10732": [],
					"FXGCSSJGTIME": {
						"value": $('#FXGCSSJGTIME').val(),
						"unix": Date.parse(new Date($('#FXGCSSJGTIME').val()))
					},
					"stuff19n00245": [],
					"CONTACT_NAME": $scope.CONTACT_NAME,
					"FXGCSSMC": $scope.FXGCSSMC,
					"expId": "",
					...
					"FXGCSSCDHKD": $scope.FXGCSSCDHKD,
					"mdata": "",
					"FCYY": $scope.FCYY,
					"APPLIDATE": {
						"value": $('#APPLIDATE').val(),
					...
					"licenseType": {
						"name": $scope.mlicenseType.name,
						"value": $scope.mlicenseType.value
					},
					...
					"FXGCSSDZ": $scope.FXGCSSDZ,
					"EMALL": $scope.EMALL,
					"LINKMAN": $scope.LINKMAN,
					"FXGCSJSY": $scope.FXGCSJSY,
...
app.controller("infoFinishController", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	$http.jsonp($.getConfigMsg.declareUrl + '/aci/declare/submitCNApply.do', {
