...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "412fc67f-b6ce-43d0-81dd-02755583a2fe"
		},
		{
			"stStatusName": "延续",
			"itemId": "7bdb7694-bc19-49a7-b402-6c58e595385d"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_pluggingDrainagePipes"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = data.statusName === "新办" ? "310100543000-01" : "310100543000-02";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			"GDFDSGDWYX": $scope.GDFDSGDWYX,
			"GDFDSGDWCZ1": $scope.GDFDSGDWCZ1,
			"mdata": "",
			"userType": "法人",
			...
			"JHQFRQ": {
				"value": $("#JHQFRQ").val(),
				"unix": Date.parse(new Date($('#JHQFRQ').val()))
			},
			"GDFDSGDWDH1": $scope.GDFDSGDWDH1,
			"SGDWFZR": $scope.SGDWFZR,
			"licenseType": {
			...
			"DWTEL": $scope.DWTEL,
			"GDFDSGDWLXR": $scope.GDFDSGDWLXR,
			"PROCESSID": $scope.PROCESSID,
			"FACS": $scope.FACS,
			"stuffPreFill": {
				"cert": {}
			},
			"GDFDSGDWCZ": $scope.GDFDSGDWCZ,
			"mobile": $scope.mobile,
			"GDFDSGDWFZR": $scope.GDFDSGDWFZR,
			"QYZYZSBH": $scope.QYZYZSBH,
			"CONTACT_NAME": $scope.CONTACT_NAME,
			"stuff19n00245": [],
			"GDFDSGDWDH": $scope.GDFDSGDWDH,
			"expId": "",
			"LSFDGJ": $scope.LSFDGJ,
			...
			"GDFDSGDWLXR1": $scope.GDFDSGDWLXR1,
			"XYQK": $scope.XYQK,
			"LINKPHONE": $scope.LINKPHONE,
			"SGDWYB": $scope.SGDWYB,
			"GDFDSGDWSJ": $scope.GDFDSGDWSJ,
			"stuff19n10723": [],
			"GDFDSGDWYX1": $scope.GDFDSGDWYX1,
			"LSFDWZ": $scope.LSFDWZ,
			"SGDWDZ": $scope.SGDWDZ,
			"GDFDSGDWSJ1": $scope.GDFDSGDWSJ1,
			"EMALL": $scope.EMALL,
			"LINKMAN": $scope.LINKMAN,
			"SJQFRQ": {
				"value": $("#SJQFRQ").val(),
				"unix": Date.parse(new Date($('#SJQFRQ').val()))
			},
			...
			"LSFDRQ": {
				"value": $("#LSFDRQ").val(),
				"unix": Date.parse(new Date($('#LSFDRQ').val()))
			}
		};
		var fConfig = {
