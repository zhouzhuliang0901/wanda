...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.totalPages = 1;
	$scope.itemList = [
		{"stStatusName":"依申请变更","itemId":"45b86019-7004-40cf-8a3f-5aa316ccc646","index":1},
		{"stStatusName":"延续","itemId":"55a37085-829a-4b3f-9922-a6fc9f883eba","index":2},
		{"stStatusName":"新办(市批非建设项目)","itemId":"63705683-bef1-4da7-8702-92c5b57e4ab0","index":4},
		{"stStatusName":"新办(市批建设项目)","itemId":"07ea3a09-618c-4489-b6f8-1bf713f0f4b1","index":5}];
...
//		$location.path("/select");
		...
		+"&href=SW_verificationOfSeaRight"+"&type="+$scope.type+"&itemIndex="+$scope.itemIndex
		+"&deal="+$scope.deal);
	}
});
...
//	if(){
//		data.itemCode = "310101227000-01";
//	}
//	
	data.itemCode = "310101227000-0" + data.itemIndex
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.selected_usetype = $scope.USETYPE[0]
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"USETYPEAREATHR": $scope.USETYPEAREATHR,
			"mdata": "",
			"userType": "法人",
			"username": data.idCardName,
			"eformCode": data.itemCode,
			"USETYPEJTYTTWO": $scope.USETYPEJTYTTWO,
			...
			"IDCARD": $scope.IDCARD,
			"USETYPEJTYTONE": $scope.USETYPEJTYTONE,
			"USETYPEJTYTFOU": $scope.USETYPEJTYTFOU,
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"stuffPreFill": {
				"cert": {},
				"img": {}
			},
			"USETHREE": $scope.USETHREE,
			"mobile": $scope.mobile,
			...
			"USETYPE": {
				"items": [$scope.selected_usetype.value],
				"names": [$scope.selected_usetype.name],
				"value": $scope.selected_usetype.value
			},
			"stuff19n00245": [],
			"expId": "",
			...
			"USETYPEAREATWO": $scope.USETYPEAREATWO,
			"ST_LINK_NAME": $scope.ST_LINK_NAME,
			"ADDRESS": $scope.ADDRESS,
			"USEFOUR": $scope.USEFOUR,
			"USETOTALAREA": $scope.USETOTALAREA,
			"USEYEAR": $scope.USEYEAR,
			"ARTFAX": $scope.ARTFAX,
			"USETYPEAREAFOU": $scope.USETYPEAREAFOU,
			"LINKMAN": $scope.LINKMAN,
			"DWEMAIL": $scope.DWEMAIL,
			"USETYPEJTYTTHR": $scope.USETYPEJTYTTHR,
			"ARTPHONE": $scope.ARTPHONE,
			"USETWO": $scope.USETWO,
			"USETYPEAREAONE": $scope.USETYPEAREAONE,
