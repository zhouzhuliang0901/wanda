...
app.controller("printController", function ($scope, $route, $location, $rootScope, $timeout) {
	...
	$scope.whereToGo = $location.search().whereToGo;
	if ($scope.PRINT_TYPE == undefined && $scope.PRINT_IMGAGE_TYPE == undefined && $scope.PRINT_IMGFILEPATH === undefined) {
		OcxControl.Light.documentprinterLightOpen();
		$timeout(function () {
			OcxControl.Light.documentprinterLightClose();
			if($scope.whereToGo==undefined){

				window.external.GoHome();
			}else{
				$location.path($scope.whereToGo).search({});
			}
		}, 6000);
	} else {
