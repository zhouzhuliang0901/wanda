...
window.OcxControl = {
	...
	reSaveGray: function(fileName, success) {
		...
		var newImg = EloamGlobal.CreateImage(w + 5, h + 5, 3);
	...
	reSaveImage: function(fileName, success) {
		//图片重新存储
		var EloamGlobal = document.getElementById("OCX_Scan");
		var srcImg = EloamGlobal.CreateImageFromFile(fileName, 0);
		var h = srcImg.GetHeight();
		...
		if(result == true) {
			newImg.Save(fileName, 0);
			OcxControl.imgWatermark({
				height: 1000,
				width: 1000
			}, [fileName, "d:/bg.png"], "d:/imgWatermark.jpg");
			success && success(fileName);
		}
	},
	...
	imgWatermark: function(targetSize, imgArr, targetFile, success, error) {
		//targetSize<height,width> 图片路径 frontImgfile backImgfile 目标存储 targetFile
		var EloamGlobal = document.getElementById("OCX_Scan");
		var frontImg = EloamGlobal.CreateImageFromFile(imgArr[0], 0);
		var backImg = EloamGlobal.CreateImageFromFile(imgArr[1], 0);
		var mergeImg = EloamGlobal.CreateImage(targetSize.width, targetSize.height, 3);

		var frontRcDest = EloamGlobal.CreateRect(0, 0, targetSize.width, targetSize.height);
		var frontRcSrc = EloamGlobal.CreateRect(0, 0, targetSize.width, targetSize.height);

		var backRcDest = EloamGlobal.CreateRect(0, 0, targetSize.width, targetSize.height);
		var backRcSrc = EloamGlobal.CreateRect(0, 0, targetSize.width, targetSize.height);
		...
		if(mergeImg) {
			...
			if(result1 === true) {
				...
				if(result2 === true) {
					// mergeImg.ToGray();
					mergeImg.Save(targetFile, 0);

					success && success(targetFile);
				} else {
