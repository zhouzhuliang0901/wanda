(function (global) {
  ...
  PROMISE_METHOD.fetchGet = function (method, data) {
    ...
    return new Promise(function (resolve, reject) {
      $.ajax({
        url: $.getConfigMsg.declareUrl + method,
        type: "get",
        dataType: "jsonp",
        jsonp: "jsonpCallback",
        data: data,
  ...
  PROMISE_METHOD.imageReSaveGray = function (targetName, type) {
    //将图片存为灰度图片
    return new Promise(function (resolve, reject) {
      if (type == undefined || type == 'gray') {
        OcxControl.scanSaveRotate(targetName, function (err) {
          console.log(err);
        });
