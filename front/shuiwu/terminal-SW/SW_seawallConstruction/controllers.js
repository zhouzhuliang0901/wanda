...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "271dd7d8-c31a-4a4c-b367-2edafac3aca4"
		},
		{
			"stStatusName": "依申请变更",
			"itemId": "5b5c093e-4fd7-4b87-9a54-1784706ca8f5"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_seawallConstruction"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = data.statusName === "新办" ? "310101241000-02" : "310101241000-01";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	// 申请类别
	$scope.SQTYPE2 = SQTYPE2;
	$scope.selected_sqtype2 = $scope.SQTYPE2[0];
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"JSCOPE": $scope.JSCOPE,
			"SQTYPE2": {
				"name": $scope.selected_sqtype2.name,
				"value": $scope.selected_sqtype2.value
			},
			"DISTRICTS": {
			...
			"mdata": "",
			"userType": "法人",
			"ST_ORG_TYPE": {
			...
			"RIVERSCOPE": $scope.RIVERSCOPE,
			"APPADDRESS": $scope.APPADDRESS,
			"TETIME": {
				"value": $('#TETIME').val(),
				"unix": Date.parse(new Date($('#TETIME').val()))
			},
			"ST_LINK_NAME": $scope.ST_LINK_NAME,
			"TEXTSCOPE": $scope.TEXTSCOPE,
