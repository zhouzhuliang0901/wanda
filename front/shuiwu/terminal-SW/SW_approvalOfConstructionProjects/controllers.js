...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "河道管理范围内建设项目工程建设方案审批";
	...
	$scope.itemList = [
		{"stStatusName":"新办","itemId":"5c1e1d25-f83c-48e4-92b8-5b3b98f00ec9"},
		{"stStatusName":"变更","itemId":"fc08f69a-8d28-4f01-93ee-2aa77205a2fe"}];
	$scope.statusItemList = [{
			"stStatusName": "社会投资项目、非排污口事项、不涉及排水口",
			"stStatusIndex": 1
		},
		{
			"stStatusName": "社会投资项目、非排污口事项、涉及排水口",
			"stStatusIndex": 2
		},
		{
			"stStatusName": "社会投资项目、排污口事项、设置或扩大排污口",
			"stStatusIndex": 3
		},
		{
			"stStatusName": "非社会投资项目、非排污口事项、不涉及排水口",
			"stStatusIndex": 4
		},
		{
			"stStatusName": "非社会投资项目、非排污口事项、涉及排水口",
			"stStatusIndex": 5
		},
		{
			"stStatusName": "非社会投资项目、排污口事项、设置或扩大排污口",
			"stStatusIndex": 6
		},
	];
...
//	$scope.getSearchMatter();
	...
	$scope.getSubItem = function(type,itemId) {
		$scope.chooseStatus = true;
		data.itemId = itemId;
		data.type = type === "新办" ? 0 : 1;
		data.itemName = type === "新办" ? data.itemName+"(新办)" : data.itemName+"(变更)";
	}
	
	$scope.chooseStatusItem = function(statusName, stStatusIndex) {
		data.statusIndex = stStatusIndex;
		data.statusName = statusName;
		$location.path("/guideline");  // 暂时跳过办事指南
	};
});
...
app.controller("preController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.next = function() {
		...
		+"&href=SW_approvalOfConstructionProjects"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
//	}
//});
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {// 准专门处理水务局河道管理范围内建设项目的审核事项
	// 登录流程相关
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-01" : "310100522000-02-01";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	// 投资项目类型
	$scope.TZXMLX = TZXMLX;
	$scope.selected_tzxmlx = $scope.TZXMLX[0];
	// 排污口事项类型
	$scope.PWKSXLX = PWKSXLX;
	$scope.selected_pwksxlx = $scope.PWKSXLX[0];
	// 涉及类型
	$scope.SJLX = SJLX;
	$scope.selected_sjlx = $scope.SJLX[0];
	// 行政区
	...
	$scope.selected_codetype = $scope.CODETYPE[0];
	// 建设性质
	$scope.JSTYPE = JSTYPE;
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"BY1": $scope.BY1,
			"LEGAL_PERS_DUTY": $scope.LEGAL_PERS_DUTY,
			"ZZJGLX": {
				"name": $scope.selected_zzjglx.name,
				"value": $scope.selected_zzjglx.value
			},
			"FXZRLXRCZ": $scope.FXZRLXRCZ,
			"targetNo": $scope.targetNo,
			"FXZRLXRSJ": $scope.FXZRLXRSJ,
			"mdata": "",
			"FWDATE": $('#FWDATE').val(),
			"userType": "法人",
			...
			"userId": "",
			"LEGAL_PERS_NAME": $scope.LEGAL_PERS_NAME,
			"JZAREA": $scope.JZAREA,
			"JSCONTENT": $scope.JSCONTENT,
			"TZXMLX": {
				"name": $scope.selected_tzxmlx.name,
				"value": $scope.selected_tzxmlx.value
			},
			"licenseType": {
				"name": "身份证",
				"value": "a"
			...
			"mobile": $scope.mobile,
			"FXZRLXRDH": $scope.FXZRLXRDH,
			"APPLIER": $scope.APPLIER,
			"CONTACT_NAME": $scope.CONTACT_NAME,
			"expId": "",
			"SJDW": $scope.SJDW,
			"PWKSXLX": {
				"name": $scope.selected_pwksxlx.name,
				"value": $scope.selected_pwksxlx.value
			},
			"APPLICATIONCONTENT": $scope.APPLICATIONCONTENT,
			"DISTRICTS": {
			...
			"SJLX": {
				"name": $scope.selected_sjlx.name,
				"value": $scope.selected_sjlx.value
			},
			"LINKMOBILEPHONE": $scope.LINKMOBILEPHONE,
			"ZYFXQAD": $scope.ZYFXQAD,
			"CODETYPE": {
				"name": $scope.selected_codetype.name,
				"value": $scope.selected_codetype.value
			},
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout) {
	// 登录流程相关
	var url = decodeURI(window.location.href);
	var temp1 = url.split('?');
    var pram = temp1[1];
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-02" : "310100522000-02-02";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.selected_codetype = $scope.CODETYPE[0];
	// 排水（污）口设置类型
	$scope.PSKSZLX = PSKSZLX;
	$scope.selected_pskszlx = $scope.PSKSZLX[0];
	// 排水（污）口性质
	$scope.PSKXZ = PSKXZ;
	$scope.selected_pskxz = $scope.PSKXZ[0];
	// 排放方式
	$scope.PFFS = PFFS;
	$scope.selected_pffs = $scope.PFFS[0];
	// 入河方式
	$scope.RHFS = RHFS;
	$scope.selected_rhfs = $scope.RHFS[0];
	// 建设性质
	$scope.JSTYPE = JSTYPE;
	$scope.selected_jstype = $scope.JSTYPE[0];
	// 立项或批准级别
	...
	var info = {
			...
			"PRDSGNQMC": $scope.PRDSGNQMC,
			"PSKSZLX": {
				"name": $scope.selected_pskszlx.name,
				"value": $scope.selected_pskszlx.value
			},
			"targetNo": $scope.TZTOTAL,
			"mdata": "",
			"FWDATE": {
				"value": $("#FWDATE").val(),
				"unix": Date.parse(new Date($('#FWDATE').val()))
			},
			"PSKXZ": {
				"name": $scope.selected_pskxz.name,
				"value": $scope.selected_pskxz.value
			},
			"userType": "法人",
			"LXPZFILENAME": $scope.LXPZFILENAME,
			"username": data.idCardName,
			...
			"FWJG": $scope.FWJG,
			"RHFS": {
				"name": $scope.selected_rhfs.name,
				"value": $scope.selected_rhfs.value
			},
			"ARTIFICIALPERSON": $scope.ARTIFICIALPERSON,
			"DWXZ": $scope.DWXZ,
			"JSTYPE": {
			...
			"PSKDX": $scope.PSKDX,
			"PFFS": {
				"name": $scope.selected_pffs.name,
				"value": $scope.selected_pffs.value
			},
			"JD": $scope.JD,
			"LXPZWAY": {
				"name": $scope.selected_lxpzway.name,
			...
			"CODETYPE": $scope.selected_codetype.name,
			"APPPOST": $scope.APPPOST,
			"FWMJ": $scope.FWMJ
		}	
		var fConfig =	
...
app.controller("riverInfo3Controller", function($scope, $route, $http, $location, data, $timeout) {
	// 登录流程相关
	var url = decodeURI(window.location.href);
	var temp1 = url.split('?');
    var pram = temp1[1];
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-03" : "310100522000-02-03";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.goNext = function() {
		...
		var info = {
			"NH3NPFND": $scope.NH3NPFND,
			"TZTOTAL": $scope.TZTOTAL,
			"LEGAL_PERS_DUTY": $scope.LEGAL_PERS_DUTY,
			"P2RPFZL": $scope.P2RPFZL,
			"CLFS": $scope.CLFS,
			"ZZJGLX": {
				"name": $scope.selected_zzjglx.name,
				"value": $scope.selected_zzjglx.value
			...
			"targetNo": $scope.targetNo,
			"mdata": "",
			"FWDATE": {
				"value": $("#FWDATE").val(),
				"unix": Date.parse(new Date($('#FWDATE').val()))
			},
			...
			"stuff2036": [],
			"targetName": $scope.targetName,
			"APPLIDATE":$("#APPLIDATE").val(),
			"SHWSPFZL": $scope.SHWSPFZL,
			"LXPZLEVEL": {
				"name": $scope.selected_lxpzlevel.name,
				"value": $scope.selected_lxpzlevel.value
			},
			...
			"CODPFND": $scope.CODPFND,
			"P1NPFZL": $scope.P1NPFZL,
			"LINKPHONE": $scope.LINKPHONE,
			"P3NPFZL": $scope.P3NPFZL,
			"ZLRPFZL": $scope.ZLRPFZL,
			"PRSTMC": $scope.PRSTMC,
			"CODRPFZL": $scope.CODRPFZL,
			"ADDRESS": $scope.ADDRESS,
			"JSDW": $scope.JSDW,
			"WSSFJGCL": $scope.WSSFJGCL,
			"P1PFND": $scope.P1PFND,
			"GJCODE": $scope.GJCODE,
			"SZXZQ": $scope.SZXZQ,
			"LOCATION": {
			...
			"LINKMAN": $scope.LINKMAN,
			"DWEMAIL": $scope.DWEMAIL,
			"CODNPFZL": $scope.CODNPFZL,
			"SJLX": {
				"name": $scope.selected_sjlx.name,
				"value": $scope.selected_sjlx.value
			},
			"P2NPFZL": $scope.P2NPFZL,
			"QTWSPFZL": $scope.QTWSPFZL,
			"CODETYPE": $scope.selected_codetype.name,
			"APPPOST": $scope.APPPOST,
			"FWMJ": $scope.FWMJ,
			"P4RPFZL": $scope.P4RPFZL
		}
...
app.controller("riverInfo4Controller", function($scope, $route, $http, $location, data, $timeout) {
	// 登录流程相关
	var url = decodeURI(window.location.href);
	var temp1 = url.split('?');
    var pram = temp1[1];
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-04" : "310100522000-02-04";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"FXZRLXRCZ": $scope.FXZRLXRCZ,
			"targetNo": $scope.targetNo,
			"mdata": "",
			"FXZRLXRSJ": $scope.FXZRLXRSJ,
			"userType": "法人",
			"username": data.idCardName,
			"eformCode": data.itemCode,
...
app.controller("riverInfo5Controller", function($scope, $route, $http, $location, data, $timeout) {
	// 登录流程相关
	var url = decodeURI(window.location.href);
	var temp1 = url.split('?');
    var pram = temp1[1];
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-05" : "310100522000-02-05";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"targetNo": $scope.targetNo,
			"PRDSGNQMC": $scope.PRDSGNQMC,
			"mdata": "",
			"PSKXZ": {
				"name": $scope.selected_pskxz.name,
				"value": $scope.selected_pskxz.value
			},
			...
			"stuff2036": [],
			"APPLICATIONCONTENT": $scope.APPLICATIONCONTENT,
			"targetName": $scope.targetNameTAL,
			"stuff2611": [],
			"ST_ORG_TYPE": {
				"name": $scope.selected_st_org_type.name,
				"value": $scope.selected_st_org_type.value
...
app.controller("riverInfo6Controller", function($scope, $route, $http, $location, data, $timeout) {
	// 登录流程相关
	var url = decodeURI(window.location.href);
	var temp1 = url.split('?');
    var pram = temp1[1];
	...
	$scope.itemName = name;
	data.itemCode = data.type == 0 ? "310100522000-01-06" : "310100522000-02-06";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"targetNo": $scope.targetNo,
			"PRDSGNQMC": $scope.PRDSGNQMC,
			"mdata": "",
			"PSKXZ": {
				"name": $scope.selected_pskxz.name,
				"value": $scope.selected_pskxz.value
			},
