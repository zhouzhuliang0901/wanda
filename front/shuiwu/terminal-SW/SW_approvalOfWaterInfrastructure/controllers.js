...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.totalPages = 1;
	$scope.itemList = [
		{"stStatusName":"新办","itemId":"7f47eaf4-0be8-4ab7-81f0-c95e9678f4e9"},
//		{"stStatusName":"延续","itemId":"7f47eaf4-0be8-4ab7-81f0-c95e9678f4e9"},
//		{"stStatusName":"变更","itemId":"7f47eaf4-0be8-4ab7-81f0-c95e9678f4e9"},
//		{"stStatusName":"补证","itemId":"7f47eaf4-0be8-4ab7-81f0-c95e9678f4e9"}
		];
...
//	$scope.getSearchMatter();
	...
	$scope.getSubItem = function(statusName,itemId,index) {
		...
		data.statusIndex = statusName == "补证" ? 3 : index +1;
...
app.controller("preController", function($scope, $route, $http, $location, data, $timeout) {
	// 判断每一个值是否是undefined，如果不是保存到localStroage.xxx中，方便以后点击上一步，找到对应的数据。
	if(undefined != data.itemName &&undefined != data.itemIndex && undefined != data.statusName && undefined != data.statusIndex&& undefined != data.type&& undefined != data.deal){
		localStorage.setItem("itemName",data.itemName);
		localStorage.setItem("itemIndex",data.itemIndex);
		localStorage.setItem("statusName",data.statusName);
...
//		$location.path("/select");
		...
		+"&href=SW_approvalOfWaterInfrastructure"+"&type="+$scope.type+"&itemIndex="+$scope.itemIndex
		+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"mdata": "",
			"userType": "法人",
			"ST_ORG_TYPE": {
			...
			"DESIGNCORPORATION": $scope.DESIGNCORPORATION,
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"EX3": {
				"items": [$scope.ex3],
				"names": [$scope.ex3.name],
				"value": $scope.ex3.value
			},
			"ARTPHONE": $scope.ARTPHONE,
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"stuffPreFill": {
