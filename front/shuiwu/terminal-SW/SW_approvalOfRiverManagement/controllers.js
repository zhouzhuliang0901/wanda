...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "河道管理范围内有关活动（不含河道采砂）审批";	
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "1b6ad776-964f-4d3d-af19-1f5566c3036e"
		},
		{
			"stStatusName": "延续",
			"itemId": "6ef98ffd-9837-4c9e-921c-42853b314252"
		},
	];
...
app.controller("preController", function($scope, $route, $http, $location, data, $timeout) {
	// 判断每一个值是否是undefined，如果不是保存到localStroage.xxx中，方便以后点击上一步，找到对应的数据。
	if(undefined != data.itemIndex &&undefined != data.itemName && undefined != data.statusName && undefined != data.statusIndex&& undefined != data.type&& undefined != data.deal){
		localStorage.setItem("itemName",data.itemName);
...
//		$location.path("/select");
		...
		+"&href=SW_approvalOfRiverManagement"+"&type="+$scope.type+"&itemIndex="+$scope.itemIndex
		+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = "310100526000-0" + data.itemIndex
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"stuff2364": [],
			"stuff2363": [],
			"mdata": "",
			"userType": "法人",
			"ST_ORG_TYPE": {
