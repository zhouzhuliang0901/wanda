app.controller("listController", function($scope, $route, $location, $http, data, $timeout) {
	...
	$scope.getMatterDetail = function(itemName, placeCode, organName, itemNo, itemTenNo) {
		data.itemName = itemName;
		data.organCode = placeCode;
...
app.controller("preController", function($scope, $route, $http, $location, data, $timeout) {
	...
	//滚动条上移
	$scope.moveUp = function() {
		$(".right-inner").mCustomScrollbar("scrollTo", "+=" + $scope.rollingSpeed);
	};
	//滚动条下移
	$scope.moveDown = function() {
		$(".right-inner").mCustomScrollbar("scrollTo", "-=" + $scope.rollingSpeed);
...
app.controller("applyController", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	$scope.itemName = name;
	// 设置跳转链接
	$scope.applyUrl = "https://zwdtuser.sh.gov.cn/uc/login/login.jsp?self=my&redirect_uri=http://ywtb.sh.gov.cn:18018" + localStorage.applyUrl + '&source=xuhui&type=pcac';
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	$scope.itemName = name;
	// http://ywtb.sh.gov.cn:18018/ac-product-net-ext/netapply/index.do?itemCode=0103022000-05-1-SHGAXH-JJ01&source=0&type=pcac&uniqueId=340881199303145313&personName=夏雷&peosonPhone=17621418416
	// 下面使用的是转发后的链接
	$scope.infoUrl = "http://31.0.178.111/ac-product-net-ext/netapply/index.do?itemCode=" + data.itemTenNo + '-' + data.organCode + "&source=0&type=pcac&uniqueId=" + data.idCardNum + "&personName=" + data.idCardName + "&peosonPhone=" + data.mobile;
	...
	$scope.goNext = function() {
		//		console.log( $(window.frames["infoForm"].document).find('form.form-horizontal').serialize() );
		if(!$scope.flag) {
			return ''
		}
		var form = $(window.frames["infoForm"].document).find('form.form-horizontal').serialize();
		
//		$("#infoForm")[0].contentWindow.submitsave();
		console.log( $("#infoForm")[0].contentWindow.submitsave() )
		//jyback();
		/*var fConfig = {
			'applyNo': '',
		...
		$http.jsonp(urlHost + '/aci/declare/saveApply.do?' + form, {
			params: fConfig
		}).success(function(dataJson) {
			$scope.flag = true;
...
app.controller("materialPicController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.profileShow = function() {
		$('.loading').show();
		$scope.imageShow1 = true;
		$scope.imageShow2 = false;
		$('.profile').css('background', '#1566aa');
		$('.elecCert').css('background', 'rgba(255, 255, 255, 0.2)');
		$.ajax({
			type: "get",
			async: true,
			dataType: "jsonp",
			...
			data: {
				...
				stIdNo: '412702199406084145' // data.idCardNum	
			},
			...
			success: function(json) {
				...
				if(!dataJson) { // !dataJson[0].address
					...
					var $temp1 = new ImageShow(".imageShow1");
					$('.loading').hide();
					data.profile = dataJson;
					data.profileFlag = 'personal';
					$temp1.SetUrls(data.profile, 'personal');
				}
			},
			error: function(err) {
				console.log(e)
	...
	$scope.elecShow = function() {
		$('.loading').show();
		$scope.imageShow1 = false;
		$scope.imageShow2 = true;
		$('.elecCert').css('background', '#1566aa');
		$('.profile').css('background', 'rgba(255, 255, 255, 0.2)');
		$.ajax({
			type: "get",
			async: true,
			dataType: "jsonp",
			jsonp: "jsonpCallback",
			url: "http://31.0.178.111/autoTeminal/aci/app/queryCertBaseDatas.do", // 徐汇
			data: {
				jsonpCallback: "JSON_CALLBACK",
				type: 0,
				certNo: '310226198609270312' // data.idCardNum	
			},
			success: function(json) {
				var dataJson = eval("(" + JSON.stringify(json) + ")");
				if(!dataJson) { // !dataJson[0].address
					...
					var $temp2 = new ImageShow(".imageShow2");
					$('.loading').hide();
					data.profile = dataJson;
					data.profileFlag = 'legal';
					$temp2.SetUrls(dataJson, 'legal');
				}
			},
			error: function(err) {
				console.log(e)
	...
	$scope.goNext = function() {
		var page = '';
		if(data.profileFlag == 'personal') {
			page = $('.imageShow1 .currentPage').html();
		} else {
			page = $('.imageShow2 .currentPage').html();
		}
		page = page.split('/')[0] - 1;
		data.page = page;
...
app.controller("takePhotoController", function($scope, $route, $http, $location, data, $timeout, $routeParams) {
	...
	if($routeParams.flag == 'U') { // U盘上传
		...
		if(data.profileFlag == 'personal') {
			...
			$scope.waitUploadImgUrl = 'data:image/jpeg;base64,' + data.profile[data.page].str;
			$scope.imgName = '电子证照图片';
			// 电子证照图片下载时需转化为url地址
			$.ajax({
				type: "get",
				async: true,
				dataType: "jsonp",
				jsonp: "jsonpCallback",
				url: urlHost + "/aci/app/queryCertBaseData.do", // 徐汇
				data: {
					jsonpCallback: "JSON_CALLBACK",
					type: 0,
					certNo: '310226198609270312' // data.idCardNum	
				...
				success: function(json) {
					$scope.waitUploadImgUrl2 = urlHost + json[data.page].pictureUrl;
				},
				error: function(err) {
					console.log(e)
				}
	...
	$scope.highCapture = function() { // 高拍仪拍照
		if($routeParams.flag == 'U') { // U盘上传
			...
			// 电子证照图片下载时需转化为url地址
			if(data.profileFlag != 'personal') {
				$scope.waitUploadImgUrl = $scope.waitUploadImgUrl2;
			}
