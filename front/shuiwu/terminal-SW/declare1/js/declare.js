...
//var urlHost = 'http://hengshui.5uban.com/ac-xuhui';			// 外网地址
var urlHost = 'http://31.0.178.111/autoTeminal'; // 数据最新的地址
var formalUrl = "http://ywtb.sh.gov.cn:18018"; // 正式网上申报链接
// 切换图片
var cn = '';
function ImageShow(className) {
	...
	this.SetUrls = function(urls,flag) {
		if(flag == 'personal') {
			...
			$.each(urls, function(index, value) {
				$node.find(".imageList").append('<img onclick="JumpImageShow(this)"  src="data:image/jpeg;base64,' +value.str + '" />');
			});
		}
		setTimeout('JumpImageShow($("' + className + '").find(".imageList>img").eq(0))');
	}
	$node.find(".left").click(function() {
		var num = $(this).parents(className).attr("nowImage");
		if(num > 0) {
			num -= 1;
