...
app.controller("guidelineController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "海域使用金减免审批";
	$scope.guideTitle = data.itemName;
	data.itemId = "9acea3fb-57c8-40e8-864c-46265e41a5c6";// 获取相对应的办事指南
...
//		$location.path("/select");
		...
		+"&href=SW_marineGoldBreaks"+"&type="+data.type+"&deal="+data.deal.indexOf("自然人"));
	}
});
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		...
		var info =  {
					...
					"mdata": "",
					"APPLIDATE": {
						"value": $('#APPLIDATE').val(),
					...
					"YHXZ": {
						"name": $scope.mYHXZ.name,
						"value": $scope.mYHXZ.value
					},
					"licenseType": {
...
//		layer.alert(data.isUpload.length);
//		layer.alert(data.stuffListLength);
