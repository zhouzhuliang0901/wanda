...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "98190c2d-cda4-4aa4-846a-10d4d36c4106"
		},
		{
			"stStatusName": "依申请变更",
			"itemId": "adecbf2e-81c4-453f-b587-7b9cbcac1b99"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_waterDetectionQualification"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = data.statusName === "新办" ? "310100554000-02" : "310100554000-01";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			"YQZS": $scope.YQZS,
			"CPOSITION": $scope.CPOSITION,
			"FZDEPART2": $scope.FZDEPART2,
			"ZZJGLX": {
			...
			"SLDATE": {
				"value": $('#SLDATE').val(),
				"unix": Date.parse(new Date($('#SLDATE').val()))
			},
