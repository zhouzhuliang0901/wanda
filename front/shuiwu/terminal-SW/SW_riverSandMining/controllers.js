...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "29627b4e-9308-4646-ba0b-e8d6af16a20a"
		},
		{
			"stStatusName": "延续",
			"itemId": "052f6437-0b7d-4222-97f2-48ad6a4a4507"
		},
		{
			"stStatusName": "依申请变更",
			"itemId": "0e1c4870-cf0b-411b-b991-3c7b70d3bb7b"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_riverSandMining"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	if(data.statusName == "新办"){
		data.itemCode = "310100534000-01";
	}else if(data.statusName == "延续"){
		data.itemCode = "310100534000-02";
	}else{
		data.itemCode = "310100534000-03";
	}
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"KAICAIDIDIAN": $scope.KAICAIDIDIAN,
			...
			"STOPTIME": {
				"value": $('#STOPTIME').val(),
				"unix": Date.parse(new Date($('#STOPTIME').val()))
			},
