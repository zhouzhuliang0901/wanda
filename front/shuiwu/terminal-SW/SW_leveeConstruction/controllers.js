...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "在一线河道堤防破堤施工或者开缺、凿洞的审核和审批";
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "4cab4a38-65fd-4db7-a032-9af3568efffe"
		},
		{
			"stStatusName": "延续",
			"itemId": "67bd7862-facc-4d07-8818-b8e6a0af346b"
		}
	];
...
//		$location.path("/select");
		...
		+"&href=SW_leveeConstruction"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = data.statusName === "新办" ? "310100527000-01" : "310100527000-02";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	$scope.goNext = function() {
		...
		var info = {
			"LINKMOBILEPHONE1": $scope.LINKMOBILEPHONE1,
			"FANGXUNEMAIL": $scope.FANGXUNEMAIL,
			"ZZJGLX": {
				"name": $scope.selected_zzjglx.name,
				"value": $scope.selected_zzjglx.value
			},
			"targetNo": $scope.targetNo,
			"mdata": "",
			"userType": "法人",
			...
			"LINKEMAIL1": $scope.LINKEMAIL1,
			"LINKMAN1": $scope.LINKMAN1,
			"LINKEMAIL": $scope.LINKEMAIL,
			"ARTIFICIALPERSON": $scope.ARTIFICIALPERSON,
			"PROJECTNAME": $scope.PROJECTNAME,
			"JLUNIT": $scope.JLUNIT,
			"LINKPHONE1": $scope.LINKPHONE1,
			"DWTEL": $scope.DWTEL,
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"APPPOST1": $scope.APPPOST1,
			"FANGXUNFAX": $scope.FANGXUNFAX,
			"ZJUNIT": $scope.ZJUNIT,
			"mobile": $scope.mobile,
			"QISHIDATE": {
				"value": $('#QISHIDATE').val(),
				"unix": Date.parse(new Date($('#QISHIDATE').val()))
			},
			"APPLIER": $scope.APPLIER,
			"WANGONGDATE": {
				"value": $('#WANGONGDATE').val(),
				"unix": Date.parse(new Date($('#WANGONGDATE').val()))
			},
			"expId": "",
			...
			"KQZDCD": $scope.KQZDCD,
			"FANGXUNMAN": $scope.FANGXUNMAN,
			"SGUNIT": $scope.SGUNIT,
			"licenseNo": data.idCardNum,
			"LINKFAX": $scope.LINKFAX,
			"SJUNIT": $scope.SJUNIT,
			"LINKPHONE": $scope.LINKPHONE,
			"LINKFAX1": $scope.LINKFAX1,
			"FXQSJTH": $scope.FXQSJTH,
			"SGADDRESS": $scope.SGADDRESS,
			"ARTIFICIALPERSON1": $scope.ARTIFICIALPERSON1,
			"ST_LINK_NAME": $scope.ST_LINK_NAME,
			"XCFBDATE": {
				"value": $('#XCFBDATE').val(),
				"unix": Date.parse(new Date($('#XCFBDATE').val()))
			},
			"FANGXUNPHONE": $scope.FANGXUNPHONE,
			"STARTTIME": {
				"value": $('#STARTTIME').val(),
				"unix": Date.parse(new Date($('#STARTTIME').val()))
			},
			"PROJECTADDRESS": $scope.PROJECTADDRESS,
			"FANGXUNMOBILEPHONE": $scope.FANGXUNMOBILEPHONE,
