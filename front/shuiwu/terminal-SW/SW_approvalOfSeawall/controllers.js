...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "在海塘堤防上破堤、开缺、凿洞施工的审批";	
	...
	$scope.totalPages = 1;
	$scope.itemList = [
		{"stStatusName":"新办","itemId":"5a1da067-8709-49aa-a4a3-e5736a5be6b2"},
		{"stStatusName":"延续","itemId":"2cbe48b1-c23a-4109-a0e5-b7933c978150"},
		{"stStatusName":"依申请变更","itemId":"df6ffe56-7542-4e02-9a58-63ff4ab05ab5"},
		];
...
//		$location.path("/select");
		...
		+"&href=SW_approvalOfSeawall"+"&type="+$scope.type+"&itemIndex="+$scope.itemIndex+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = "310100531000-0" + data.itemIndex;
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	laydate.render({
	  elem: '#LSHTJHWGSJ' ,//指定元素
	  trigger: 'click'
	});
	laydate.render({
	  elem: '#YJHTJHXCFBSJ' ,//指定元素
	  trigger: 'click'
	});
	...
	$scope.goNext = function() {
		...
		var info = {
			"YJHTJHXCFBSJ": {
				"value": $("#YJHTJHXCFBSJ").val(),
				"unix": Date.parse(new Date($('#YJHTJHXCFBSJ').val()))
			},
			"SGSJ": {
				"value": $("#SGSJ").val(),
				"unix": Date.parse(new Date($('#SGSJ').val()))
			},
			"SGDWSJ": $scope.SGDWSJ,
			"EX2": $scope.EX2,
			"ZZJGLX": {
			...
			"mdata": "",
			"userType": "法人",
			"username": data.idCardName,
			"FXDWZRLXRDH": $scope.FXDWZRLXRDH,
			...
			"FXDWZRLXRCZ": $scope.FXDWZRLXRCZ,
			"APPADDRESS": $scope.APPADDRESS,
			"FXDWZRLXRDZYX": $scope.FXDWZRLXRDZYX,
			...
			"DWTEL": $scope.DWTEL,
			"stuff2211": [],
			"FXDWZRLXRSJ": $scope.FXDWZRLXRSJ,
			"SGDWYX": $scope.SGDWYX,
			"stuff20n00076": [],
			"LSHTSJTH": $scope.LSHTSJTH,
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"stuffPreFill": {},
			"ZJDW": $scope.ZJDW,
			"mobile": $scope.mobile,
			"APPLIER": $scope.APPLIER,
			"SGDWLXR": $scope.SGDWLXR,
			"expId": "",
			"SJHT": $scope.SJHT,
			"SJDW": $scope.SJDW,
			"stuff2036": [],
			...
			"JLDW": $scope.JLDW,
			"SGDWFDDBR": $scope.SGDWFDDBR,
			"SGDWYB": $scope.SGDWYB,
			"LINKPHONE": $scope.LINKPHONE,
			"FXDWZRLXR": $scope.FXDWZRLXR,
			"ST_LINK_NAME": $scope.ST_LINK_NAME,
			"LSHTJHWGSJ": {
				"value": $("#LSHTJHWGSJ").val(),
				"unix": Date.parse(new Date($('#LSHTJHWGSJ').val()))
			},
