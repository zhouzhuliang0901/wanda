...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
			"stStatusName": "新办",
			"itemId": "13374f42-6f2e-4ed9-8997-8ae255d0bc63"
		},
		{
			"stStatusName": "延续",
			"itemId": "8f121022-2671-471c-919f-4d0deca96214"
		},
		{
			"stStatusName": "现场核验",
			"itemId": "ba3daf36-e13e-4559-962b-efb95dfa9ebd"
		},
		{
			"stStatusName": "依申请变更",
			"itemId": "f952d50b-ac2b-4172-90aa-ae8cfb53e0ef"
		},
	];
...
//	$scope.getSearchMatter();
	...
	$scope.getSubItem = function(statusName, itemId, index) {
		data.statusIndex = (statusName === "新办" || statusName === "现场核验") ? 1 : 2;
...
app.controller("preController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.next = function() {
		window.location.replace(encodeURI("../login/index.html#/select?itemName="+$scope.itemName
		+"&statusIndex="+$scope.statusIndex
		+"&statusName="+$scope.statusName
		+"&nmBelong="+$scope.nmBelong
		+"&href=SW_waterIntakePermit"
		+"&type="+$scope.type
		+"&deal="+$scope.deal));
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.itemCode = data.statusName === "新办" ? "310100536000-01" : "310100536000-01-02";
	if(data.itemCode) {
		$.ajax({
				type: "get",
	...
	//申请类型
	$scope.SHENPI2 = SHENPI2
	$scope.selected_shenpi2 = $scope.SHENPI2[0]
	//类型
	$scope.LEIXING = LEIXING
	$scope.selected_leixing = $scope.LEIXING[0]
	//受理
	$scope.SHOULI = SHOULI
	$scope.selected_shouli = $scope.SHOULI[0]
	//行业类别
	$scope.HYLB = HYLB
	...
	$scope.goNext = function() {
		...
		var info = {
			"SHOULI": {
				"name": $scope.selected_shouli.name,
				"value": $scope.selected_shouli.value
			},
			"HYLB": {
			...
			"REG_DATE": {
				"value": $("#REG_DATE").val(),
				"unix": Date.parse(new Date($('#REG_DATE').val()))
			},
			"licenseNo": data.idCardNum,
			"eformCode": data.itemCode,
			"FARENZHIWU": $scope.FARENZHIWU,
			"userId": "",
			...
			"EMALL": $scope.EMALL,
			"LEIXING": {
				"name": $scope.selected_leixing.name,
				"value": $scope.selected_leixing.value
			},
			"LINKZW": $scope.LINKZW,
			"LINKMAN": $scope.LINKMAN,
			"LINKDEPT": $scope.LINKDEPT,
			"stuffPreFill": {},
			"APPPOST": $scope.APPPOST,
			"SHENPI": {
				"name": $scope.selected_shenpi2.name,
				"value": $scope.selected_shenpi2.value
			},
			"mobile": $scope.mobile
		}
		var fConfig = {
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.fullItemName = data.itemName + "——" + data.statusName;
	$scope.itemName = data.itemName;
	data.itemCode = data.statusName === "依申请变更" ? "310100536000-02" : "310100536000-03";
	if(data.itemCode) {
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
	...
	$scope.selected_division = $scope.DIVISION[0]
	//申请类型
	$scope.SHENPI1 = SHENPI1
	$scope.selected_shenpi1 = $scope.SHENPI1[0]
	//类型
	$scope.LEIXING = LEIXING
	$scope.selected_leixing = $scope.LEIXING[0]
	//受理
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"APPPOST": $scope.APPPOST,
			"SHENPI": {
				"name": $scope.selected_shenpi1.name,
				"value": $scope.selected_shenpi1.value
			},
			"mobile": $scope.mobile
		}
