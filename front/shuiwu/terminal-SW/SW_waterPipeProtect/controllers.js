...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "原水引水管渠保护范围内实施建设工程的审批";
	...
	$scope.itemList = [{
		"stStatusName": "新办",
		"itemId": "046acc70-ef4e-43d2-8e0f-1048d737c070"
	}];
...
//		$location.path("/select");
		...
		+"&href=SW_waterPipeProtect"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"mdata": "",
			"userType": "法人",
			"username": data.idCardName,
			"eformCode": "310100542000-01",
			"COMPANYCODE": $scope.COMPANYCODE,
			"userId": "91c62fca-79f6-44a4-a65e-4f20f09a67a6",
			...
			"SGENDTIME": {
				"value": $('#SGENDTIME').val(),
				"unix": Date.parse(new Date($('#SGENDTIME').val()))
			},
			...
			"APPPOST2": $scope.APPPOST2,
			"APPPOST1": $scope.APPPOST1,
			"mobile": $scope.mobile,
			"APPADDRESS2": $scope.APPADDRESS2,
			"APPADDRESS1": $scope.APPADDRESS1,
			"APPLIER": $scope.APPLIER,
			"stuff19n00245": [],
			"expId": "",
			"DISTRICTS": {
				"name": selected_districts.name,
				"value": selected_districts.value
			},
			...
			"LCOMPANY": $scope.LCOMPANY,
			"licenseNo": data.idCardNum,
			"CONSTRUS": $scope.CONSTRUS,
			"LINKFAX": $scope.LINKFAX,
			"LINKPHONE": $scope.LINKFAX,
			"stuff19n10756": [],
			"SGADDRESS": $scope.SGADDRESS,
			"ST_LINK_NAME": $scope.ST_LINK_NAME,
			"ARTIFICIALPERSON1": $scope.ARTIFICIALPERSON1,
			"ARTIFICIALPERSON2": $scope.ARTIFICIALPERSON2,
			"SGSTARTTIME": {
				"value": $('#SGSTARTTIME').val(),
				"unix": Date.parse(new Date($('#SGSTARTTIME').val()))
			},
			"LRELATION": $scope.LRELATION,
			"PCONTENT": $scope.PCONTENT,
			"CONSTRUSQ": $scope.CONSTRUSQ,
			"LINKMPHONE": $scope.LINKMPHONE,
			"APPLIER2": $scope.APPLIER2,
			"ARTFAX": $scope.ARTFAX,
			"APPLIER1": $scope.APPLIER1,
