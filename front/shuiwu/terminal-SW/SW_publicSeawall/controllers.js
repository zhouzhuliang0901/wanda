...
app.controller("guidelineController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "公用岸段海塘使用的审批";
	$scope.guideTitle = data.itemName;
	data.itemId = "20bfff37-e7f8-4cb0-96cb-5796bdf9cfe4";// 获取相对应的办事指南
...
//		$location.path("/select");
		...
		+"&href=SW_publicSeawall"+"&type="+data.type+"&deal="+data.deal.indexOf("自然人"));
	}
});
...
app.controller("infoController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.goNext = function() {
		console.log($scope.mDISTRICTS);
		console.log(Date.parse(new Date($('#APPLIDATE').val())));
		// 判空操作，正式环境开放
		...
		var info = {
					...
					"mdata": "",
					"APPLIDATE": {
						"value": $('#APPLIDATE').val(),
					...
					"HAITANGADDRESS": $scope.HAITANGADDRESS,
					"userId": "",
