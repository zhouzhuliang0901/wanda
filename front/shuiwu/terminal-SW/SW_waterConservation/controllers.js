...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	$scope.itemList = [{
		"stStatusName": "新办",
		"itemId": "acfafae0-aba4-4cfa-ab08-5dc215e9704c"
	}];
...
//		$location.path("/select");
		...
		+"&href=SW_waterConservation"+"&type="+$scope.type+"&deal="+$scope.deal);
	}
});
...
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	//申请类型
	$scope.SQTYPE = SQTYPE
	$scope.selected_sqtype = $scope.SQTYPE[0]
	// 行业类别
	$scope.HYLB = HYLB
	$scope.selected_hylb = $scope.HYLB[0]
	//绿地浇灌节水装置
	$scope.GREENINGDEVICEY = GREENINGDEVICEY
	$scope.selected_greeningdevicey = $scope.GREENINGDEVICEY[0]
	//车辆清洁节水装置
	$scope.VEHICLEDEVICEN = VEHICLEDEVICEN
	$scope.selected_vehicledevicen = $scope.VEHICLEDEVICEN[0]
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"WSLBH": $scope.WSLBH,
			"SQTYPE": {
				"name": $scope.selected_sqtype.name,
				"value": $scope.selected_sqtype.value
			},
			"APPLIER": $scope.APPLIER,
			"HYLB": {
				"name": $scope.selected_hylb.name,
				"value": $scope.selected_hylb.value
			},
			"APPLCODE": $scope.APPLCODE,
			...
			"DESIGNUNINAME": $scope.DESIGNUNINAME,
			"DESIGNUNIADDRE": $scope.DESIGNUNIADDRE,
			"DESIGNPERSONRESPON": $scope.DESIGNPERSONRESPON,
			"DESIGNRESPONNUM": $scope.DESIGNRESPONNUM,
			"EVALUATIONCOMADDRE": $scope.EVALUATIONCOMADDRE,
			"EVALUATIONADDRESS": $scope.EVALUATIONADDRESS,
			"EVALUATIONCOMRESP": $scope.EVALUATIONCOMRESP,
			"EVALUATIONREPNUM": $scope.EVALUATIONREPNUM,
			"DATAOFSTART": {
				"value": $('#DATAOFSTART').val(),
				"unix": Date.parse(new Date($('#DATAOFSTART').val()))
			},
			"FINISHOFSTART": {
				"value": $('#FINISHOFSTART').val(),
				"unix": Date.parse(new Date($('#FINISHOFSTART').val()))
			},
			"INDRECTWATERRECYCING": $scope.INDRECTWATERRECYCING,
			"DIRECTUTILIZATION": $scope.DIRECTUTILIZATION,
			"BOILERRECYCLING": $scope.BOILERRECYCLING,
			"SEWAGERATE": $scope.SEWAGERATE,
			"GREENINGDEVICEY": {
				"name": $scope.selected_greeningdevicey.name,
				"value": $scope.selected_greeningdevicey.value
			},
			"VEHICLEDEVICEN": {
				"name": $scope.selected_vehicledevicen.name,
				"value": $scope.selected_vehicledevicen.value
			},
			"UNTRADITIONDEVICE": $scope.UNTRADITIONDEVICE,
			"RIVERWATERDEVICE": $scope.RIVERWATERDEVICE,
			"ADDTHAT": $scope.ADDTHAT,
			"userType": "法人",
			...
			"stuff19n10755": [],
			"stuff2373": [],
			"PROCESSWATERRATE": $scope.PROCESSWATERRATE,
			"stuff2371": []
		}
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"expId": "",
			"DISTRICTS": {
				"name": $scope.DISTRICTS,
				"value": ""
			},
			"targetName": $scope.targetName,
			"LXPZLEVEL": {
				"name": $scope.LXPZLEVEL,
				"value": ""
			},
			"REG_DATE": {
				"value": $('#REG_DATE').val(),
