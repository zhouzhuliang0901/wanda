...
app.controller("matterController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.listImg = [];
	data.fileName = [];
	data.itemName = "城镇污水排入排水管网许可" //"核发《排水许可证》";	
	...
	$scope.itemList = [{
			"stStatusName": "补证",
			"itemId": "70ef52e3-2293-4cb7-8084-95b6cddb87c4"
		},
		{
			"stStatusName": "新改扩",
			"itemId": "dc136dcb-0f13-4b7a-a9b1-7accf0e7346d"
		},
		{
			"stStatusName": "延续",
			"itemId": "e059d9d8-9725-4a7a-b866-495b7da3ca27"
		},
		{
			"stStatusName": "依申请变更",
			"itemId": "4006fbe1-2077-416b-9962-da76ef9c2e0b"
		}
	];
...
//	$scope.getSearchMatter();
	...
	$scope.getSubItem = function(statusName, itemId, index) {
		...
		data.statusIndex = statusName == "新改扩" ? 1 : 2;
...
//		$http.jsonp('http://180.169.7.194:8080/ac-product/aci/declare/getCNGuideInfoByZhallId.do', {
			params: oConfig
		}).success(function(dataJson) {
...
//		alert("00000000:"+$scope.nmBelong);
		...
		+"&href=SW_drainagePermit"+"&type="+$scope.type+"&deal="+$scope.deal
		);
	}
});
...
//	}
//});
app.controller("riverInfo1Controller", function($scope, $route, $http, $location, data, $timeout) {// 新改扩
	// 登录流程相关
...
//	data.itemCode = data.itemNo + "-0" + data.itemIndex
	data.itemCode = "310100544000-02";
	if(data.itemCode) {
		$.ajax({
			type: "get",
	...
	$scope.selected_org_type = $scope.ST_ORG_TYPE[0]
	// 证件类型
	$scope.ZZJGLX = ZZJGLX
	...
	// 项目性质 
	$scope.EX2 = EX2
	$scope.selected_ex2 = $scope.EX2[0]
	// 地理位置
	$scope.EX3 = EX3
	$scope.selected_ex3 = $scope.EX3[0]
	// 排水活动类型
	$scope.PSHTYPE = PSHTYPE
	$scope.selected_pshtype = $scope.PSHTYPE[0]
	...
	$scope.goNext = function() {
		...
		var info = {
			"expId": "",
			"PSHTYPE": {
				"items": [$scope.selected_pshtype],
				"names": [$scope.selected_pshtype.name],
				"value": $scope.selected_pshtype.value
			},
			"EX2": {
				"name": $scope.selected_ex2.name,
				"value": $scope.selected_ex2.value
			},
			"stuff2038": [],
			"stuff2037": [],
			"DISTRICTS": {
			...
			"mdata": "",
			"userType": "法人",
			"WSCLGY": $scope.WSCLGY,
			"ST_ORG_TYPE": {
				"name": $scope.selected_org_type.name,
				"value": $scope.selected_org_type.value
			},
			...
			"stuff2041": [],
			"EX3": {
				"items": [$scope.selected_ex3.value],
				"names": [$scope.selected_ex3.name],
				"value": $scope.selected_ex3.value
			},
			"ARTPHONE": $scope.ARTPHONE,
			"ST_LINK_ADDRESS": $scope.ST_LINK_ADDRESS,
			"stuffPreFill": {},
			"APPPOST": $scope.APPPOST,
			"ST_CONTENT": $scope.ST_CONTENT,
			"mobile": $scope.mobile
		};
		var fConfig = {
...
app.controller("riverInfo2Controller", function($scope, $route, $http, $location, data, $timeout) {
	...
	if(data.statusName=="补证"){
		data.itemCode="310100544000-01";
	}else if(data.statusName=="延续"){
		data.itemCode="310100544000-03";
	}else{
		data.itemCode="310100544000-04";
	}
//	data.itemCode = "310100544000-0" + data.itemIndex;
	if(data.itemCode) { 
		$.ajax({
				type: "get",
				url: $.getConfigMsg.declareUrl + '/aci/getItemApplyPlace.do?',
...
//	var temp = {name:"",value:""};
	...
	$scope.goNext = function() {
		...
		var info = {
			...
			"targetName": $scope.targetName,
			"mdata": "",
			"userType": "法人",
			"WSCLGY": $scope.WSCLGY,
			"ST_ORG_TYPE": {
				"name": $scope.selected_org_type.name,
...
app.controller("materialUploadController", function($scope, $route, $http, $location, data, $timeout) {
	...
	if(data.listImg == undefined){
		data.listImg = [];
	}
	if(data.isUpload == undefined){
		data.isUpload = [];
	}
...
app.controller("uploadMethodController", function($scope, $route, $http, $location, data, $timeout) {
	console.log("data.isOcxOpen==========uploadMethodController========"+data.isOcxOpen);
	if(data.isOcxOpen){
		data.isOcxOpen = false;
...
app.controller("finishController", function($scope, $route, $http, $location, data, $timeout) {
	...
	data.isOcxOpen = true;
	console.log("data.isOcxOpen=========finishController========="+data.isOcxOpen);
	// 重拍
...
app.controller("infoFinishController", function($scope, $route, $http, $location, data, $timeout, $sce) {
	...
	//text: 'http://zwdt.sh.gov.cn/zwdtSW/spnetQuery/bjxqxg2.jsp?ST_WF_ID=' + $scope.applyNo + '&IdOrCode=' + data.idCardName,
        text: 'http://ywtb.sh.gov.cn:18018/ac-product-net/codePic/showQRCode.do?unicode=' + $scope.applyNo + '&applicant=' + data.idCardName,
		width: 200,
		height: 200,
		correctLevel: 0,
		render: "table"
	...
	$scope.print = function() {
		f++;
		if($scope.statusText == '打印凭证' && f == 1) {
			$scope.statusText = '返回首页';
			OcxControl.receiptPrint(
				'自助申报申请回执\n' +
				'@qrcode@' + 'http://ywtb.sh.gov.cn:18018/ac-product-net/codePic/showQRCode.do?unicode=' + $scope.applyNo + '&applicant=' + data.idCardName + '@qrcode@\n' +
				'\n事项名称：' + $scope.allName +
				'\n事件编码：' + $scope.applyNo +
