...
app.controller("licenseTakeController", function(
	...
	$scope.loginRequest = function(code, isRe) {
		...
		$http
			.jsonp("http://zwdtyp.sh.gov.cn:8088/ac/aci/window/getInfoByCodeForLogin.do", {
	...
	$scope.recognition = function() {
		...
		$scope.recognitionAjax = $.ajax({
			...
			data: {
				account: "cuihuibin",
				password: "qwer",
				ST_IDCARD_NAME: appData.userName,
				ST_IDCARD_NO: appData.idCardNumber,
				idCard: appData.ImageUrlStr,
				photo: appData.capturePhoto
			},
			success: function(data) {
				$.log.debug(data);
			...
			$scope.getResideQuantity = function() {
				$.ajax({
					...
					success: function(res) {
						appData.resideQuantity = 30 - res.cabList.length;
